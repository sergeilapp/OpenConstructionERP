#!/usr/bin/env python3
"""Import the TCG review-slice package into OCERP.

Run from OCERP/backend so app imports resolve, for example:

    python ../scripts/import_tcg_review_slice.py \
      --data-dir ../bedrock_tcg_review_slice_import_1 --dry-run

The package is intentionally review-only. Use --cleanup to remove only rows
created with source=bedrock_tcg_review_slice_import_1 and
region=BEDROCK-TCG-REVIEW before re-importing.
"""
...
async def cleanup(session) -> dict[str, int]:
    assembly_ids = [
        row[0]
...
async def async_main(args: argparse.Namespace) -> None:
    ...
    if args.validate_only:
        print(json.dumps({"valid": True, "catalog_resources": len(catalog), "cost_items": len(costs), "assemblies": len(assemblies)}, indent=2))
        return
    async with async_session_factory() as session:
        before = await count_existing(session)
        plan = {"before": before, "package_counts": {"catalog_resources": len(catalog), "cost_items": len(costs), "assemblies": len(assemblies)}, "cleanup": bool(args.cleanup)}
        if args.dry_run:
        ...
        try:
            ...
            result["after"] = await count_existing(session)
...
def main() -> None:
    ...
    parser.add_argument("--data-dir", type=Path, default=Path("../bedrock_tcg_review_slice_import_1"))
