#!/usr/bin/env python3
"""Import a review-only TCG JSON package into OCERP.

Run from OCERP/backend, for example:

    python ../scripts/import_tcg_package.py \
      --data-dir ../bedrock_tcg_model_job_extract_v4_review --dry-run
...
def validate_files(data_dir: Path, source: str, region: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
...
async def count_existing(session, source: str, region: str) -> dict[str, int]:
    catalog = await session.scalar(select(func.count()).select_from(CatalogResource).where(CatalogResource.region == region, CatalogResource.source == source))
    costs = await session.scalar(select(func.count()).select_from(CostItem).where(CostItem.region == region, CostItem.source == source))
    assemblies = await session.scalar(select(func.count()).select_from(Assembly).where(Assembly.metadata_["source"].as_string() == source))
    components = await session.scalar(
        select(func.count()).select_from(Component).join(Assembly, Component.assembly_id == Assembly.id).where(Assembly.metadata_["source"].as_string() == source)
    )
    return {"catalog_resources": int(catalog or 0), "cost_items": int(costs or 0), "assemblies": int(assemblies or 0), "assembly_components": int(components or 0)}


async def cleanup(session, source: str, region: str) -> dict[str, int]:
    assembly_ids = [row[0] for row in (await session.execute(select(Assembly.id).where(Assembly.metadata_["source"].as_string() == source))).all()]
    ...
    return {"components": deleted_components, "assemblies": deleted_assemblies, "cost_items": deleted_costs, "catalog_resources": deleted_catalog}
...
async def load_lookup_maps(session, source: str, region: str) -> tuple[dict[str, Any], dict[str, Any]]:
    cost_rows = (await session.execute(select(CostItem.code, CostItem.id).where(CostItem.region == region, CostItem.source == source))).all()
    resource_rows = (await session.execute(select(CatalogResource.resource_code, CatalogResource.id).where(CatalogResource.region == region, CatalogResource.source == source))).all()
...
async def async_main(args: argparse.Namespace) -> None:
    ...
    if args.validate_only:
        print(json.dumps({"valid": True, "source": source, "region": region, "catalog_resources": len(catalog), "cost_items": len(costs), "assemblies": len(assemblies)}, indent=2))
        return
    async with async_session_factory() as session:
        before = await count_existing(session, source, region)
        plan = {"source": source, "region": region, "before": before, "package_counts": {"catalog_resources": len(catalog), "cost_items": len(costs), "assemblies": len(assemblies)}, "cleanup": bool(args.cleanup)}
        if args.dry_run:
        ...
        try:
            ...
            # Force backend cache invalidation for the new region to show up in the UI
