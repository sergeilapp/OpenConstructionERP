    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t("ru_gesn.preview", { defaultValue: "Preview‌⁠‍" })}:{" "}
          {positions.length}{" "}
...
export default function RUExchangeModule() {
  ...
  const [exportFormat, setExportFormat] =
    useState<RUExportFormat>("gesn-detailed");
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        {
          method: "POST",
          body: JSON.stringify({ positions, source: "ru_gesn_import" }),
        } as never,
      );
      ...
      addToast({
        ...
        title: t("ru_gesn.import_complete", {
          defaultValue: "Russian GESN import complete",
        }),
      ...
      addToast({
        type: "error",
        title: t("ru_gesn.import_failed", {
          defaultValue: "GESN import failed",
        }),
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      addToast({
        type: "success",
        title: t("ru_gesn.export_complete", {
          defaultValue: "Russian GESN export complete",
        }),
      ...
      addToast({
        type: "error",
        title: t("ru_gesn.export_failed", {
          defaultValue: "GESN export failed",
        }),
        message: err instanceof Error ? err.message : "Unknown error",
      });
    } finally {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t("ru_gesn.title", {
              defaultValue: "Russia GESN / FER / TER Import / Export",
            })}
          </h1>
              ...
              <div className="space-y-3">
                ...
                {parsedPositions && (
                  <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-600">
                    ...
                    {t("ru_gesn.positions_found", {
                      defaultValue: "positions found",
                    })}
                    {parsedPositions.some((p) => p.unitRate > 0) && (
                      <Badge variant="blue" className="ml-2">
                        {t("ru_gesn.gesn_detailed", {
                          defaultValue: "GESN Detailed",
                        })}
                      </Badge>
                    )}
                    {parsedPositions.every((p) => p.unitRate === 0) && (
                      <Badge variant="neutral" className="ml-2">
                        {t("ru_gesn.gesn_summary", {
                          defaultValue: "GESN Summary",
                        })}
                      </Badge>
                    )}
                  </div>
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
              <div className="flex items-center gap-1.5 text-xs font-medium text-content-secondary mb-2">
                <Info size={13} />
                {t("ru_gesn.trades_ref", {
                  defaultValue: "GESN Collections Reference",
                })}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  value={exportFormat}
                  onChange={(e) =>
                    setExportFormat(e.target.value as RUExportFormat)
                  }
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                >
                  <Eye size={13} />
                  {showExportPreview
                    ? t("ru_gesn.hide_preview", {
                        defaultValue: "Hide preview",
                      })
                    : t("ru_gesn.show_preview", {
