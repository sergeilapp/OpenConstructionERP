...
describe("ConflictResolutionPanel — rendering", () => {
  ...
  it('renders "Your version" and remote-user version labels', () => {
    ...
    const theirCard = screen
      .getByTestId("value-remote")
      .closest('[class*="rounded"]')!;
    expect(
      within(theirCard as HTMLElement).getByText("· Carol"),
    ).toBeInTheDocument();
  });
...
describe("ConflictResolutionPanel — multi-conflict navigation", () => {
  const conflicts: ConflictItem[] = [
    makeConflict({
      id: "c1",
      positionOrdinal: "01.01",
      field: "description",
      localValue: "Concrete wall",
      remoteValue: "RC Wall",
    }),
    makeConflict({
      id: "c2",
      positionOrdinal: "01.02",
      field: "unit_rate",
      localValue: "200",
      remoteValue: "220",
    }),
...
describe("ConflictResolutionPanel — edge cases", () => {
  ...
  it("renders long values without breaking layout", () => {
    ...
    renderPanel([
      makeConflict({ localValue: longValue, remoteValue: longValue }),
    ]);
  ...
  it("handles JSON object values gracefully", () => {
    ...
    expect(screen.getByTestId("value-local")).toHaveTextContent(
      '{"quantity":10,"unit":"m2"}',
    );
    expect(screen.getByTestId("value-remote")).toHaveTextContent(
      '{"quantity":12,"unit":"m2"}',
    );
  });
