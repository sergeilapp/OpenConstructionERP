...
export default function CNExchangeModule() {
  ...
  const [exportFormat, setExportFormat] =
    useState<CNExportFormat>("gbt-detailed");
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        {
          method: "POST",
          body: JSON.stringify({ positions, source: "cn_gbt50500_import" }),
        } as never,
      );
      ...
      addToast({
        ...
        title: t("cn.import_complete", {
          defaultValue: "Chinese BOQ import complete",
        }),
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      addToast({
        type: "success",
        title: t("cn.export_complete", {
          defaultValue: "Chinese BOQ export complete",
        }),
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-content-tertiary">
            {t("cn.subtitle", {
              defaultValue:
                "Exchange Bills of Quantities in GB/T 50500 format (Excel / CSV)",
            })}
          </p>
        </div>
      </div>
            ...
            {importFile ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t("cn.drop_file", {
                    defaultValue:
                      "Drop a Chinese BOQ file here (Excel or CSV), or",
                  })}
                </p>
                <Button
                  variant="secondary"
                ...
                <p className="text-2xs text-content-quaternary">
                  {t("cn.formats_hint", {
                    defaultValue:
                      "Supported: .csv, .tsv, .xlsx (GB/T 50500-formatted BOQ)",
                  })}
                </p>
              </div>
            )}
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
              <div className="flex items-center gap-1.5 text-xs font-medium text-content-secondary mb-2">
                <Info size={13} />
                {t("cn.trades_ref", {
                  defaultValue: "GB/T 50500 Trade Sections Reference",
                })}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  value={exportFormat}
                  onChange={(e) =>
                    setExportFormat(e.target.value as CNExportFormat)
                  }
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
