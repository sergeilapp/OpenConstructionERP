...
const ANNOTATION_TOOLS: AnnotationToolType[] = [
  "cloud",
  "arrow",
  "text",
  "rectangle",
...
export default function TakeoffViewerModule({
  ...
  const [countLabel, setCountLabel] = useState(
    t("takeoff_viewer.default_count_label", { defaultValue: "Element‌⁠‍" }),
  );
  ...
  useEffect(() => {
    try {
      localStorage.setItem("takeoff.sidebarTab", sidebarTab);
    } catch {
      /* ignore */
    }
  }, [sidebarTab]);
  ...
  const annotationCounterRef = useRef<Record<string, number>>({
    distance: 0,
    polyline: 0,
    area: 0,
    volume: 0,
  ...
  const [linkingMeasurementId, setLinkingMeasurementId] = useState<
    string | null
  >(null);
  ...
  const { hasPersistedData, saveNow, clearPersisted, syncing, syncedToServer } =
  ...
  const handleFileUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        addToast({
          type: "error",
          title: t("takeoff_viewer.pdf_load_failed", {
            defaultValue: "Failed to load PDF‌⁠‍",
          }),
          message:
            err instanceof Error
              ? err.message
              : t("takeoff_viewer.pdf_load_error_hint", {
                  defaultValue:
  ...
  useEffect(() => {
    ...
    addToast({
      ...
      message: t("takeoff_viewer.calibration_warn_msg", {
        defaultValue:
          "Measurements may be inaccurate. Use the Calibrate tool to set a real-world length.",
      }),
    });
  ...
  useEffect(() => {
    ...
    for (const m of measurements.filter(
      (m) =>
        m.page === currentPage &&
    ...
    if (activePoints.length > 0) {
      ...
      if (activePoints.length >= 2 && activeTool === "cloud") {
        ...
        ctx.beginPath();
        ctx.moveTo(
          activePoints[0]!.x * dpr * zoom,
          activePoints[0]!.y * dpr * zoom,
        );
        for (let i = 1; i < activePoints.length; i++) {
          ctx.lineTo(
            activePoints[i]!.x * dpr * zoom,
            activePoints[i]!.y * dpr * zoom,
          );
        }
        ctx.stroke();
        ctx.setLineDash([]);
  ...
  const nextAnnotation = useCallback(
    (type: string) => {
      annotationCounterRef.current[type] =
        (annotationCounterRef.current[type] || 0) + 1;
      const n = annotationCounterRef.current[type];
      if (type === "distance")
        return t("takeoff.distance_n", { defaultValue: "Distance {{n}}", n });
      if (type === "polyline")
        return t("takeoff.polyline_n", { defaultValue: "Polyline {{n}}", n });
      if (type === "area")
        return t("takeoff.area_n", { defaultValue: "Area {{n}}", n });
      if (type === "volume")
        return t("takeoff.volume_n", { defaultValue: "Volume {{n}}", n });
      if (type === "count")
        return t("takeoff.count_n", { defaultValue: "Count {{n}}", n });
      if (type === "cloud")
        return t("takeoff.cloud_n", { defaultValue: "Cloud {{n}}", n });
      if (type === "arrow")
        return t("takeoff.arrow_n", { defaultValue: "Arrow {{n}}", n });
      if (type === "text")
        return t("takeoff.text_n", { defaultValue: "Text {{n}}", n });
      if (type === "rectangle")
        return t("takeoff.rectangle_n", { defaultValue: "Rectangle {{n}}", n });
      if (type === "highlight")
        return t("takeoff.highlight_n", { defaultValue: "Highlight {{n}}", n });
      return `${type} ${n}`;
    },
    [t],
  );
      ...
      setMeasurements((prev) =>
        prev.map((m) => {
          ...
          pushUndo({
            kind: "change_annotation",
            measurementId: id,
            previousAnnotation: m.annotation,
          });
    ...
    (e: React.TouchEvent<HTMLCanvasElement>) => {
      if (e.touches.length === 2 && touchStateRef.current) {
        ...
        const newZoom = Math.max(
          0.25,
          Math.min(4.0, touchStateRef.current.initialZoom * scaleFactor),
        );
  ...
  const handleCanvasClickRef = useRef<
    ((e: React.MouseEvent<HTMLCanvasElement>) => void) | null
  >(null);
      ...
      if (activeTool === "distance") {
        ...
        if (newPoints.length === 2) {
          ...
          pushUndo({
            ...
            previousActivePoints: [...activePoints],
          });
      ...
      if (activeTool === "rectangle" || activeTool === "highlight") {
        if (!rectStartPoint) {
          ...
          pushUndo({
            kind: "complete_measurement",
            measurement: newMeasurement,
            previousActivePoints: [rectStartPoint],
          });
          setMeasurements((prev) => [...prev, newMeasurement]);
          setRectStartPoint(null);
          setIsDraggingRect(false);
  ...
  const handleScaleConfirm = useCallback(() => {
    if (scaleRefPixels <= 0 || scaleRefReal <= 0) {
      addToast({
        type: "warning",
        title: t("takeoff_viewer.scale_invalid", {
          defaultValue: "Invalid scale value",
        }),
        message: t("takeoff_viewer.scale_must_be_positive", {
          defaultValue: "Reference distance must be greater than zero.",
        }),
      });
      return;
    }
  ...
  const handleCalibrationConfirm = useCallback(
    (
      ...
      entry?: { realLength: number; unit: CalibrationUnit },
    ) => {
      setScale(nextScale);
      ...
      const meters =
        calibrationPixels > 0 ? calibrationPixels / nextScale.pixelsPerUnit : 0;
      ...
      addToast({
        type: "success",
        title: t("takeoff_viewer.calibrated", {
        ...
        requestAnimationFrame(() => {
          if (containerRef.current) {
            containerRef.current.scrollLeft =
              (container.scrollLeft + cx) * ratio - cx;
            containerRef.current.scrollTop =
              (container.scrollTop + cy) * ratio - cy;
          }
        });
        return next;
      });
  ...
  const nextPage = useCallback(
    () =>
      setCurrentPage((p) => (totalPages > 0 ? Math.min(p + 1, totalPages) : p)),
    [totalPages],
  );
  ...
  const updateSelectedMeasurement = useCallback(
    (patch: Partial<Measurement>) => {
      ...
      setMeasurements((prev) =>
        prev.map((m) =>
          m.id === selectedMeasurementId ? { ...m, ...patch } : m,
        ),
      );
    },
  ...
  const handleExportCSV = useCallback(() => {
    ...
    for (const [groupName, groupMs] of Object.entries(byGroup)) {
      ...
      const distMs = groupMs.filter(
        (m) => m.type === "distance" || m.type === "polyline",
      );
      ...
      if (distMs.length > 0) {
        rows.push(
          `"${groupName} - Subtotal","distance","Total distance",${distMs.reduce((s, m) => s + m.value, 0).toFixed(3)},"${distMs[0]!.unit}",""`,
        );
      }
      if (areaMs.length > 0) {
        rows.push(
          `"${groupName} - Subtotal","area","Total area",${areaMs.reduce((s, m) => s + m.value, 0).toFixed(3)},"${areaMs[0]!.unit}",""`,
        );
      }
      if (volMs.length > 0) {
        rows.push(
          `"${groupName} - Subtotal","volume","Total volume",${volMs.reduce((s, m) => s + m.value, 0).toFixed(3)},"${volMs[0]!.unit}",""`,
        );
      }
      if (countMs.length > 0) {
        rows.push(
          `"${groupName} - Subtotal","count","Total count",${countMs.reduce((s, m) => s + m.value, 0).toFixed(0)},"pcs",""`,
        );
      }
    }
    ...
    addToast({
      type: "success",
      title: t("takeoff.csv_exported", {
        defaultValue: "Measurements exported to CSV",
      }),
    });
  ...
  const handleExportPdf = useCallback(async () => {
    if (!pdfDoc) {
      addToast({
        type: "warning",
        title: t("takeoff_viewer.pdf_export_no_doc", {
          defaultValue: "Load a PDF first",
        }),
      });
      return;
    }
    if (measurements.length === 0) {
      addToast({
        type: "warning",
        title: t("takeoff_viewer.pdf_export_empty", {
      ...
      setMeasurements((prev) => {
        ...
        if (target) {
          pushUndo({
            ...
            measurement: { ...target, points: [...target.points] },
          });
        }
  ...
  const openExportDialog = useCallback(async () => {
    ...
    try {
      const projects =
        await apiGet<{ id: string; name: string }[]>("/v1/projects/");
      setExportProjects(projects);
    } catch (err) {
      setExportProjects([]);
      addToast({
        type: "error",
        title: t("takeoff.load_projects_failed", {
  ...
  const handleProjectChange = useCallback(async (projectId: string) => {
    ...
    try {
      ...
      addToast({
        type: "error",
        title: t("takeoff.load_boqs_failed", {
          defaultValue: "Failed to load BOQ list",
        }),
        message: err instanceof Error ? err.message : "",
      });
    }
  ...
  const handleExportToBOQ = useCallback(async () => {
    ...
    try {
      ...
      const exportableMeasurements = measurements.filter(
        (m) => !isAnnotationType(m.type),
      );
      ...
      addToast({
        type: "success",
        title: t("takeoff.added_to_boq_success", {
          defaultValue: "Measurements exported to BOQ",
        }),
      });
      ...
      addToast({
        type: "error",
        title: t("takeoff.export_failed", {
          defaultValue: "Export to BOQ failed",
        }),
        message:
          err instanceof Error
            ? err.message
            : t("takeoff.export_error_hint", {
  ...
  const loadPickerPositions = useCallback(async (boqId: string) => {
    ...
    try {
      const boqData = await apiGet<{ positions: Position[] }>(
        `/v1/boq/boqs/${boqId}`,
      );
  ...
  const handleOpenLinkToBoq = useCallback(
    async (measurementId: string) => {
      ...
      const seedBoq =
        (selectedProjectId ? selectedBoqId : "") ||
        (activeProjectId ? (activeBoqIdFromStore ?? "") : "") ||
        "";
  ...
  const handleLinkToPosition = useCallback(
    async (measurementId: string, position: Position) => {
      ...
      try {
        ...
        if (measurement.serverId) {
          try {
            await takeoffApi.linkToBoq(measurement.serverId, position.id);
          } catch {
            /* non-critical */
          }
        }
        ...
        addToast({
          ...
          message:
            `${newQty} ${canonicalUnit} → ${position.ordinal} ${position.description?.slice(0, 40) || ""}`.trim(),
        });
    ...
    async (measurementId: string) => {
      ...
      try {
        ...
        const takeoffOrdinals = linkBoqPositions
          ...
          .filter((n) => n > 0);
        const nextNum =
          (takeoffOrdinals.length ? Math.max(...takeoffOrdinals) : 0) + 1;
        ...
        if (measurement.serverId) {
          try {
            await takeoffApi.linkToBoq(measurement.serverId, newPos.id);
          } catch {
            /* non-critical */
          }
        }
        ...
        addToast({
          type: "success",
          title: t("takeoff.linked_created", {
        ...
        addToast({
          type: "error",
          title: t("takeoff.create_link_failed", {
   ...
   */
  const handleUnlinkMeasurement = useCallback(
    async (measurementId: string) => {
      const measurement = measurements.find((m) => m.id === measurementId);
      if (!measurement) return;
      setLinkingInProgress(true);
  ...
  const handleOpenLinkedPosition = useCallback((m: Measurement) => {
    ...
    window.open(
      `/boq/${m.linkedBoqId}?highlight=${m.linkedPositionId}`,
      "_blank",
      "noopener",
    );
  }, []);
  ...
  const handleUndo = useCallback(() => {
    ...
    switch (op.kind) {
      ...
      case "complete_measurement":
        ...
        setMeasurements((prev) =>
          prev.filter((m) => m.id !== op.measurement.id),
        );
        setActivePoints(op.previousActivePoints);
        // Clear selection if we undid its creation
        setSelectedMeasurementId((sel) =>
          sel === op.measurement.id ? null : sel,
        );
        break;
      ...
      case "add_count_point":
        if (op.wasNew) {
          ...
          setMeasurements((prev) =>
            prev.filter((m) => m.id !== op.measurementId),
          );
        } else {
    ...
    addToast({
      type: "info",
      title: t("takeoff.undo", { defaultValue: "Undo" }),
      message: t("takeoff.measurement_undone", {
        defaultValue: "Measurement undone",
      }),
    });
  }, [addToast, t]);
  ...
  const handleRedo = useCallback(() => {
    ...
    addToast({
      ...
      message: t("takeoff.measurement_redone", {
        defaultValue: "Measurement redone",
      }),
    });
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (
        ...
        setMeasurements((prev) => {
          ...
          if (target)
            pushUndo({ kind: "delete_measurement", measurement: target });
  ...
  const landingFeatures = [
    {
      ...
      ic: "text-blue-500",
      title: t("takeoff.landing_feat_click_title", {
        defaultValue: "Click-to-measure",
      }),
      desc: t("takeoff.landing_feat_click_desc", {
        defaultValue:
          "Distance, area, polyline and count tools — click directly on the PDF to capture quantities.",
      }),
    },
    {
      icon: Scan,
      ...
      ic: "text-emerald-500",
      title: t("takeoff.landing_feat_symbol_title", {
        defaultValue: "Auto symbol detection",
      }),
      desc: t("takeoff.landing_feat_symbol_desc", {
        defaultValue:
          "AI spots doors, windows and MEP symbols and proposes counts with confidence scores.",
      }),
    },
    {
      icon: Ruler,
      color:
        "bg-violet-50 dark:bg-violet-950/20 border-violet-100 dark:border-violet-800",
      ic: "text-violet-500",
      title: t("takeoff.landing_feat_scale_title", {
        defaultValue: "Scale calibration",
      }),
      desc: t("takeoff.landing_feat_scale_desc", {
        defaultValue:
          "Two-point calibration or preset scales (1:50, 1:100) — every measurement stays accurate.",
      }),
    },
    {
      icon: Layers,
      color:
        "bg-orange-50 dark:bg-orange-950/20 border-orange-100 dark:border-orange-800",
      ic: "text-orange-500",
      title: t("takeoff.landing_feat_units_title", {
        defaultValue: "Area · length · count",
      }),
      desc: t("takeoff.landing_feat_units_desc", {
        defaultValue:
          "Switch freely between m, m², m³, pcs — grouped by trade with hide/show per layer.",
      }),
    },
    {
      icon: Sparkles,
      color:
        "bg-pink-50 dark:bg-pink-950/20 border-pink-100 dark:border-pink-800",
      ic: "text-pink-500",
      title: t("takeoff.landing_feat_ai_title", {
        defaultValue: "AI-assisted quantities",
      }),
      desc: t("takeoff.landing_feat_ai_desc", {
        defaultValue:
          "Let the AI extract walls, slabs and rooms from a drawing — you review and confirm before committing.",
      }),
    },
    {
      icon: Link2,
      ...
      ic: "text-cyan-500",
      title: t("takeoff.landing_feat_export_title", {
        defaultValue: "Export to BOQ",
      }),
      desc: t("takeoff.landing_feat_export_desc", {
        defaultValue:
          "Send measurements straight into your Bill of Quantities — with links back to the source drawing.",
      }),
    },
  ];
  ...
  const landingFormats = [
    {
      ext: "PDF",
      label: t("takeoff.landing_fmt_pdf", {
        defaultValue: "Vector drawings, floor plans, sections",
      }),
      icon: FileText,
      size: "up to 50MB",
      primary: true,
    ...
    {
      ext: "PNG",
      label: t("takeoff.landing_fmt_png", {
        defaultValue: "Raster plan images",
      }),
      icon: ImageIcon,
      size: "up to 50MB",
    },
    {
      ext: "JPG",
      label: t("takeoff.landing_fmt_jpg", {
        defaultValue: "Scanned or photographed plans",
      }),
      icon: ImageIcon,
      size: "up to 50MB",
    },
    {
      ext: "TIFF",
      label: t("takeoff.landing_fmt_tiff", {
        defaultValue: "High-resolution scans",
      }),
      icon: ImageIcon,
      size: "up to 50MB",
    },
    {
      ext: "DWG",
      label: t("takeoff.landing_fmt_dwg", {
        defaultValue: "Use DWG Takeoff module instead",
      }),
      icon: Box,
      size: "via DWG module",
      muted: true,
    ...
    <div className="relative space-y-4">
          ...
          <div className="max-w-7xl mx-auto px-6 pt-8 pb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch mb-5">
              ...
              <div className="flex flex-col">
                <div className="rounded-2xl bg-white dark:bg-gray-800/60 border border-border-light shadow-lg shadow-black/5 dark:shadow-black/20 p-4 flex flex-col h-full">
                  <label
                    aria-label={t("takeoff.landing_dropzone_aria", {
                      defaultValue: "Drop a PDF here or click to browse.",
                    })}
                    onDragOver={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                    ...
                    onDrop={(e) => {
                      ...
                      if (pdf) {
                        ...
                        addToast({
                          type: "warning",
                          title: t("takeoff.landing_drop_pdf_only_title", {
                            defaultValue: "PDF only",
                          }),
                          message: t("takeoff.landing_drop_pdf_only_msg", {
                            defaultValue:
                              "Image support is coming soon — drop a PDF for now.",
                          }),
                        });
                      }
                    }}
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-semibold text-content-primary">
                        {t("takeoff.landing_drop_here", {
                          defaultValue: "Drop a PDF here or click to browse",
                        })}
                      </p>
                      <p className="text-xs text-content-tertiary mt-1">
                        {t("takeoff.landing_size_hint", {
                          defaultValue: "PDF — up to 50MB",
                        })}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap justify-center">
                      <span className="text-[10px] font-mono px-2 py-1 rounded-md bg-oe-blue/8 text-oe-blue border border-oe-blue/15 font-semibold">
                        .pdf
                      </span>
                    </div>
                    <p className="text-[10px] text-content-quaternary leading-relaxed mt-1 text-center">
                      {t("takeoff.landing_dropzone_hint", {
                        defaultValue:
                          "Architectural drawings \u00B7 floor plans \u00B7 sections \u00B7 scans",
                      })}
                    </p>
                    <input
                      type="file"
              ...
              <div className="flex flex-col justify-center gap-4">
                <div>
                  <h1 className="text-2xl font-bold text-content-primary tracking-tight leading-tight">
                    {t("takeoff.landing_hero_title", {
                      defaultValue: "PDF Takeoff",
                    })}
                  </h1>
                  <p className="text-base text-content-secondary mt-3 leading-relaxed">
                    {t("takeoff.landing_hero_subtitle", {
                      defaultValue:
                        "Click-to-measure on any drawing \u2014 lengths, areas, counts \u2014 with AI that suggests quantities and sends them straight into your BOQ.",
                    })}
                  </p>
                </div>
                ...
                <div>
                  <h2 className="text-[10px] font-bold text-content-tertiary uppercase tracking-widest mb-2">
                    {t("takeoff.landing_supported_formats", {
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1">
                            <span
                              className={`text-[11px] font-mono font-bold ${f.primary ? "text-oe-blue" : "text-content-primary"}`}
                            >
                              .{f.ext.toLowerCase()}
                            </span>
                            {f.primary && (
                              <span className="text-[8px] font-semibold uppercase tracking-wider text-oe-blue">
                                {t("takeoff.landing_fmt_primary", {
                                  defaultValue: "Primary",
                                })}
                              </span>
                            )}
                          </div>
                          <p className="text-[10px] text-content-tertiary leading-snug mt-0.5 truncate">
                            {f.label}
                          </p>
                        </div>
                      </div>
            ...
            <div>
              <h2 className="text-xs font-bold text-content-tertiary uppercase tracking-widest mb-2">
                {t("takeoff.landing_what_you_get", {
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                {landingFeatures.map((f, i) => (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-lg ${f.color} border flex items-center justify-center shrink-0`}
                    >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-content-tertiary leading-snug mt-1">
                        {f.desc}
                      </p>
                    </div>
                  </div>
          ...
          <div className="flex-1 min-w-0 space-y-2">
            ...
            <div className="flex items-center gap-1 rounded-lg border border-border bg-surface-primary p-1.5 overflow-x-auto">
              ...
              <button
                ...
                className="p-1.5 rounded hover:bg-surface-secondary disabled:opacity-30 transition-colors"
                aria-label={t("takeoff_viewer.prev_page", {
              ...
              <details className="relative shrink-0" data-testid="page-jump">
                <summary
                  className="text-xs text-content-secondary tabular-nums px-1 cursor-pointer hover:text-content-primary list-none select-none whitespace-nowrap"
                  title={t("takeoff_viewer.jump_to_page", {
                    defaultValue: "Click to jump to a page",
                  })}
                >
                  {currentPage}/{totalPages}
                </summary>
                      ...
                      (p) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {cnt > 0 && (
                              <span
                                className={`tabular-nums rounded-full px-1.5 py-0.5 text-[10px] ${p === currentPage ? "bg-white/20" : "bg-purple-500/15 text-purple-500"}`}
                              >
                                {cnt}
                              </span>
                            )}
              ...
              <button
                ...
                className="p-1.5 rounded hover:bg-surface-secondary disabled:opacity-30 transition-colors"
                aria-label={t("takeoff_viewer.next_page", {
              ...
              <button
                ...
                title={t("takeoff_viewer.zoom_out", {
                  defaultValue: "Zoom out",
                })}
                aria-label={t("takeoff_viewer.zoom_out", {
                  defaultValue: "Zoom out",
                })}
              >
                <ZoomOut size={16} />
              </button>
              <span className="text-xs text-content-tertiary tabular-nums w-10 text-center">
                {(zoom * 100).toFixed(0)}%
              </span>
              <button
                onClick={zoomIn}
                className="p-1.5 rounded hover:bg-surface-secondary transition-colors"
                title={t("takeoff_viewer.zoom_in", { defaultValue: "Zoom in" })}
                aria-label={t("takeoff_viewer.zoom_in", {
              ...
              <button
                onClick={zoomFit}
                className="p-1.5 rounded hover:bg-surface-secondary transition-colors"
                title={t("takeoff_viewer.zoom_fit", { defaultValue: "Fit" })}
                aria-label={t("takeoff_viewer.zoom_fit", {
                  defaultValue: "Fit",
                })}
              >
                <Maximize size={16} />
              ...
              {(
                [
                  {
                    tool: "select" as MeasureTool,
                    icon: MousePointer2,
                    label: t("takeoff_viewer.tool_select", {
                      defaultValue: "Select",
                    }),
                  },
                  {
                    tool: "distance" as MeasureTool,
                    icon: Minus,
                    label: t("takeoff_viewer.tool_distance", {
                      defaultValue: "Distance",
                    }),
                  },
                  {
                    tool: "polyline" as MeasureTool,
                    icon: Route,
                    label: t("takeoff_viewer.tool_polyline", {
                      defaultValue: "Polyline",
                    }),
                  },
                  {
                    tool: "area" as MeasureTool,
                    icon: Pentagon,
                    label: t("takeoff_viewer.tool_area", {
                      defaultValue: "Area",
                    }),
                  },
                  {
                    tool: "volume" as MeasureTool,
                    icon: Box,
                    label: t("takeoff_viewer.tool_volume", {
                      defaultValue: "Volume",
                    }),
                  },
                  {
                    tool: "count" as MeasureTool,
                    icon: Hash,
                    label: t("takeoff_viewer.tool_count", {
                      defaultValue: "Count",
                    }),
                  },
                ] as const
              ...
              {(
                [
                  {
                    tool: "cloud" as MeasureTool,
                    icon: Cloud,
                    label: t("takeoff_viewer.tool_cloud", {
                      defaultValue: "Cloud",
                    }),
                  },
                  {
                    tool: "arrow" as MeasureTool,
                    icon: ArrowUpRight,
                    label: t("takeoff_viewer.tool_arrow", {
                      defaultValue: "Arrow",
                    }),
                  },
                  {
                    tool: "text" as MeasureTool,
                    icon: Type,
                    label: t("takeoff_viewer.tool_text", {
                      defaultValue: "Text",
                    }),
                  },
                  {
                    tool: "rectangle" as MeasureTool,
                    icon: Square,
                    label: t("takeoff_viewer.tool_rectangle", {
                      defaultValue: "Rectangle",
                    }),
                  },
                  {
                    tool: "highlight" as MeasureTool,
                    icon: Highlighter,
                    label: t("takeoff_viewer.tool_highlight", {
              ...
              <button
                ...
                className={`flex items-center gap-1 px-2 py-1.5 rounded text-xs transition-colors ${
                  settingScale && !calibrationMode
                    ? "bg-purple-500 text-white"
                    : "hover:bg-surface-secondary text-content-secondary"
                }`}
                title={t("takeoff_viewer.set_scale", {
                  defaultValue: "Set scale",
                })}
                aria-label={t("takeoff_viewer.set_scale", {
                  defaultValue: "Set scale",
                })}
              >
                <Settings2 size={14} />
              ...
              <button
                ...
                title={t("takeoff_viewer.calibrate", {
                  defaultValue: "Calibrate scale (two-click)",
                })}
                aria-label={t("takeoff_viewer.calibrate", {
              ...
              >
                <Ruler size={14} />
                <span className="hidden sm:inline">
                  {t("takeoff_viewer.calibrate", { defaultValue: "Calibrate" })}
                </span>
              </button>
              ...
              {isCalibrated && !calibrationMode && !settingScale && (
                <button
                  ...
                  title={t("takeoff_viewer.calibrated_tooltip", {
                    defaultValue:
                      "Calibrated · {{ratio}}{{atLen}} — click to recalibrate",
              ...
              {!isCalibrated && !calibrationMode && !settingScale && (
                <button
                  ...
                  title={t("takeoff_viewer.uncalibrated_hint", {
                    defaultValue:
                      "Drawing is not calibrated — measurements may be inaccurate. Click to calibrate.",
                  })}
                ...
                >
                  ...
                  />
                  <span>
                    {t("takeoff_viewer.calibrate_short", {
              ...
              <button
                ...
                title={t("takeoff_viewer.toggle_legend", {
                  defaultValue: "Toggle legend",
                })}
                aria-label={t("takeoff_viewer.toggle_legend", {
                  defaultValue: "Toggle legend",
                })}
              ...
              <button
                ...
                title={
                  t("takeoff.undo", { defaultValue: "Undo" }) + " (Ctrl+Z)"
                }
                data-testid="undo-button"
              >
                <Undo2 size={14} />
                <span className="hidden sm:inline">
                  {t("takeoff.undo", { defaultValue: "Undo" })}
                </span>
              </button>
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1.5 rounded text-xs transition-colors hover:bg-surface-secondary text-content-secondary disabled:opacity-30 disabled:pointer-events-none"
                title={
                  t("takeoff.redo", { defaultValue: "Redo" }) + " (Ctrl+Y)"
                }
                data-testid="redo-button"
              >
                <Redo2 size={14} />
                <span className="hidden sm:inline">
                  {t("takeoff.redo", { defaultValue: "Redo" })}
                </span>
              </button>
              ...
              <button
                ...
                className="p-1.5 rounded hover:bg-surface-secondary text-content-tertiary transition-colors"
                title={t("takeoff_viewer.clear_all", {
                  defaultValue: "Clear all",
                })}
                aria-label={t("takeoff_viewer.clear_all", {
                  defaultValue: "Clear all",
                })}
              >
                <Trash2 size={14} />
              ...
              {/* New file */}
              <label
                className="p-1.5 rounded hover:bg-surface-secondary text-content-tertiary transition-colors cursor-pointer"
                title={t("takeoff_viewer.load_new_pdf", {
                  defaultValue: "Load new PDF",
                })}
                aria-label={t("takeoff_viewer.load_new_pdf", {
                  defaultValue: "Load new PDF",
                })}
              >
                <Upload size={14} />
            ...
            >
                ...
                >
                  {calibrationMode
                    ? scalePoints.length === 0
                      ? t("takeoff_viewer.calibrate_click_first", {
                          defaultValue:
                            "Calibrate: click point A on a known dimension",
                        })
                      : t("takeoff_viewer.calibrate_click_second", {
                          defaultValue: "Calibrate: click point B",
                        })
                    : scalePoints.length === 0
                      ? t("takeoff_viewer.scale_click_first", {
                          defaultValue: "Click first point of known dimension",
                        })
                      : t("takeoff_viewer.scale_click_second", {
                          defaultValue: "Click second point",
                        })}
                </div>
              )}
              ...
              {!settingScale && activeTool !== "select" && (
                ...
                >
                  <span>
                    {activeTool === "count" &&
                      t("takeoff_viewer.hint_count", {
                        defaultValue: "Click on each item to count.",
                      })}
                    {activeTool === "distance" &&
                      t("takeoff_viewer.hint_distance", {
                        defaultValue: "Click two points for a distance.",
                      })}
                    {activeTool === "polyline" &&
                      t("takeoff_viewer.hint_polyline", {
                        defaultValue: "Click points along the line.",
                      })}
                    {activeTool === "area" &&
                      t("takeoff_viewer.hint_area", {
                        defaultValue: "Click polygon vertices.",
                      })}
                    {activeTool === "volume" &&
                      t("takeoff_viewer.hint_volume", {
                        defaultValue: "Click area outline.",
                      })}
                    {activeTool === "cloud" &&
                      t("takeoff_viewer.hint_cloud", {
                        defaultValue: "Click cloud outline points.",
                      })}
                    {activeTool === "arrow" &&
                      t("takeoff_viewer.hint_arrow", {
                        defaultValue: "Click arrow start, then end.",
                      })}
                    {activeTool === "rectangle" &&
                      t("takeoff_viewer.hint_rectangle", {
                        defaultValue: "Click two corners.",
                      })}
                    {activeTool === "highlight" &&
                      t("takeoff_viewer.hint_highlight", {
                        defaultValue: "Drag to highlight a region.",
                      })}
                    {activeTool === "text" &&
                      t("takeoff_viewer.hint_text", {
                        defaultValue: "Click to place a text pin.",
                      })}
                  </span>
                  {(activeTool === "count" ||
                    activeTool === "polyline" ||
                    ...
                    <span className="opacity-80 border-l border-white/30 pl-2">
                      {activeTool === "count"
                        ? t("takeoff_viewer.hint_esc_to_finish", {
                            defaultValue: "Esc: switch tool · Del: undo last",
                          })
                        : t("takeoff_viewer.hint_dblclick_close", {
                            defaultValue:
                              "Double-click: close shape · Esc: cancel",
                          })}
                    </span>
                  )}
                </div>
              ...
              {showTextInput && (
                ...
                >
                  <input
                    ...
                    autoFocus
                    placeholder={t("takeoff_viewer.text_placeholder", {
                      defaultValue: "Type annotation text...",
                    })}
              ...
              {activeTool === "cloud" && activePoints.length > 0 && (
                <div className="absolute top-2 left-2 bg-orange-500/90 text-white px-3 py-1.5 rounded-lg text-xs font-medium">
                  {t("takeoff_viewer.cloud_hint", {
                    defaultValue:
                      "Click points to define cloud shape. Double-click or right-click to finish.",
                  })}
                </div>
              )}
                ...
                >
                  <div className="flex items-center justify-between px-2.5 py-1.5 border-b border-border-light bg-surface-secondary/40">
                    ...
                    <button
                      ...
                      aria-label={t("takeoff_viewer.hide_legend", {
                        defaultValue: "Hide legend",
                      })}
                    >
                      <X size={10} />
                    </button>
                  ...
                  <div className="py-1">
                    ...
                    {(() => {
                      ...
                      for (const m of pageMeasurements)
                        allGroupsOnPage.add(m.group || "General");
                      const visible = new Map(
                        legendSummaries.map((s) => [s.name, s]),
                      );
                      const rows: Array<{
                        name: string;
                        color: string;
                      ...
                      return rows.map((row) => (
                        <button
                          ...
                          title={
                            row.hidden
                              ? t("takeoff_viewer.show_group", {
                                  defaultValue: "Show group",
                                })
                              : t("takeoff_viewer.hide_group", {
                                  defaultValue: "Hide group",
                                })
                          }
                        >
                          <span
          ...
          <div className="w-72 shrink-0 space-y-2">
            ...
            <div className="rounded-md border border-border/80 bg-surface-primary/80 backdrop-blur-sm p-3 shadow-sm">
              <label className="text-[10px] font-bold uppercase tracking-widest text-content-tertiary block mb-1">
                {t("takeoff_viewer.active_group", {
                  defaultValue: "Active Group",
                })}
              </label>
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{
                    backgroundColor: GROUP_COLOR_MAP[activeGroup] || "#3B82F6",
                  }}
                />
                <select
                  value={activeGroup}
            ...
            {isAnnotationTool(activeTool) && (
              <div className="rounded-md border border-border/80 bg-surface-primary/80 backdrop-blur-sm p-3 shadow-sm">
                <label className="text-[10px] font-bold uppercase tracking-widest text-content-tertiary block mb-1.5">
                  {t("takeoff_viewer.annotation_color", {
            ...
            >
              ...
              >
                {t("takeoff_viewer.tab_properties", {
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    aria-label={t("takeoff_viewer.close_properties", {
                      defaultValue: "Close properties",
                    })}
                  >
                    <X size={12} />
                  </button>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (val === "__new__") {
                        const name = prompt(
                          t("takeoff_viewer.new_group_prompt", {
                  ...
                  <div className="flex items-center gap-1.5">
                    {ANNOTATION_COLORS.map((c) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          updateSelectedMeasurement({ color: c.value })
                        }
                        className={clsx(
                  ...
                  <div>
                    ...
                    >
                      {selectedMeasurement.value
                        ? selectedMeasurement.value.toFixed(3)
                        : "—"}
                    </div>
                  </div>
                  <div>
                ...
                <div>
                  <label className="text-[10px] font-semibold text-content-tertiary block mb-0.5">
                    {t("takeoff_viewer.prop_annotation", {
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateSelectedMeasurement({ annotation: e.target.value })
                    }
                    placeholder={t(
                      "takeoff_viewer.prop_annotation_placeholder",
                      { defaultValue: "Label for this measurement" },
                    )}
                    className="w-full rounded border border-border bg-surface-primary px-2 py-1 text-xs text-content-primary"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      updateSelectedMeasurement({ notes: e.target.value })
                    }
                    placeholder={t("takeoff_viewer.prop_notes_placeholder", {
              ...
              <div className="rounded-md border border-border/80 bg-surface-primary/80 backdrop-blur-sm p-3 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-semibold text-content-primary">
                    {t("takeoff_viewer.measurements", {
                      defaultValue: "Measurements",
                    })}{" "}
                    <span className="tabular-nums text-content-tertiary font-normal">
                      {(() => {
                        const onPage = pageMeasurements.filter(
                          (m) => !isAnnotationType(m.type),
                        ).length;
                        const total = measurements.filter(
                          (m) => !isAnnotationType(m.type),
                        ).length;
                  ...
                  {fileName && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        title={t("takeoff_viewer.save_measurements", {
                          defaultValue: "Save measurements",
                        })}
                      >
                ...
                {pageMeasurements.length === 0 &&
                  (() => {
                    const totalOtherPages = measurements.filter(
                      (m) =>
                        !isAnnotationType(m.type) && m.page !== currentPage,
                    ).length;
                    return (
                ...
                <div className="space-y-2 max-h-[400px] overflow-auto">
                  ...
                  {Object.entries(groupedPageMeasurements).map(
                    ([groupName, groupMs]) => {
                      const measurementOnly = groupMs.filter(
                        (m) => !isAnnotationType(m.type),
                      );
                      if (measurementOnly.length === 0) return null;
                      const groupColor =
                        GROUP_COLOR_MAP[groupName] || "#3B82F6";
                                  ...
                                  <div className="flex items-center gap-2 leading-tight">
                                    ...
                                    <div className="flex-1 min-w-0 flex items-center gap-1.5">
                                      {editingAnnotationId === m.id ? (
                                        <input
                                          ...
                                          placeholder={t("takeoff.add_label", {
                                    ...
                                    <div className="flex items-center gap-0.5 shrink-0">
                                      ...
                                      <button
                                        ...
                                        aria-label={t(
                                          "takeoff_viewer.link_to_boq",
                                          { defaultValue: "Link to BOQ" },
                                        )}
                                        title={
                                          m.linkedPositionId
                                            ? t(
                                                "takeoff_viewer.relink_to_boq",
                                                {
                                                  defaultValue:
                                                    "Re-link or unlink BOQ position",
                                                },
                                              )
                                            : t(
                                                "takeoff_viewer.link_to_boq_hint",
                                                {
                                                  defaultValue:
                                                    "Push this measurement's quantity to a BOQ position",
                                                },
                                              )
                                        }
                                      >
                                      ...
                                      <button
                                        ...
                                        aria-label={t(
                                          "takeoff_viewer.delete_measurement",
                                          {
                                            defaultValue: "Delete measurement",
                                          },
                                        )}
                                    ...
                                    <div className="mt-1.5 rounded-lg border border-rose-200 dark:border-rose-800/40 bg-rose-50/50 dark:bg-rose-950/20 p-2 animate-fade-in">
                                      <div className="flex items-center justify-between mb-1.5">
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-rose-700 dark:text-rose-400">
                                          {m.linkedPositionId
                                            ? t("takeoff_viewer.relink_title", {
                                                defaultValue:
                                                  "Linked — pick new or unlink",
                                              })
                                            : t(
                                      ...
                                      <div className="mb-1.5 flex items-center gap-1.5 rounded bg-rose-100/70 dark:bg-rose-950/40 border border-rose-200/60 dark:border-rose-800/30 px-1.5 py-1 text-[10px]">
                                        ...
                                        <span className="text-content-tertiary shrink-0">
                                          {t("takeoff.will_transfer", {
                                            defaultValue: "Will transfer:",
                                          })}
                                        </span>
                                      ...
                                      {linkPickerMode === "pick" ? (
                                        !linkPickerBoqId ? (
                                          <p className="text-[10px] text-content-tertiary py-1">
                                            {t(
                                              "takeoff.link_need_project_boq",
                                              {
                                                defaultValue:
                                                  "Pick a project and BOQ above.",
                                              },
                                            )}
                                          </p>
                                        ...
                                        ) : linkBoqPositions.filter(
                                            (p) => p.unit,
                                          ).length === 0 ? (
                                          <p className="text-[10px] text-content-tertiary py-1">
                                            {t("takeoff.link_boq_empty", {
                                              defaultValue:
                                          ...
                                          <>
                                            <input
                                              ...
                                              placeholder={t(
                                                "takeoff.link_search_placeholder",
                                                {
                                                  defaultValue:
                                                  ...
                                                  return (
                                                    <button
                                                      ...
                                                      title={
                                                        unitMismatch
                                                          ? t(
                                                              "takeoff.unit_mismatch_warning",
                                                              {
                                                                defaultValue:
                                                                  "Unit mismatch: position is in {{posUnit}}, measurement is in {{measUnit}}. Linking will overwrite the position's unit.",
                                                                posUnit:
                                                                  pos.unit,
                                                                measUnit:
                                                                  measurementUnit,
                                                          ...
                                                          : t(
                                                              "takeoff.link_overwrites_qty",
                                                              {
                                                                defaultValue:
                                                                  "Link → overwrites current quantity ({{q}} {{u}}) with the measurement value",
                                                                q: currentQty,
                                                                u:
                                                                  pos.unit ??
                                                                  "",
                                                    ...
                                                    >
                                                      <span className="font-mono text-rose-600 dark:text-rose-400 shrink-0">
                                                        {pos.ordinal}
                                                      </span>
                                                      ...
                                                      {unitMismatch ? (
                                                        <span
                                                          className="inline-flex items-center gap-0.5 font-mono shrink-0 text-[9px] text-amber-600 dark:text-amber-400"
                                                          aria-label={t(
                                                            "takeoff.unit_mismatch",
                                                            {
                                                              defaultValue:
                                                                "unit mismatch",
                                                            },
                                                        ...
                                                        <span className="text-content-tertiary shrink-0 text-[9px]">
                                                          {pos.unit}
                                                        </span>
                                                      )}
                                                    </button>
                                        ...
                                        <div className="rounded bg-surface-primary/60 p-1.5 space-y-1">
                                          ...
                                          >
                                            ...
                                            {t("takeoff.create_and_link", {
                                              defaultValue:
                                                "Create position & link",
                                            })}
                                          </button>
                                        </div>
                                      )}
                  ...
                  {(() => {
                    const annotations = pageMeasurements.filter((m) =>
                      isAnnotationType(m.type),
                    );
                    if (annotations.length === 0) return null;
                    const annoCollapsed =
                      collapsedGroups.has("__annotations__");
                    ...
                    return (
                      <div>
                        <div className="flex items-center gap-1.5 mb-1 mt-2 pt-2 border-t border-border">
                          <button
                            onClick={() =>
                              toggleGroupCollapse("__annotations__")
                            }
                            className="p-0.5 rounded hover:bg-surface-secondary text-content-tertiary transition-colors"
                          >
                            {annoCollapsed ? (
                          ...
                          <button
                            onClick={() =>
                              toggleGroupVisibility("__annotations__")
                            }
                            className="p-0.5 rounded hover:bg-surface-secondary text-content-tertiary transition-colors"
                            title={
                              annoHidden
                                ? t("takeoff_viewer.show_annotations", {
                                    defaultValue: "Show annotations",
                                  })
                                : t("takeoff_viewer.hide_annotations", {
                                ...
                                >
                                  <div className="flex items-center gap-2 leading-tight">
                                    ...
                                    <input
                                      ...
                                      aria-label={t(
                                        "takeoff_viewer.change_annotation_color",
                                        {
                                          defaultValue:
                                    ...
                                    <button
                                      ...
                                      aria-label={t(
                                        "takeoff_viewer.delete_annotation",
            ...
            {measurements.length > 0 && (
              <div className="space-y-1.5">
                ...
                >
                  {t("takeoff_viewer.export_to_boq", {
                    defaultValue: "Export {{count}} measurements to BOQ",
                ...
                >
                  ...
                  {t("takeoff_viewer.export_excel_xlsx", {
                    defaultValue: "Export Excel (.xlsx)",
                  })}
                </button>
                <button
          ...
          <div className="w-80 rounded-xl border border-border bg-surface-elevated p-5 shadow-lg">
            ...
            <p className="text-xs text-content-tertiary mb-3">
              {t("takeoff_viewer.scale_desc", {
                defaultValue:
                  "You marked a line of {{pixels}} pixels. Enter the real-world length:",
      ...
      {showVolumeDepthInput && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-80 rounded-xl border border-border bg-surface-elevated p-5 shadow-lg">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t("takeoff_viewer.volume_depth_title", {
                defaultValue: "Enter Depth for Volume",
              })}
            </h3>
            <p className="text-xs text-content-tertiary mb-3">
              {t("takeoff_viewer.volume_depth_desc", {
                defaultValue:
                  "The polygon area has been captured. Enter the depth to calculate volume:",
              })}
            </p>
            <div className="flex items-center gap-2 mb-4">
              <input
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-oe-blue text-white text-xs font-medium hover:bg-oe-blue-hover transition-colors"
              >
                {t("takeoff_viewer.calculate_volume", {
                  defaultValue: "Calculate Volume",
                })}
              </button>
            </div>
          </div>
      ...
      {showExportDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-96 rounded-xl border border-border bg-surface-elevated p-5 shadow-lg">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t("takeoff_viewer.export_to_boq_title", {
                defaultValue: "Export Measurements to BOQ",
              })}
            </h3>
            <p className="text-xs text-content-tertiary mb-4">
              {t("takeoff_viewer.export_to_boq_desc", {
                defaultValue:
                  "{{count}} measurements will be added as new positions.",
                count: measurements.length,
              })}
            </p>
            ...
            <div className="space-y-3 mb-4">
              <div>
                ...
                >
                  <option value="">
                    {t("takeoff.select_project_placeholder", {
              ...
              <div>
                <label className="text-xs font-medium text-content-secondary block mb-1">
                  {t("takeoff.select_boq", {
                    defaultValue: "Bill of Quantities",
                  })}
                </label>
                <select
                  value={selectedBoqId}
                ...
                >
                  <option value="">
                    {t("takeoff.select_boq_placeholder", {
                      defaultValue: "Select BOQ...",
                    })}
                  </option>
                  {exportBoqs.map((b) => (
                    <option key={b.id} value={b.id}>
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {isExporting
                  ...
                  : t("takeoff_viewer.export_count", {
                      defaultValue: "Export {{count}} positions",
                      count: measurements.length,
                    })}
              </button>
            </div>
        ...
        >
          ...
          >
            <h3 className="text-sm font-semibold text-content-primary mb-2">
              {t("takeoff_viewer.clear_confirm_title", {
            ...
            <p className="text-xs text-content-secondary mb-4">
              {t("takeoff_viewer.clear_confirm_message", {
                defaultValue:
                  "All {{count}} measurement(s) and annotations will be permanently removed. This cannot be undone.",
                count: measurements.length,
              })}
            </p>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setShowClearConfirm(false)}
                className="px-3 py-1.5 rounded-lg text-xs font-medium text-content-secondary hover:bg-surface-secondary transition-colors"
              >
                {t("common.cancel", { defaultValue: "Cancel" })}
              </button>
              <button
