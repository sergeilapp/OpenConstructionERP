...
beforeAll(() => {
  ...
  HTMLCanvasElement.prototype.getContext = function getContext(
    ...
    if (contextId === "2d")
      return makeCtxStub() as unknown as CanvasRenderingContext2D;
    return null;
...
describe("buildExportFilename", () => {
  it("produces takeoff-{slug}-{YYYY-MM-DD}.{ext}", () => {
    ...
    expect(
      buildExportFilename("Berlin · Wohnpark Lichtenberg", "pdf", date),
    ).toBe("takeoff-berlin_wohnpark_lichtenberg-2026-05-20.pdf");
  ...
  it('falls back to "untitled" on empty project name', () => {
    ...
    expect(buildExportFilename("", "pdf", date)).toBe(
      "takeoff-untitled-2026-01-02.pdf",
    );
...
describe("summariseByGroupType", () => {
  it("sums numeric totals per (group, type) and ignores annotations", () => {
    ...
    const generalDistance = rows.find(
    ...
    const structuralArea = rows.find(
      (r) => r.group === "Structural" && r.type === "area",
    );
    expect(structuralArea).toBeDefined();
    expect(structuralArea!.count).toBe(1);
    expect(structuralArea!.total).toBeCloseTo(12.5, 5);

    const generalCloud = rows.find(
      (r) => r.group === "General" && r.type === "cloud",
    );
...
describe("selectAnnotatedPages + buildTakeoffPdf page count", () => {
  it("returns only pages with at least one visible measurement", () => {
    ...
    expect(selectAnnotatedPages(SAMPLE_MEASUREMENTS, new Set())).toEqual([
      1, 2, 3,
    ]);
  });
...
describe("buildTakeoffWorkbook", () => {
  it("emits the documented column layout on the Measurements sheet", async () => {
    ...
    const headerRow = ws!.getRow(1).values as unknown as Array<
      string | undefined
    >;
  ...
  it("writes a Summary sheet with grand total row matching count semantics", async () => {
    ...
    // Header row.
    const headers = (
      summary!.getRow(1).values as unknown as Array<string | undefined>
    )
      .slice(1)
      .map((v) => String(v ?? ""));
