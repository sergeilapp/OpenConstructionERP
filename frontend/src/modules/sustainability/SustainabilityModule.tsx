...
function getComplianceLevel(
  gwpPerM2: number,
): "excellent" | "good" | "acceptable" | "non-compliant" {
...
export default function SustainabilityModule() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<EPDCategory | "all">(
    "all",
  );
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t("sustainability.epd_title", {
              defaultValue: "EPD / Embodied Carbon‌⁠‍",
            })}
          </h1>
          <p className="text-sm text-content-tertiary">
            {t("sustainability.epd_subtitle", {
              defaultValue:
                "EU CPR 2024/3110 compliance — GWP calculation per position‌⁠‍",
            })}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1 rounded-xl border border-border bg-surface-primary p-4">
          <h2 className="text-sm font-semibold text-content-primary mb-3">
            {t("sustainability.material_database", {
              defaultValue: "EPD Material Database‌⁠‍",
            })}
          </h2>
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedCategory(e.target.value as EPDCategory | "all")
            }
            className="w-full mb-3 rounded-lg border border-border bg-surface-secondary py-1.5 px-3 text-sm text-content-primary"
            aria-label={t("sustainability.filter_category", {
              defaultValue: "Filter by category",
            })}
          >
            <option value="all">
              {t("common.all_categories", { defaultValue: "All Categories" })}
            </option>
              ...
              return (
                <div key={cat}>
                  <button
                    ...
                    aria-label={t("sustainability.toggle_category", {
                      defaultValue: "Toggle {{category}}",
                    ...
                    <div className="ml-4 space-y-0.5">
                      {materials.map((m) => (
                        ...
                        >
                          ...
                          <span
                            className={`ml-2 whitespace-nowrap font-mono ${m.gwp < 0 ? "text-emerald-600" : "text-content-tertiary"}`}
                          >
                            {m.gwp < 0 ? "" : "+"}
                            {m.gwp.toFixed(m.gwp < 1 ? 3 : 1)} {m.unit}
                          </span>
                        </div>
                      ))}
                    </div>
          ...
          <p className="mt-3 text-2xs text-content-quaternary flex items-start gap-1">
            ...
            {t("sustainability.data_sources", {
              defaultValue:
                "Data: Okobaudat, ICE v3.0 (Bath), EU Level(s). GWP A1-A3 (cradle to gate).",
            })}
          </p>
        </div>
        ...
        <div className="xl:col-span-2 space-y-4">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            <label className="flex items-center gap-2 text-sm text-content-secondary">
              {t("sustainability.gfa", {
                defaultValue: "Gross Floor Area (m2)",
              })}
              <input
                type="number"
                value={gfa}
            ...
            <div
              className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium ${cStyle.bg} ${cStyle.text}`}
            >
              <CIcon className="h-4 w-4" />
              {t("sustainability.eu_cpr_label", {
                defaultValue: "EU CPR",
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-secondary/50">
                  ...
                  <th className="px-3 py-2 text-right text-xs font-medium text-content-tertiary">
                    {t("sustainability.gwp_column", {
                      defaultValue: "GWP (kg CO2e)",
                    })}
                  </th>
                  ...
                  >
                    <td className="px-3 py-2">
                      <input
                        ...
                        onChange={(e) =>
                          updatePosition(item.id, "description", e.target.value)
                        }
                        className="w-full bg-transparent text-content-primary outline-none"
                        placeholder={t(
                          "sustainability.position_desc_placeholder",
                    ...
                    <td
                      className={`px-3 py-2 text-right font-mono text-xs ${item.gwp < 0 ? "text-emerald-600" : "text-content-primary"}`}
                    >
                      {item.gwp < 0 ? "" : "+"}
                      {item.gwp.toLocaleString("en", {
                    ...
                    <td className="px-3 py-2">
                      <button
                        ...
                        aria-label={t("sustainability.remove_position", {
              ...
              <tfoot>
                <tr className="border-t border-border bg-surface-secondary/30">
                  ...
                  <td
                    className={`px-3 py-2 text-right font-mono text-sm font-bold ${carbonBreakdown.totalGWP < 0 ? "text-emerald-600" : "text-content-primary"}`}
                  >
                    {carbonBreakdown.totalGWP.toLocaleString("en", {
                      maximumFractionDigits: 0,
                    })}{" "}
                    kg
                  </td>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="rounded-xl border border-border bg-surface-primary p-4">
              <p className="text-xs text-content-tertiary mb-1">
                {t("sustainability.total_gwp", {
                  defaultValue: "Total Embodied Carbon",
                })}
              </p>
              <p className="text-xl font-bold text-content-primary">
                {(carbonBreakdown.totalGWP / 1000).toFixed(1)}{" "}
                <span className="text-sm font-normal">t CO2e</span>
              </p>
            </div>
            <div className="rounded-xl border border-border bg-surface-primary p-4">
              <p className="text-xs text-content-tertiary mb-1">
                {t("sustainability.gwp_per_m2", {
                  defaultValue: "Carbon per m2 GFA",
                })}
              </p>
              <p className="text-xl font-bold text-content-primary">
                {carbonBreakdown.gwpPerM2.toFixed(1)}{" "}
                <span className="text-sm font-normal">kg CO2e/m2</span>
              </p>
            </div>
            <div className="rounded-xl border border-border bg-surface-primary p-4">
              <p className="text-xs text-content-tertiary mb-1">
                {t("sustainability.gwp_annual", {
                  defaultValue: "Annual (50yr RSP)",
                })}
              </p>
              <p className="text-xl font-bold text-content-primary">
                {carbonBreakdown.gwpPerM2Year.toFixed(2)}{" "}
                <span className="text-sm font-normal">kg/m2/yr</span>
              </p>
            </div>
          </div>
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-4">
              <h3 className="text-sm font-semibold text-content-primary mb-3">
                {t("sustainability.breakdown", {
                  defaultValue: "Carbon Breakdown by Category",
                })}
              </h3>
              <div className="space-y-2">
                {categoryTotals.map((cat) => (
                  <div key={cat.label} className="flex items-center gap-3">
                    <span className="w-40 text-xs text-content-secondary truncate">
                      {cat.label}
                    </span>
          ...
          <div className="rounded-xl border border-border bg-surface-secondary/30 p-4 text-xs text-content-tertiary">
            <p className="font-semibold text-content-secondary mb-1">
              {t("sustainability.cpr_benchmarks_title", {
                defaultValue:
                  "EU CPR 2024/3110 — GWP Benchmarks (A1-A3, 50yr RSP)",
              })}
            </p>
            <div className="flex flex-wrap gap-4 mt-2">
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-emerald-500" />{" "}
                {t("sustainability.level_excellent", {
                  defaultValue: "Excellent",
                })}
                : &le; {EU_CPR_BENCHMARKS.excellent} kg/m2/yr
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-green-500" />{" "}
                {t("sustainability.level_good", { defaultValue: "Good" })}: &le;{" "}
                {EU_CPR_BENCHMARKS.good} kg/m2/yr
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-amber-500" />{" "}
                {t("sustainability.level_acceptable", {
                  defaultValue: "Acceptable",
                })}
                : &le; {EU_CPR_BENCHMARKS.acceptable} kg/m2/yr
              </span>
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-red-500" />{" "}
                {t("sustainability.level_non_compliant", {
                  defaultValue: "Non-compliant",
                })}
                : &gt; {EU_CPR_BENCHMARKS.limit} kg/m2/yr
              </span>
            </div>
          </div>
        </div>
