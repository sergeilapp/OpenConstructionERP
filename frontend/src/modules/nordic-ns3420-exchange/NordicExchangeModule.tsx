    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t("nordic.preview", { defaultValue: "Preview‌⁠‍" })}:{" "}
          {positions.length}{" "}
...
export default function NordicExchangeModule() {
  ...
  const [exportFormat, setExportFormat] =
    useState<NordicExportFormat>("ns3420-detailed");
    ...
    async (file: File) => {
      ...
      try {
        const result = await parseExcelFile(
          file,
          NORDIC_TEMPLATE.defaultColumns,
        );
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      await apiGet<{ imported: number }>(
        ...
        {
          method: "POST",
          body: JSON.stringify({ positions, source: "nordic_import" }),
        } as never,
      );
      ...
      addToast({
        ...
        title: t("nordic.import_complete", {
          defaultValue: "Nordic BOQ import complete",
        }),
      ...
      addToast({
        type: "error",
        title: t("nordic.import_failed", {
          defaultValue: "Nordic import failed",
        }),
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      ...
      addToast({
        type: "success",
        title: t("nordic.export_complete", {
          defaultValue: "Nordic BOQ export complete",
        }),
      ...
      addToast({
        type: "error",
        title: t("nordic.export_failed", {
          defaultValue: "Nordic export failed",
        }),
        message: err instanceof Error ? err.message : "Unknown error",
      });
    } finally {
            ...
            {importFile ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t("nordic.drop_file", {
                    defaultValue:
                      "Drop a Nordic BOQ file here (Excel or CSV), or",
                  })}
                </p>
                <Button
                  variant="secondary"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select
                  value={exportFormat}
                  onChange={(e) =>
                    setExportFormat(e.target.value as NordicExportFormat)
                  }
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
