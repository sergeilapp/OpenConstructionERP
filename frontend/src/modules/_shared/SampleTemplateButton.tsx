...
> = {
  ordinal: {
    labelKey: "exchange.col_ordinal",
    label: "Item / Position no.",
    ex1: "01.01.001",
    ex2: "01.01.002",
  },
  description: {
    labelKey: "exchange.col_description",
    label: "Description",
    ex1: "Reinforced concrete C30/37, foundation slab",
    ex2: "Formwork to slab edges",
  },
  unit: { labelKey: "exchange.col_unit", label: "Unit", ex1: "m3", ex2: "m2" },
  quantity: {
    labelKey: "exchange.col_quantity",
    label: "Quantity",
    ex1: "125.000",
    ex2: "48.000",
  },
  unitRate: {
    labelKey: "exchange.col_unit_rate",
    label: "Unit rate",
    ex1: "142.50",
    ex2: "38.00",
  },
  total: {
    labelKey: "exchange.col_total",
    label: "Total",
    ex1: "17812.50",
    ex2: "1824.00",
  },
  section: {
    labelKey: "exchange.col_section",
    label: "Section / trade",
    ex1: "Substructure",
    ex2: "Substructure",
  },
  classification: {
    labelKey: "exchange.col_classification",
...
export function SampleTemplateButton({
  template,
}: {
  ...
  const handleDownload = () => {
    ...
    const row1 = cols.map((k) =>
      k === "classification" ? "" : COLUMN_META[k].ex1,
    );
    const row2 = cols.map((k) =>
      k === "classification" ? "" : COLUMN_META[k].ex2,
    );
    ...
    <div className="rounded-lg border border-border-light bg-surface-secondary/30 overflow-hidden">
      ...
      {open && (
        <div className="px-3 pb-3 pt-1 space-y-2.5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {cols.map((k, i) => (
              <span
                ...
                title={
                  required.has(k)
                    ? t("exchange.col_required", {
                        defaultValue: "Required‌⁠‍",
                      })
                    : t("exchange.col_optional", {
                        defaultValue: "Optional‌⁠‍",
                      })
                }
              >
                <span className="font-mono text-content-quaternary">
                  {i + 1}
                </span>
                <span className="font-medium">
                  {k === "classification"
                    ? template.classification ||
                      t(COLUMN_META[k].labelKey, {
                        defaultValue: COLUMN_META[k].label,
                      })
                    : t(COLUMN_META[k].labelKey, {
                        defaultValue: COLUMN_META[k].label,
                      })}
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-2xs text-content-quaternary">
            <span>
              ...
              <span className="font-mono">
                {template.acceptedExtensions.join("  ")}
              </span>
            </span>
            <span>
          ...
          >
            <Download size={12} />
            {t("exchange.download_sample", {
              defaultValue: "Download a ready-to-fill sample CSV",
            })}
          </button>
        </div>
      )}
