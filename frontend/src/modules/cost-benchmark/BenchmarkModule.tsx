...
function getPercentileLabelKey(pct: number): {
  key: string;
  defaultValue: string;
} {
  if (pct <= 25)
    return {
      key: "benchmarks.pct_below_avg",
      defaultValue: "Below average (cost-effective)‌⁠‍",
    };
  if (pct <= 50)
    return {
      key: "benchmarks.pct_below_median",
      defaultValue: "Below median‌⁠‍",
    };
  if (pct <= 75)
    return {
      key: "benchmarks.pct_above_median",
      defaultValue: "Above median‌⁠‍",
    };
  return {
    key: "benchmarks.pct_above_avg",
    defaultValue: "Above average (premium)‌⁠‍",
  };
}
...
export default function BenchmarkModule() {
  ...
  const analysis = useMemo(() => {
    ...
    const diffPct =
      benchmarkRange.median > 0
        ? (diffFromMedian / benchmarkRange.median) * 100
        : 0;
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-content-tertiary">
            {t("benchmarks.subtitle", {
              defaultValue: "Compare your estimate against industry benchmarks",
            })}
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface-primary p-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-content-tertiary mb-1">
              {t("benchmarks.total_cost", { defaultValue: "Your Total Cost" })}{" "}
              ({regionInfo.currency})
            </label>
            <input
              type="number"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="rounded-xl border border-border bg-surface-primary p-5">
          ...
          <p className="text-xs text-content-tertiary mt-1">
            {t("benchmarks.median", { defaultValue: "Median" })}:{" "}
            {formatCurrency(benchmarkRange.median, regionInfo.currency)}/m2
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-surface-primary p-5">
          <p className="text-xs text-content-tertiary mb-1">
            {t("benchmarks.percentile", {
              defaultValue: "Percentile Position",
            })}
          </p>
          <p
            className={`text-2xl font-bold ${getPercentileColor(analysis.percentile)}`}
          >
            P{analysis.percentile.toFixed(0)}
          </p>
          <p className="text-xs text-content-tertiary mt-1">
            {(() => {
              const lbl = getPercentileLabelKey(analysis.percentile);
              return t(lbl.key, { defaultValue: lbl.defaultValue });
            })()}
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-surface-primary p-5">
          <p className="text-xs text-content-tertiary mb-1">
            {t("benchmarks.diff_median", {
              defaultValue: "Difference from Median",
            })}
          </p>
          <p
            className={`text-2xl font-bold ${analysis.diffFromMedian > 0 ? "text-red-600" : "text-emerald-600"}`}
          >
            {analysis.diffFromMedian > 0 ? "+" : ""}
            {formatCurrency(analysis.diffFromMedian, regionInfo.currency)}
          </p>
          <p className="text-xs text-content-tertiary mt-1">
            {analysis.diffPct > 0 ? "+" : ""}
            {analysis.diffPct.toFixed(1)}%{" "}
            {t("benchmarks.vs_median", { defaultValue: "vs median" })}
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface-primary p-5">
        ...
        <div className="relative">
          ...
          <div className="flex justify-between mt-2 text-2xs text-content-quaternary">
            <span>
              {formatCurrency(benchmarkRange.min, regionInfo.currency)}
            </span>
            <span>
              {t("benchmarks.q1_short", { defaultValue: "Q1" })}:{" "}
              {formatCurrency(benchmarkRange.q1, regionInfo.currency)}
            </span>
            <span>
              {t("benchmarks.median", { defaultValue: "Median" })}:{" "}
              {formatCurrency(benchmarkRange.median, regionInfo.currency)}
            </span>
            <span>
              {t("benchmarks.q3_short", { defaultValue: "Q3" })}:{" "}
              {formatCurrency(benchmarkRange.q3, regionInfo.currency)}
            </span>
            <span>
              {formatCurrency(benchmarkRange.max, regionInfo.currency)}
            </span>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-5 gap-2 text-center text-xs">
          <div className="rounded-lg bg-emerald-50 dark:bg-emerald-950/20 p-2">
            <p className="text-content-tertiary">
              {t("benchmarks.min", { defaultValue: "Min" })}
            </p>
          ...
          <div className="rounded-lg bg-green-50 dark:bg-green-950/20 p-2">
            <p className="text-content-tertiary">
              {t("benchmarks.q1", { defaultValue: "Q1 (25th)" })}
            </p>
            <p className="font-semibold text-content-primary">
              {formatCurrency(benchmarkRange.q1, regionInfo.currency)}
            </p>
          ...
          <div className="rounded-lg bg-amber-50 dark:bg-amber-950/20 p-2">
            <p className="text-content-tertiary">
              {t("benchmarks.q3", { defaultValue: "Q3 (75th)" })}
            </p>
            <p className="font-semibold text-content-primary">
              {formatCurrency(benchmarkRange.q3, regionInfo.currency)}
            </p>
          ...
          <div className="rounded-lg bg-red-50 dark:bg-red-950/20 p-2">
            <p className="text-content-tertiary">
              {t("benchmarks.max", { defaultValue: "Max" })}
            </p>
            <p className="font-semibold text-content-primary">
              {formatCurrency(benchmarkRange.max, regionInfo.currency)}
            </p>
      ...
      <div className="rounded-xl border border-border bg-surface-primary p-5">
        <h3 className="text-sm font-semibold text-content-primary mb-3">
          {t("benchmarks.all_types", { defaultValue: "All Building Types" })} —{" "}
          {regionInfo.label}
        </h3>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 rounded-lg px-3 py-2 text-left transition-all ${
                  isSelected
                    ? "bg-oe-blue/10 ring-1 ring-oe-blue"
                    : "hover:bg-surface-secondary"
                }`}
                aria-pressed={isSelected}
                aria-label={t("benchmarks.select_type", {
                  defaultValue: "Select {{type}}",
                  type: bt.label,
                })}
              >
                <span className="w-44 text-sm text-content-primary truncate">
                  {bt.label}
                </span>
                <div className="flex-1 h-4 bg-surface-secondary rounded-full overflow-hidden relative">
                  ...
                  <div
                    ...
                    style={{
                      left: `${((range.median - range.min) / (range.max - range.min)) * 100}%`,
                    }}
                  />
                </div>
      ...
      <div className="flex items-start gap-2 text-xs text-content-quaternary">
        ...
        <p>
          {t("benchmarks.disclaimer", {
            defaultValue:
              "Benchmark data from BKI (DE), BCIS (UK), ENR (US), Stat. Austria (AT), SIA/BFS (CH). Values represent KG 300+400 (construction + technical systems) costs per m2 GFA. Actual costs vary by location, specification, and market conditions.",
          })}
        </p>
      </div>
    </div>
