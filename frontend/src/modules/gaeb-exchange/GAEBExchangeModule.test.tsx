...
function makeOptions(
  overrides?: Partial<GAEBExportOptions>,
): GAEBExportOptions {
  return {
    format: "X83",
...
describe("generateGAEBXML", () => {
  ...
  it("escapes XML special characters in descriptions", () => {
    ...
    const result = generateGAEBXML(
      makeOptions({ positions: positionsWithSpecialChars }),
    );
  ...
  it("preserves multi-level hierarchy on export (Los → Titel → Position)", () => {
    const positions: ExportPosition[] = [
      ...
      },
      {
        id: "s2",
        ordinal: "01.01",
        description: "Titel 1.1 — Erdarbeiten",
        unit: "",
        quantity: 0,
        unitRate: 0,
        total: 0,
    ...
    const titel11 = result.xml.indexOf(
      "<LblTx>Titel 1.1 — Erdarbeiten</LblTx>",
    );
    ...
    expect(parsed.find((p) => p.description === "Aushub")?.ordinal).toBe(
      "01.01.001",
    );
    expect(parsed.find((p) => p.description === "C30/37")?.ordinal).toBe(
      "01.02.001",
    );
  });
  ...
  it("round-trips XML special characters in descriptions", () => {
    ...
    expect(parsed[0].description).toBe(
      'Wall > 2m & "tested" <special> apostrophe\'s',
    );
  });
