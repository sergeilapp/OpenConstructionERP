  ...
  return (
    <div className="space-y-2">
      <h4 className="text-xs font-semibold text-content-tertiary uppercase tracking-wide">
        {t("risk.distribution", {
          defaultValue: "Cost Distribution (Histogram)‌⁠‍",
        })}
      </h4>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <div className="w-20 h-2 bg-surface-tertiary rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(driver.contributionPct, 100)}%`,
                        }}
                      />
                    </div>
...
function ParamRow({
  ...
  return (
    <tr className="hover:bg-surface-secondary/30 transition-colors">
      <td className="px-3 py-1.5 font-mono text-2xs text-content-tertiary">
        {param.ordinal}
      </td>
      <td
        className="px-3 py-1.5 text-xs text-content-primary max-w-[200px] truncate"
        title={param.description}
      >
        {param.description || "-"}
      </td>
      <td className="px-3 py-1.5 text-right text-xs tabular-nums">
        {fmtCurrency(param.baseCost)}
      </td>
      <td className="px-1 py-1.5">
        <input
          ...
          onChange={(e) =>
            onChange({
              ...param,
              optimistic: parseFloat(e.target.value) || 0.85,
            })
          }
      ...
      <td className="px-1 py-1.5">
        <input
          ...
          onChange={(e) =>
            onChange({
              ...param,
              pessimistic: parseFloat(e.target.value) || 1.25,
            })
          }
          className="w-16 rounded border border-border bg-surface-secondary px-1.5 py-1 text-xs text-center tabular-nums"
      ...
      <td className="px-1 py-1.5">
        ...
        >
          <option value="triangular">
            {t("risk.dist_triangular", { defaultValue: "Triangular" })}
          </option>
          <option value="pert">
            {t("risk.dist_pert", { defaultValue: "PERT" })}
          </option>
          <option value="uniform">
            {t("risk.dist_uniform", { defaultValue: "Uniform" })}
          </option>
        </select>
      </td>
    </tr>
...
export default function RiskAnalysisModule() {
  ...
  const [defaultDistribution, setDefaultDistribution] =
    useState<DistributionType>("triangular");
  ...
  >({
    ...
    queryFn: async () => {
      const boq = await apiGet<{ positions?: BOQPositionForRisk[] }>(
  ...
  const handleRun = useCallback(() => {
    if (params.length === 0) {
      addToast({
        type: "warning",
        title: t("risk.no_params", { defaultValue: "No positions loaded" }),
      });
      return;
    }
    setIsRunning(true);
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const lines = [
      ...
      ...result.riskDrivers
        ...
        .map(
          (d) =>
            `"${d.ordinal}","${d.description}",${d.baseCost.toFixed(2)},${d.contributionPct.toFixed(2)}`,
        ),
    ];
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-content-tertiary">
            {t("risk.subtitle", {
              defaultValue:
                "Probabilistic cost estimation with Monte Carlo simulation",
            })}
          </p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface-primary p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            >
              <option value="">
                {t("risk.select_boq_item", { defaultValue: "— Select BOQ —" })}
              </option>
              {boqs.map((b) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mt-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDefaultOptimistic(parseFloat(e.target.value) || 0.85)
                  }
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
                  {t("risk.pessimistic", {
                    defaultValue: "Default Pessimistic",
                  })}
                </label>
                <input
                  type="number"
                  ...
                  onChange={(e) =>
                    setDefaultPessimistic(parseFloat(e.target.value) || 1.25)
                  }
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDefaultDistribution(e.target.value as DistributionType)
                  }
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
          ...
          >
            <h3 className="text-sm font-semibold text-content-primary">
              ...
              <span className="ml-2 text-xs font-normal text-content-tertiary">
                ({params.length}{" "}
                {t("risk.positions_label", { defaultValue: "positions" })})
              </span>
            </h3>
            {showParams ? (
              <ChevronDown size={14} />
            ...
            <div className="border-t border-border-light overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-surface-tertiary/50">
                    <th className="px-3 py-2 text-left font-medium text-content-secondary">
                      {t("boq.ordinal", { defaultValue: "Ord." })}
                    </th>
                    <th className="px-3 py-2 text-left font-medium text-content-secondary">
                      {t("boq.description", { defaultValue: "Description" })}
                    </th>
                    ...
                    <th className="px-3 py-2 text-center font-medium text-content-secondary">
                      {t("risk.optimistic", { defaultValue: "Opt. (x)" })}
                    </th>
                    <th className="px-3 py-2 text-center font-medium text-content-secondary">
                      {t("risk.pessimistic", { defaultValue: "Pess. (x)" })}
                    </th>
                    <th className="px-3 py-2 text-center font-medium text-content-secondary">
                      {t("risk.dist", { defaultValue: "Distribution" })}
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border-light">
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-5">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
              ...
              <PercentileCard
                label="P80"
                value={result.percentiles.p80}
                variant="orange"
              />
              <PercentileCard
                label="P90"
                value={result.percentiles.p90}
                variant="red"
              />
              <PercentileCard
                label="P95"
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-lg font-bold text-blue-700 dark:text-blue-400 tabular-nums mt-0.5">
                    ...
                    <span className="text-sm font-medium text-blue-600/70">
                      ({result.contingencyPct.toFixed(1)}%)
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-2xs font-medium text-content-tertiary uppercase tracking-wide">
                    {t("risk.recommended_budget", {
                      defaultValue: "Recommended Budget (P80)",
                    })}
                  </div>
          ...
          <div className="flex items-start gap-2 text-xs text-content-quaternary">
            ...
            <p>
              {t("risk.disclaimer", {
                defaultValue:
                  "Monte Carlo simulation uses random sampling to model cost uncertainty. Results are probabilistic estimates, not guarantees. The simulation assumes independent cost drivers. Adjust optimistic/pessimistic multipliers per position for more accurate results. Recommended: use P80 for budget planning.",
              })}
            </p>
          </div>
        </div>
