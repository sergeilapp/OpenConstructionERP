...
export function BackendModulePage({
  ...
  const description = t(`backend_modules.${moduleKey}.desc`, {
    defaultValue:
      "Backend API is live. The interactive UI for this module is in progress.‌⁠‍",
  });
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="inline-flex items-center rounded-full bg-white/15 text-white text-[11px] font-semibold uppercase tracking-wide px-2.5 py-0.5">
              {t("backend_modules.badge", {
                defaultValue: "Backend API live‌⁠‍",
              })}
            </span>
          </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mt-6">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-3">
            {t("backend_modules.endpoints_title", {
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
            {t("backend_modules.endpoints_body", {
              defaultValue:
                "All routes are mounted and authenticated. Browse the OpenAPI spec to explore models, schemas, and try requests.‌⁠‍",
            })}
          </p>
          <a
            href="/docs"
          ...
          >
            {t("backend_modules.open_api", {
              defaultValue: "Open API docs‌⁠‍",
            })}
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
            {t("backend_modules.ui_body", {
              defaultValue:
                "The interactive frontend for this module is on the roadmap. Tell us your priority — we ship most-requested modules first, for free.",
            })}
          </p>
          <Link
            ...
            className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-violet-600 dark:text-violet-400 hover:underline"
          >
            {t("backend_modules.dev_guide", {
              defaultValue: "Build the UI yourself",
            })}
            <ArrowRight size={14} />
          </Link>
        </div>
      ...
      {highlights && highlights.length > 0 && (
        <div className="mt-6 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-3">
            {t("backend_modules.what_works", {
              defaultValue: "What's already working",
            })}
          </h2>
      ...
      <div className="mt-6 rounded-2xl border border-violet-200 dark:border-violet-900/60 bg-violet-50/50 dark:bg-violet-950/30 p-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
        <div className="min-w-0">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
            {t("backend_modules.cta_title", {
              defaultValue: "Need this UI soon?",
            })}
          </h2>
          <p className="text-sm text-slate-700 dark:text-slate-300 mt-0.5">
            {t("backend_modules.cta_body", {
              defaultValue:
                "Drop us a line — popular requests get built for free.",
            })}
          </p>
        </div>
        <a
