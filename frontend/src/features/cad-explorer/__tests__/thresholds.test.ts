...
describe("thresholds — resolveThresholdColor", () => {
  ...
  it("returns null when the rule has non-finite bounds", () => {
    expect(
      resolveThresholdColor(0.5, makeRule({ low: NaN, high: 1 })),
    ).toBeNull();
    expect(
      resolveThresholdColor(0.5, makeRule({ low: 0.8, high: Infinity })),
    ).toBeNull();
  });
...
describe("thresholds — URL round-trip", () => {
  ...
  it("tolerates malformed input gracefully", () => {
    ...
    expect(
      parseThresholds("volume~0~1~ef4444~f59e0b~10b981;broken"),
    ).toHaveLength(1);
  });
...
describe("thresholds — findRuleForColumn", () => {
  ...
  it("returns the matching rule", () => {
    const rules = [
      makeRule({ column: "area" }),
      makeRule({ column: "volume" }),
    ];
...
describe("thresholds — isRuleValid", () => {
  ...
  it("rejects when low >= high", () => {
    expect(isRuleValid(makeRule({ low: 1, high: 0.5 }), ["volume"])).toBe(
      false,
    );
