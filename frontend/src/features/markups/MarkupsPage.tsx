              ...
              {ALL_MARKUP_TYPES.map((tp) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      size={13}
                      className={isSelected ? TYPE_COLORS[tp] : ""}
                    />
...
function StatsBar({ summary }: { summary: MarkupsSummary | undefined }) {
  ...
  const typeCounts = ALL_MARKUP_TYPES.filter((tp) => (byType[tp] ?? 0) > 0).map(
    (tp) => ({
      type: tp,
      count: byType[tp] ?? 0,
      Icon: TYPE_ICONS[tp],
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-content-secondary">
            {markup.document_id
              ? documentName ||
                t("markups.unknown_document", { defaultValue: "Document" })
        ...
        <div>
          ...
          <p className="text-sm text-content-secondary">
            {markup.linked_boq_position_id
              ? t("markups.linked_to_boq", {
                  defaultValue: "Linked to a BOQ position",
                })
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium capitalize text-content-primary">
            {t(`markups.type_${markup.type}`, {
              defaultValue: TYPE_LABELS[markup.type],
            })}
          </span>
        </div>
        <Badge
          variant={STATUS_BADGE_VARIANT[markup.status] ?? "neutral"}
          size="sm"
        >
          {t(`markups.status_${markup.status}`, {
            defaultValue:
              markup.status.charAt(0).toUpperCase() + markup.status.slice(1),
          })}
        </Badge>
      </div>
...
export function MarkupsPage() {
  ...
  const [annotateStamp, setAnnotateStamp] = useState<string | undefined>(
    undefined,
  );
  ...
  const statusMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("markups.status_updated", {
          defaultValue: "Markup status updated",
        }),
      });
    },
    onError: (e: Error) =>
  ...
  const handleExportCSV = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        title: t("markups.exported", {
          defaultValue: "Markups exported to CSV",
        }),
      });
    } catch (e) {
      addToast({
  ...
  const displayStamps = useMemo(() => {
    ...
    const result: Array<{ name: string; label: string; color: string }> =
      DEFAULT_STAMPS.map((def) => {
        const apiStamp = apiStampMap.get(def.name);
        return apiStamp
          ? {
              name: apiStamp.name,
              label: apiStamp.text || apiStamp.name,
    ...
    <div className="mx-auto max-w-7xl px-4 py-4 animate-fade-in">
      ...
      <Breadcrumb
        items={[
          ...
          {
            label: t("markups.title", {
              defaultValue: "Markups & Annotations",
            }),
          },
        ]}
      />
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {projectId && (
            <Button
              ...
              onClick={() => {
                ...
                navigate(
                  `/markups/compare${params.toString() ? `?${params.toString()}` : ""}`,
                );
              }}
              className="shrink-0 whitespace-nowrap"
              title={t("markups.compare_revisions", {
                defaultValue: "Compare revisions",
              })}
            >
              <GitCompare size={14} />
          ...
          {documents.length > 0 && (
            <Button
              ...
              className="shrink-0 whitespace-nowrap"
              title={t("markups.annotate_pdf", {
                defaultValue: "Annotate on PDF",
              })}
            >
              <PenTool size={14} />
              <span className="hidden lg:inline ml-1">
                {t("markups.annotate_pdf", { defaultValue: "Annotate" })}
              </span>
            </Button>
          )}
          ...
          >
            <Plus size={14} />
            <span className="hidden lg:inline ml-1">
              {t("markups.add_markup", { defaultValue: "Add" })}
            </span>
          </Button>
      ...
      {!projectId ? (
        <div className="mt-10">
          <EmptyState
            ...
            title={t("markups.no_project_title", {
            ...
            action={{
              label: t("markups.select_project_btn", {
        ...
        <>
          ...
          <div
            ...
            role="tablist"
            aria-label={t("markups.scope_tabs", {
              defaultValue: "Annotation scope",
            })}
          >
            <button
              type="button"
          ...
          {annotateDocId && (
            <div className="mt-3">
              <InlinePdfAnnotator
                ...
                documentName={
                  documents.find((d) => d.id === annotateDocId)?.name ||
                  "Document"
                }
                projectId={projectId}
                onClose={() => {
              ...
              {showFilters && (
                <div className="mt-2 flex items-center gap-2 flex-wrap animate-fade-in">
                  <select
                    value={filterType}
                    onChange={(e) =>
                      setFilterType(e.target.value as MarkupType | "")
                    }
                    className={selectCls + " max-w-[140px]"}
                  >
                    <option value="">
                  ...
                  <select
                    value={filterStatus}
                    onChange={(e) =>
                      setFilterStatus(e.target.value as MarkupStatus | "")
                    }
                    className={selectCls + " max-w-[140px]"}
                  >
                    <option value="">
                      {t("markups.all_statuses", {
                  ...
                  <EmptyState
                    ...
                    title={
                      searchQuery || filterType || filterStatus
                        ? t("markups.no_match_title", {
                            defaultValue: "No matching markups",
                          })
                        : t("markups.empty_title", {
                            defaultValue: "No markups yet",
                          })
                    }
                    description={
                      searchQuery || filterType || filterStatus
                    ...
                    action={
                      searchQuery || filterType || filterStatus
                        ? undefined
                        : {
                            label: t("markups.add_first", {
                              defaultValue: "Add First Markup",
                            }),
                            onClick: () => setShowAddModal(true),
                          }
                    }
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (documents.length > 0) {
                        ...
                        setAnnotateDocId(
                          filterDocumentId || documents[0]?.id || null,
                        );
                      } else {
                        addToast({
                          type: "info",
                          title: t("markups.upload_doc_first", {
                            defaultValue:
                              "Upload a document first to place stamps on it.",
                          }),
                        });
                      }
                    }}
                    ...
                    title={t("markups.click_to_place_stamp", {
                      defaultValue: "Click to place this stamp on a document",
                    })}
                  >
                    <Stamp size={11} />
                    {t(`markups.stamp_${stamp.name}`, {
      ...
      {showCustomStampForm && (
        <div
          ...
          onClick={() => setShowCustomStampForm(false)}
        >
          <div
          ...
          >
            <h3 className="text-sm font-semibold text-content-primary mb-4">
              {t("markups.create_custom_stamp", {
                defaultValue: "Create Custom Stamp",
              })}
            </h3>
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder={t("markups.stamp_text_placeholder", {
                    defaultValue: "e.g. CHECKED, HOLD, RFI...",
                  })}
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    addToast({
                      type: "success",
                      title: t("markups.stamp_created", {
                        defaultValue: "Stamp created",
                      }),
                    });
