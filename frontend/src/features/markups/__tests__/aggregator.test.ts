...
function makeDwgAnnotation(
  overrides: Partial<DwgAnnotation> = {},
): DwgAnnotation {
  return {
    id: "dwg-1",
    drawing_id: "draw-42",
...
describe("aggregator normalisation", () => {
  ...
  it("normalises a DWG annotation — no page, drawing file name, deep link", () => {
    ...
    expect(u.deepLink).toBe(
      "/dwg-takeoff?drawingId=draw-42&annotationId=dwg-1",
    );
  });
  ...
  it('coerces unknown type values to "other"', () => {
    const u = fromPdfMeasurement(
      makePdfMeasurement({
        type: "alien_glyph" as MeasurementResponse["type"],
      }),
    );
...
describe("applyFilters", () => {
  const dataset: UnifiedMarkup[] = [
    ...
    fromDwgAnnotation(
      makeDwgAnnotation({ id: "d1", type: "arrow", text: "here" }),
      {
        id: "draw-1",
        project_id: "p1",
        name: "Floor1.dxf",
...
describe("summarise", () => {
  it("counts by source/type and collects unique files", () => {
    const items: UnifiedMarkup[] = [
      fromMarkupsHub(makeHubMarkup({ id: "h1", type: "cloud" }), {
        documentName: "A.pdf",
      }),
      fromMarkupsHub(makeHubMarkup({ id: "h2", type: "cloud" }), {
        documentName: "A.pdf",
      }),
