...
async function loadPdfFromDocId(
  documentId: string,
...
function PaneCanvas({
  ...
  useEffect(() => {
    ...
    pdfDoc
      .getPage(Math.min(page, pdfDoc.numPages))
      .then(
        (p) => {
  ...
  return (
    <div className="flex flex-col h-full min-w-0">
      ...
      <div className="flex items-center gap-1.5 px-2 py-1.5 border-b border-border-light bg-surface-secondary/60 shrink-0">
        ...
        <span className="text-xs text-content-tertiary truncate ml-1">
          {docName}
        </span>
      </div>
      {/* Canvas scroll area */}
      ...
      >
        ...
        {!pdfDoc && !isLoading && (
          <div className="absolute inset-0 flex items-center justify-center text-content-tertiary text-xs">
            {t("pdf_compare.select_doc", {
              defaultValue: "Select a document above",
            })}
          </div>
        )}
      </div>
...
function DiffCanvas({
  ...
  useEffect(() => {
    ...
    Promise.all([docA.getPage(pageA), docB.getPage(pageB)])
      .then(
        ([pa, pb]) => {
          const { promise: promA, cancel: cA } = renderPageToOffscreen(
            pa,
            zoom,
          );
          const { promise: promB, cancel: cB } = renderPageToOffscreen(
            pb,
            zoom,
          );
          cancelA = cA;
...
function OverlayCanvas({
  ...
  useEffect(() => {
    ...
    const getPageB =
      pageB !== null && docB ? docB.getPage(pageB) : Promise.resolve(null);

    Promise.all([docA.getPage(pageA), getPageB])
      .then(
        ([pa, pb]) => {
          const { promise: promA, cancel: cA } = renderPageToOffscreen(
            pa,
...
export function PdfComparePage() {
  ...
  const {
    ...
    loading: loadingA,
    error: errorA,
  } = usePdfDoc(selectedDocAId || null);
  const {
    doc: docB,
    loading: loadingB,
    error: errorB,
  } = usePdfDoc(selectedDocBId || null);
  ...
  const { data: documents = [], isLoading: loadingDocs } = useQuery<DocItem[]>({
    ...
    queryFn: () =>
      apiGet<DocItem[]>(`/v1/documents/?project_id=${activeProjectId}`),
  ...
  const MODES: { id: CompareMode; label: string; icon: React.ElementType }[] = [
    {
      id: "overlay",
      label: t("pdf_compare.mode_overlay", { defaultValue: "Overlay" }),
      icon: Layers,
    },
    {
      id: "diff",
      label: t("pdf_compare.mode_diff", { defaultValue: "Difference" }),
      icon: Diff,
    },
    {
      id: "sidebyside",
      label: t("pdf_compare.mode_side", { defaultValue: "Side-by-side" }),
      ...
      <div className="shrink-0 px-4 py-2 border-b border-border-light bg-surface-elevated flex items-center gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-1.5">
          <span
            ...
            title={t("pdf_compare.doc_a_old", {
              defaultValue: "Old revision (A)",
            })}
          >
            A
          </span>
          {loadingDocs ? (
            ...
            >
              <option value="">
                {t("pdf_compare.choose_doc", {
                  defaultValue: "— choose PDF —",
                })}
              </option>
              {pdfDocs.map((d) => (
        ...
        <div className="flex items-center gap-1.5">
          <span
            ...
            title={t("pdf_compare.doc_b_new", {
              defaultValue: "New revision (B)",
            })}
          >
            B
          </span>
        ...
        <div
          ...
          role="tablist"
          aria-label={t("pdf_compare.compare_mode", {
          ...
          {totalPages < maxPages && (
            <span
              ...
              title={t("pdf_compare.page_count_mismatch", {
                defaultValue:
                  "Documents have different page counts; clamped to {{min}}",
                min: totalPages,
              })}
            >
              *
        ...
        <button
          ...
          title={t("pdf_compare.reset", {
            defaultValue: "Reset zoom, page and pan",
          })}
        >
          <Maximize2 size={14} />
        </button>
      ...
      {mode === "overlay" && (
        <div className="shrink-0 px-4 py-2 border-b border-border-light bg-surface-secondary/40 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 text-2xs text-content-tertiary shrink-0">
            ...
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded-full bg-oe-blue inline-block" />B
            </span>
          </div>
        </div>
      )}
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-2 px-2 py-1.5 border-b border-border-light bg-surface-secondary/60 shrink-0">
              ...
              <span className="text-xs text-content-tertiary">
                <strong className="text-semantic-error/80">A</strong>:{" "}
                {docAName || t("pdf_compare.none", { defaultValue: "none" })} +{" "}
                <strong className="text-oe-blue">B</strong>:{" "}
                {docBName || t("pdf_compare.none", { defaultValue: "none" })}{" "}
                &mdash;{" "}
                {t("pdf_compare.opacity_b_short", { defaultValue: "B at" })}{" "}
                {overlayOpacity}%
              </span>
            </div>
            <OverlayCanvas
          ...
          <div className="flex-1 flex flex-col min-w-0">
            <div className="flex items-center gap-2 px-2 py-1.5 border-b border-border-light bg-surface-secondary/60 shrink-0">
              ...
              <span className="text-xs text-content-tertiary">
                <strong className="text-semantic-error/80">A</strong>:{" "}
                {docAName || t("pdf_compare.none", { defaultValue: "none" })}{" "}
                &rarr; <strong className="text-oe-blue">B</strong>:{" "}
                {docBName || t("pdf_compare.none", { defaultValue: "none" })}
              </span>
            </div>
            <DiffCanvas
      ...
      <div className="shrink-0 flex items-center justify-between px-4 py-1 border-t border-border-light bg-surface-secondary/30 text-2xs text-content-tertiary">
        <span>
          {t("pdf_compare.status_hint", {
            defaultValue:
              "Drag to pan · Zoom controls above · Keyboard: ← → page navigation",
          })}
        </span>
        {docA && docB && totalPages < maxPages && (
          <span className="text-amber-500">
            {t("pdf_compare.page_count_warning", {
              defaultValue:
                "Different page counts — navigating up to {{max}} pages",
              max: maxPages,
            })}
          </span>
        )}
