...
export function InlinePdfAnnotator({
  ...
  const [selectedStamp, setSelectedStamp] = useState(
    initialStamp || "approved",
  );
  const availableStamps = externalStamps?.length
    ? externalStamps
    : DEFAULT_STAMPS;
      ...
      try {
        ...
        const loaded: DrawnAnnotation[] = existing.map((m: Markup) => {
          ...
          return {
            id: m.id,
            tool:
              (m.geometry?.tool as AnnotationTool) ||
              (m.type as AnnotationTool) ||
              "rectangle",
            points: (m.geometry?.points as { x: number; y: number }[]) || [
              { x: 50, y: 50 },
            ],
            ...
            page: m.page || 1,
            stampName:
              (m.geometry?.stamp_name as string) ||
              (m.type === "stamp" ? m.label || undefined : undefined),
          };
        });
  ...
  const handleSaveAll = useCallback(async () => {
    ...
    if (unsaved.length === 0) {
      addToast({
        type: "info",
        title: t("markups.nothing_to_save", {
          defaultValue: "No annotations to save‌⁠‍",
        }),
      });
      return;
    }
              ...
              style={{
                left:
                  ...
                  (pageDimensionsRef.current.width /
                    (overlayRef.current?.getBoundingClientRect().width || 1)),
                top:
                  textPosition.y /
                  (pageDimensionsRef.current.height /
                    (overlayRef.current?.getBoundingClientRect().height || 1)),
              }}
            >
              <div className="flex items-center gap-1 bg-white dark:bg-neutral-800 rounded-lg shadow-lg border border-border-light p-1">
                <input
                  ...
                  placeholder={t("markups.type_text", {
      ...
      {pageAnnotationCount > 0 && (
        <div className="flex items-center justify-between px-3 py-1.5 border-t border-border-light bg-surface-secondary/30 text-2xs text-content-tertiary">
          ...
          <span>
            {t("markups.draw_hint", {
              defaultValue:
                "Click and drag to draw. Select a tool from the toolbar.",
            })}
          </span>
        </div>
      )}
