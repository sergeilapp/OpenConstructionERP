...
describe("TranslationSettingsTab", () => {
  ...
  it("renders the empty-state when no dictionaries are downloaded", async () => {
    ...
    expect(
      await screen.findByTestId("translation-dict-empty"),
    ).toBeInTheDocument();
  });
  ...
  it("renders the dictionary table when entries are present", async () => {
    ...
    expect(
      screen.getByTestId("translation-dict-row-iate-en-fr"),
    ).toBeInTheDocument();
  });
  ...
  it("renders the in-flight tasks card when a task is running", async () => {
    ...
    expect(
      await screen.findByTestId("translation-tasks-section"),
    ).toBeInTheDocument();
  ...
  it("exposes adaptive polling via useTranslationStatus (5 s while tasks running)", async () => {
    ...
    const query = observers.find(
      (q) => q.queryKey[0] === "translation" && q.queryKey[1] === "status",
    );
    expect(query).toBeTruthy();
    const interval = (
      query!.options.refetchInterval as
  ...
  it("surfaces a user-readable error toast when the download trigger fails", async () => {
    ...
    const lastCall =
      addToastSpy.mock.calls[addToastSpy.mock.calls.length - 1][0];
