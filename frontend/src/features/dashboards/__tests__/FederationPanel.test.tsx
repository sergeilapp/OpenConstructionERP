...
function renderPanel(
  ...
  return render(
    <QueryClientProvider client={client}>
      <FederationPanel available={SNAPSHOTS} initialSelection={[]} {...props} />
    </QueryClientProvider>,
  );
}
...
describe("FederationPanel", () => {
  it("toggles snapshot chips on click", () => {
    ...
    expect(screen.getByTestId("federation-chip-snap-1")).toBeInTheDocument();

    fireEvent.click(screen.getByTestId("federation-snapshot-option-snap-3"));
    expect(screen.getByTestId("federation-chip-snap-3")).toBeInTheDocument();
  ...
  it("calls buildFederation with the selected ids and chosen schema mode", async () => {
    (buildFederation as ReturnType<typeof vi.fn>).mockResolvedValue(
      BUILD_RESPONSE,
    );
    renderPanel();
  ...
  it("runs the aggregate after a build and renders the results table", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("federation-results-table"),
      ).toBeInTheDocument(),
    );
  ...
  it("surfaces a server error in the error banner", async () => {
    ...
    expect(screen.getByTestId("federation-error").textContent).toContain(
      "Schemas mismatch",
    );
  });
});
