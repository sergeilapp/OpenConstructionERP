...
function renderTimeline(
  props?: Partial<React.ComponentProps<typeof SnapshotTimeline>>,
) {
...
describe("SnapshotTimeline", () => {
  it("renders one row per snapshot in the timeline", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("snapshot-timeline-row-snap-1"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("snapshot-timeline-row-snap-2"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("snapshot-timeline-row-snap-3"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("clicking a card fires onActiveChange with the snapshot id", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("snapshot-timeline-card-snap-2"),
      ).toBeInTheDocument(),
    );
  ...
  it("ticking two snapshots enables Compare and calls onCompare older→newer", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("snapshot-timeline-compare-toggle-snap-1"),
      ).toBeInTheDocument(),
    );
    ...
    fireEvent.click(
      screen.getByTestId("snapshot-timeline-compare-toggle-snap-3"),
    );
    fireEvent.click(
      screen.getByTestId("snapshot-timeline-compare-toggle-snap-1"),
    );
  ...
  it("renders the empty state when the timeline has no entries", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.queryByTestId("snapshot-timeline-row-snap-1"),
