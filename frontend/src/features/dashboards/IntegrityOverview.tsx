        ...
        {reportQuery.isError && (
          ...
          >
            {t("dashboards.integrity_error", {
              defaultValue:
                "Could not load the integrity report for this snapshot.‌⁠‍",
            })}
          </div>
        )}
...
function CompletenessChip({ score }: CompletenessChipProps) {
  ...
  return (
    ...
    >
      {t("dashboards.integrity_completeness", { defaultValue: "Completeness" })}
      : {pct}%
    </span>
  );
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div role="columnheader">
          {t("dashboards.integrity_col_completeness", {
  ...
  return (
    <div data-testid={`integrity-row-${column.name}`}>
      ...
      >
        ...
        <div
          role="cell"
          className="text-right text-content-secondary tabular-nums"
        >
          {column.null_count}{" "}
          <span className="text-content-tertiary">({nullPctLabel})</span>
        </div>
        <div
          role="cell"
          className="text-right text-content-secondary tabular-nums"
      ...
      {isExpanded && <RowDetail column={column} rowCount={rowCount} />}
    </div>
  );
}
...
function CompletenessBar({ pct }: { pct: number }) {
  ...
  return (
    <div
      ...
      data-testid={`integrity-completeness-bar-${pct}`}
    >
      <div className="h-2 flex-1 overflow-hidden rounded bg-surface-secondary">
        <div className={`h-full ${fill}`} style={{ width: `${pct}%` }} />
      </div>
      <span className="w-9 text-right text-content-tertiary tabular-nums">
        {pct}%
      </span>
    </div>
  );
    ...
    >
      ...
      <div>
        ...
        <dl className="grid grid-cols-[100px_1fr] gap-x-2 gap-y-0.5 text-content-secondary">
          <dt>{t("dashboards.integrity_dtype", { defaultValue: "dtype" })}</dt>
          ...
          {column.inferred_type === "numeric" && (
            <>
              ...
              <dt>
                {t("dashboards.integrity_mean", { defaultValue: "mean" })}
              </dt>
              <dd className="tabular-nums text-content-primary">
                {formatStat(column.mean_value)}
              </dd>
              <dt>
                {t("dashboards.integrity_zero_pct", { defaultValue: "zero %" })}
              </dt>
              <dd className="tabular-nums text-content-primary">
              ...
              <dt>
                {t("dashboards.integrity_outliers", {
