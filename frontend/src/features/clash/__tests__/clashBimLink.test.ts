...
describe("parseClashDeepLink", () => {
  it("round-trips a built clash link", () => {
    ...
    const parsed = parseClashDeepLink(new URLSearchParams(link.split("?")[1]));
  ...
  it("treats a non-clash isolate link as not-clash with no focus", () => {
    const parsed = parseClashDeepLink(new URLSearchParams("isolate=x,y"));
  ...
  it("rejects a malformed focus (wrong arity / NaN)", () => {
    ...
    expect(
      parseClashDeepLink(new URLSearchParams("isolate=a&clash=1&focus=1,foo,3"))
        .focus,
    ).toBeNull();
  });
