...
export function SupplierCatalogsPage() {
  ...
  const vendorsQ = useQuery({
    ...
    queryFn: () =>
      listVendors({ status: statusFilter || undefined, limit: 200 }),
    enabled:
      tab === "vendors" ||
      tab === "catalog" ||
      tab === "prs" ||
  ...
  const itemsQ = useQuery({
    ...
    queryFn: () =>
      listCatalogItems({ search: search || undefined, limit: 200 }),
    enabled: tab === "catalog",
  });
  ...
  const filteredVendors = useMemo(
    () =>
      filterByText(
        vendorsArr,
        search,
        (v) => `${v.code} ${v.name} ${v.country_code ?? ""}`,
      ),
    [vendorsArr, search],
  );
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb
        items={[
          {
            label: t("supplier_catalogs.title", {
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-content-secondary">
            {t("supplier_catalogs.subtitle", {
              defaultValue:
                "Vendors, item catalogs, price comparison, requisitions, POs and warehouses.‌⁠‍",
            })}
          </p>
        </div>
        <Button
...
function tabsDef(t: (k: string, opts?: Record<string, unknown>) => string) {
  return [
    ...
    {
      id: "prs" as const,
      label: t("supplier_catalogs.tab_prs", { defaultValue: "PRs" }),
      icon: ClipboardList,
    },
    {
      id: "pos" as const,
      label: t("supplier_catalogs.tab_pos", { defaultValue: "POs" }),
      icon: ShoppingCart,
    },
    {
      id: "match" as const,
      label: t("supplier_catalogs.tab_match", { defaultValue: "3-Way Match" }),
      icon: FileCheck,
    },
    {
      id: "warehouses" as const,
      label: t("supplier_catalogs.tab_warehouses", {
...
function createLabel(
  ...
  switch (tab) {
    ...
    case "match":
      return t("supplier_catalogs.match_invoice", {
        defaultValue: "Match Invoice",
      });
    case "warehouses":
      return t("supplier_catalogs.new_warehouse", {
        defaultValue: "New Warehouse",
      });
  }
}
...
function StarRating({ rating }: { rating: number | null }) {
  ...
  return (
    <div className="inline-flex items-center gap-0.5">
      ...
      <span className="ml-1 text-2xs text-content-tertiary tabular-nums">
        {value}/5
      </span>
    </div>
  );
...
function VendorTable({
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={<Truck size={22} />}
        title={t("supplier_catalogs.empty_vendors", {
          defaultValue: "No vendors yet",
        })}
        description={t("supplier_catalogs.empty_vendors_desc", {
          defaultValue:
            "Register suppliers with payment terms and category coverage to buy from.",
        })}
        action={{
          label: t("supplier_catalogs.new_vendor", {
        ...
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.code", { defaultValue: "Code" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.name", { defaultValue: "Name" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.country", { defaultValue: "Country" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.rating", { defaultValue: "Rating" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.payment_terms", { defaultValue: "Terms" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.status", { defaultValue: "Status" })}
            </th>
          </tr>
        </thead>
        <tbody>
...
function CatalogTable({
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={<Boxes size={22} />}
        title={t("supplier_catalogs.empty_catalog", {
          defaultValue: "No catalog items yet",
        })}
        description={t("supplier_catalogs.empty_catalog_desc", {
          defaultValue:
            "SKUs you order — pipe, fittings, materials. Tie to multiple vendors for price comparison.",
        })}
        action={{
          label: t("supplier_catalogs.new_item", { defaultValue: "New Item" }),
          onClick: onAction,
        }}
      />
    );
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.sku", { defaultValue: "SKU" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.name", { defaultValue: "Name" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.uom", { defaultValue: "UoM" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("supplier_catalogs.manufacturer", {
                defaultValue: "Manufacturer",
              })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("supplier_catalogs.reorder", { defaultValue: "Reorder" })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("supplier_catalogs.actions", { defaultValue: "Actions" })}
            </th>
          </tr>
        </thead>
        <tbody>
            ...
            >
              ...
              <td className="px-4 py-2 text-right">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => onSelectPrice(r)}
                >
                  {t("supplier_catalogs.compare_prices", {
  ...
  return (
    <div className="flex flex-col items-center">
      <EmptyState
        ...
        action={{
          label: t("supplier_catalogs.new_pr", {
            defaultValue: "New Requisition",
          }),
          onClick: onAction,
        }}
      />
...
function MatchEmptyState() {
  ...
  return (
    <EmptyState
      icon={<AlertOctagon size={22} />}
      title={t("supplier_catalogs.match_empty", {
        defaultValue: "No match exceptions",
      })}
  ...
  if (warehouses.length === 0) {
    return (
      <EmptyState
        ...
        title={t("supplier_catalogs.empty_warehouses", {
          defaultValue: "No warehouses yet",
        })}
        description={t("supplier_catalogs.empty_warehouses_desc", {
          defaultValue:
            "Register storage locations to track stock on hand, reservations and movements.",
        })}
        action={{
          label: t("supplier_catalogs.new_warehouse", {
            defaultValue: "New Warehouse",
          }),
          onClick: onAction,
        }}
    ...
    <div>
      <div className="px-5 py-3 border-b border-border-light flex items-center gap-3 flex-wrap">
        ...
        {selected?.address && (
          <div>
            ...
            <p className="text-xs text-content-secondary truncate max-w-[320px]">
              {selected.address}
            </p>
          </div>
        )}
      ...
      {balances.length === 0 ? (
        <div className="p-6 text-center text-sm text-content-tertiary">
          {t("supplier_catalogs.no_stock", {
            defaultValue: "No stock balances recorded.",
          })}
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
              <tr>
                <th className="px-4 py-2.5 text-left">
                  {t("supplier_catalogs.item", { defaultValue: "Item" })}
                </th>
                <th className="px-4 py-2.5 text-left">
                  {t("supplier_catalogs.batch", { defaultValue: "Batch" })}
                </th>
                <th className="px-4 py-2.5 text-right">
                  {t("supplier_catalogs.on_hand", { defaultValue: "On hand" })}
                </th>
                <th className="px-4 py-2.5 text-right">
                  {t("supplier_catalogs.reserved", {
                    defaultValue: "Reserved",
                  })}
                </th>
                <th className="px-4 py-2.5 text-right">
                  {t("supplier_catalogs.unit_cost_avg", {
                    defaultValue: "Avg cost",
                  })}
                </th>
                <th className="px-4 py-2.5 text-left">
                  {t("supplier_catalogs.last_moved", {
                    defaultValue: "Last moved",
                  })}
                </th>
              </tr>
            </thead>
            <tbody>
              {balances.map((b) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 text-right text-xs tabular-nums">
                    {String(b.quantity_on_hand)}
                  </td>
                  <td className="px-4 py-2 text-right text-xs tabular-nums">
                    {String(b.quantity_reserved)}
                  </td>
                  <td className="px-4 py-2 text-right text-xs tabular-nums">
                  ...
                  <td className="px-4 py-2 text-xs text-content-secondary">
                    {b.last_movement_at ? (
                      <DateDisplay value={b.last_movement_at} />
                    ) : (
                      "—"
                    )}
                  </td>
...
function PriceComparisonModal({
  ...
  return (
    <WideModal
      open
      onClose={onClose}
      title={t("supplier_catalogs.price_comparison", {
    ...
    >
      <div>
        {q.isLoading ? (
          ...
          <EmptyState
            icon={<Boxes size={20} />}
            title={t("supplier_catalogs.no_prices", {
              defaultValue: "No vendor prices for this item",
            })}
            description={t("supplier_catalogs.no_prices_desc", {
              defaultValue:
                "Import a price list against a vendor or add a catalog entry.",
            })}
          />
        ) : (
          <div className="mt-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {rows.map((r) => {
              ...
              const isCheapest =
                cheapest &&
                cheapest.vendor_id === r.vendor_id &&
                rows.length > 1;
              return (
                <div
                  ...
                  className={clsx(
                    ...
                    isCheapest
                      ? "border-semantic-success ring-2 ring-semantic-success/30"
                      : "border-border-light",
                  )}
                >
                  ...
                  <div className="space-y-1.5">
                    <div>
                      <p className="text-xs uppercase tracking-wide text-content-tertiary">
                        {t("supplier_catalogs.unit_price", {
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <p className="uppercase tracking-wide text-content-tertiary">
                          {t("supplier_catalogs.lead_time", {
                            defaultValue: "Lead time",
                          })}
                        </p>
                        <p className="text-content-primary tabular-nums">
                          {r.lead_time_days}d
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-content-primary tabular-nums">
                          {String(r.min_order_qty)}
                        </p>
                      </div>
                    </div>
                    <div className="pt-1 border-t border-border-light">
                      <p className="text-xs uppercase tracking-wide text-content-tertiary">
                        {t("supplier_catalogs.rating", {
                          defaultValue: "Rating",
                        })}
                      </p>
  ...
  const submit = async () => {
    ...
    try {
      if (kind === "vendors") {
        ...
        addToast({
          type: "success",
          title: t("supplier_catalogs.vendor_created", {
        ...
        if (!itemForm.sku.trim() || !itemForm.name.trim())
          throw new Error("SKU and name required");
        ...
        addToast({
          type: "success",
          title: t("supplier_catalogs.item_created", {
        ...
        addToast({
          type: "success",
          title: t("supplier_catalogs.pr_created", {
            defaultValue: "Requisition created",
          }),
        });
      } else if (kind === "pos") {
        if (!poForm.vendor_id || !poForm.project_id.trim())
          throw new Error("Vendor and project required");
        if (!poForm.lineDesc.trim())
          throw new Error("At least one line required");
        await createPO({
        ...
        addToast({
          type: "success",
          title: t("supplier_catalogs.po_created", {
            defaultValue: "PO created",
          }),
        });
        ...
        addToast({
          type: "success",
          title: t("supplier_catalogs.warehouse_created", {
            defaultValue: "Warehouse created",
          }),
        });
  ...
  return (
    ...
    >
        ...
        <WideModalSection columns={2}>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setVendorForm({ ...vendorForm, country_code: e.target.value })
              }
              className={inputCls}
              maxLength={3}
          ...
          <WideModalField
            label={t("supplier_catalogs.legal_name", {
          ...
          >
            <input
              ...
              onChange={(e) =>
                setVendorForm({ ...vendorForm, legal_name: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setVendorForm({ ...vendorForm, currency: e.target.value })
              }
              className={inputCls}
              maxLength={3}
            />
          ...
          <WideModalField
            label={t("supplier_catalogs.payment_terms", {
              defaultValue: "Payment terms (days)",
            })}
          >
            <input
              type="number"
        ...
        <WideModalSection columns={2}>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setItemForm({ ...itemForm, unit_of_measure: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          ...
          <WideModalField
            label={t("supplier_catalogs.description_field", {
          ...
          <WideModalField
            label={t("supplier_catalogs.manufacturer", {
              defaultValue: "Manufacturer",
            })}
            span={2}
          >
      ...
      {kind === "prs" && (
        <>
          <WideModalSection
            title={t("supplier_catalogs.section_header", {
              defaultValue: "Requisition",
            })}
            columns={3}
          >
            <WideModalField
              label={t("supplier_catalogs.project", {
            ...
            >
              <select
                ...
                onChange={(e) =>
                  setPrForm({ ...prForm, project_id: e.target.value })
                }
                className={inputCls}
              >
                <option value="">
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setPrForm({ ...prForm, currency: e.target.value })
                }
                className={inputCls}
                maxLength={3}
              />
            ...
            <WideModalField
              label={t("supplier_catalogs.needed_by", {
                defaultValue: "Needed by",
              })}
              span={2}
            >
              <input
                ...
                onChange={(e) =>
                  setPrForm({ ...prForm, needed_by: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
          ...
          <WideModalSection
            title={t("supplier_catalogs.first_line", {
              defaultValue: "First line item",
            })}
            columns={3}
          >
            <WideModalField
              label={t("supplier_catalogs.line_description", {
                defaultValue: "Description",
              })}
              span={3}
            >
              <input
                ...
                onChange={(e) =>
                  setPrForm({ ...prForm, lineDesc: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
            <WideModalField
              label={t("supplier_catalogs.qty", { defaultValue: "Quantity" })}
            >
              <input
                type="number"
                value={prForm.lineQty}
                onChange={(e) =>
                  setPrForm({ ...prForm, lineQty: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
            <WideModalField
              label={t("supplier_catalogs.est_price", {
                defaultValue: "Est. price",
              })}
              span={2}
            >
              <input
                ...
                onChange={(e) =>
                  setPrForm({ ...prForm, linePrice: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
          ...
          >
            ...
            >
              <select
                ...
                onChange={(e) =>
                  setPoForm({ ...poForm, project_id: e.target.value })
                }
                className={inputCls}
              >
                <option value="">
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setPoForm({ ...poForm, currency: e.target.value })
                }
                className={inputCls}
                maxLength={3}
              />
            ...
            <WideModalField
              label={t("supplier_catalogs.expected_delivery", {
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setPoForm({ ...poForm, expected_delivery: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
          ...
          >
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setPoForm({ ...poForm, lineDesc: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setPoForm({ ...poForm, lineQty: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
            <WideModalField
              label={t("supplier_catalogs.unit_price", {
                defaultValue: "Unit price",
              })}
              span={2}
            >
              <input
                ...
                onChange={(e) =>
                  setPoForm({ ...poForm, linePrice: e.target.value })
                }
                className={inputCls}
              />
            </WideModalField>
