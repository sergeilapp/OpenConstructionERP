...
export function ProjectKPIHero({ projectId }: ProjectKPIHeroProps) {
  ...
  const varianceQ = useQuery({
    ...
    queryFn: () =>
      apiGet<VarianceResponse>(
        `/v1/costmodel/variance/?project_id=${projectId}`,
      ),
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      ...
      >
        ...
        <div className="flex items-baseline gap-2 mt-2">
          ...
          <span
            className={`text-2xl font-bold tabular-nums ${scheduleStyle.text}`}
          >
