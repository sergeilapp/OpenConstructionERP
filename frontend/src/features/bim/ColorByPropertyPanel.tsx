...
const PALETTES: {
  value: ColorByPropertyPalette;
  labelKey: string;
  defaultLabel: string;
}[] = [
  {
    value: "categorical-12",
    labelKey: "bim.color_palette_categorical",
    defaultLabel: "Categorical (12 colors)",
  },
  {
    value: "sequential-blue",
    labelKey: "bim.color_palette_seq_blue",
    defaultLabel: "Sequential — blue",
  },
  {
    value: "sequential-red-blue",
    labelKey: "bim.color_palette_diverging",
    defaultLabel: "Diverging — red ⇄ blue",
  },
  {
    value: "fire-rating",
    labelKey: "bim.color_palette_fire_rating",
    defaultLabel: "Fire rating (F30 / F60 / F90)",
  },
];
...
export default function ColorByPropertyPanel({
  ...
  const [palette, setPalette] =
    useState<ColorByPropertyPalette>("categorical-12");
    ...
    >
      ...
      <label className="flex flex-col gap-1 text-[11px] text-content-secondary">
        ...
        >
          {availableKeys.length === 0 ? (
            <option value="">
              {t("bim.color_no_properties", {
                defaultValue: "No properties available",
              })}
            </option>
          ) : (
            availableKeys.map((k) => (
      ...
      <div className="flex flex-col gap-1">
        ...
        {palette === "categorical-12" || palette === "fire-rating" ? (
          <CategoricalLegend distinct={distinct} config={previewConfig} />
        ) : (
          <GradientLegend config={previewConfig} />
        )}
      </div>
    </div>
  );
...
function CategoricalLegend({
  ...
  if (config.palette === "fire-rating") {
    const rows = Object.keys(FIRE_RATING_PALETTE).map((key) => ({
      ...
      count:
        distinct.find((d) => String(d.value).toLowerCase() === key)?.count ?? 0,
        ...
        top.map((row, i) => (
          ...
          >
            <span
              className="inline-block h-3 w-3 rounded-sm border border-border-light"
              style={{
                backgroundColor: colorForPropertyValue(config, row.value, i),
              }}
              aria-hidden="true"
            />
