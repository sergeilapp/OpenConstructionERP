  ...
  return (
    <div className="shrink-0 bg-surface-primary border-t border-border-light">
      ...
      <button
        ...
        aria-label={t("bim.toggle_models_filmstrip", {
          defaultValue: "Toggle models filmstrip‌⁠‍",
        })}
      ...
      >
        ...
        <span className="text-[11px] text-content-tertiary ml-1.5">
          ({models.length})
        </span>
      ...
      >
        <div className="flex items-center gap-3 px-4 pb-2 overflow-x-auto">
          {isLoading ? (
            <Loader2
              size={14}
              className="animate-spin text-content-quaternary"
            />
          ) : models.length ? (
            models.map((m) => (
              <ModelCard
            ...
            <span className="text-[11px] text-content-quaternary">
              {t("bim.no_models_yet", {
                defaultValue: "No models uploaded yet‌⁠‍",
              })}
            </span>
          )}
          {/* Add model button */}
          <button
            ...
            aria-label={t("bim.upload_model", {
              defaultValue: "Upload model‌⁠‍",
            })}
          >
            <Plus
              size={20}
...
function ModelCard({
  ...
  const isError =
    model.status === "error" || model.status === "needs_converter";
  ...
  return (
    ...
    >
      {/* Top accent */}
      <div
        className={`h-[3px] ${isActive ? "bg-oe-blue" : isError ? "bg-red-400" : isProcessing ? "bg-amber-400" : "bg-emerald-400"}`}
      />
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              {fmt && (
                <>
                  ...
                  <span className="text-[10px] text-content-quaternary font-mono">
                    {fmt}
                  </span>
                </>
              )}
...
function ConverterVersionBadge({
  ...
  const source =
    typeof metadata.converter_source === "string"
      ? metadata.converter_source
      : null;
  const tooltip = source
    ? t("bim.converter_version_tooltip", {
        defaultValue:
          "Processed with DDC converter v{{version}} (source: {{source}})",
        version,
        source,
      })
...
function UploadPanel({
  ...
  const [conversionDepth, setConversionDepth] = useState<
    "standard" | "medium" | "complete"
  >("standard");
  ...
  const handleFileSelect = useCallback(
    (f: File) => {
      ...
      if (DWG_EXTENSIONS.has(ext)) {
        addToast({
          type: "info",
          title: t("bim.dwg_redirect_title", {
            defaultValue: "DWG files are handled in the DWG Takeoff module",
          }),
          message: t("bim.dwg_redirect_msg", {
            defaultValue: "Redirecting to DWG Takeoff...",
          }),
        });
      ...
      if (!CAD_EXTENSIONS.has(ext) && !DATA_EXTENSIONS.has(ext)) {
        setUploadError(
          t("bim.upload_unsupported_format", {
            defaultValue:
              "Unsupported file format. Please upload .rvt or .ifc files.",
          }),
        );
        return;
      }
  ...
  const resetForm = useCallback(() => {
    ...
    setModelName("");
    setUploadError(null);
    [fileInputRef, dataInputRef, geoInputRef].forEach((r) => {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      if (advancedMode && dataFile) {
        ...
        addToast({
          type: "info",
          title: t("bim.upload_started_title", {
        ...
        if (isCADFile(file.name)) {
          ...
          const needsConverterMatch = (["rvt"] as const).find((c) =>
  ...
  const canUpload = advancedMode
    ? !!dataFile && !uploading
    : !!file && !uploading;
  const disciplines = [
    { v: "architecture", l: t("bim.disc_architecture") },
    { v: "structural", l: t("bim.disc_structural") },
    { v: "mechanical", l: t("bim.disc_mechanical") },
    { v: "electrical", l: t("bim.disc_electrical") },
    { v: "plumbing", l: t("bim.disc_plumbing") },
    { v: "fire_protection", l: t("bim.disc_fire") },
    { v: "civil", l: t("bim.disc_civil") },
    { v: "mixed", l: t("bim.disc_mixed") },
  ];
      ...
      <div className="absolute top-0 end-0 h-full w-[380px] bg-surface-primary/95 backdrop-blur-sm border-s border-border-light shadow-lg z-30 flex flex-col animate-in slide-in-from-right duration-200">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-content-primary">
                {t("bim.upload_panel_title")}
              </h2>
              <p className="text-[10px] text-content-quaternary">
                {t("bim.upload_panel_subtitle")}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {activeUploads.length > 0 && (
            <div className="space-y-2">
              {activeUploads.map((job) => (
                <div
                  key={job.id}
                  className={`rounded-xl border p-3 ${job.status === "ready" ? "border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/20" : "border-oe-blue/30 bg-oe-blue/5"}`}
                >
              ...
              {activeUploads.some((j) => j.status !== "ready") && (
                <p className="text-[10px] text-content-tertiary text-center py-1">
                  {t("bim.upload_continue_working", {
                    defaultValue:
                      "Processing in background — you can continue working or upload another file.",
                  })}
                </p>
              )}
            </div>
          ...
          {!advancedMode ? (
            <label
              ...
              role="button"
              tabIndex={0}
              aria-label={t("bim.upload_dropzone_aria", {
                defaultValue: "Upload BIM file (RVT, IFC, CSV, XLSX)",
              })}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  e.preventDefault();
            ...
            >
              {file ? (
                <>
                  <div className="w-10 h-10 rounded-xl bg-oe-blue/10 flex items-center justify-center">
                    <CheckCircle2 size={20} className="text-oe-blue" />
                  </div>
                  ...
                  <button
                    ...
                    className="text-[10px] text-content-tertiary hover:text-red-500 underline"
                  >
                    {t("bim.upload_remove_file")}
                  </button>
                </>
              ) : (
                <>
                  <div className="w-12 h-12 rounded-xl bg-surface-secondary border border-border-light flex items-center justify-center">
                    <FileUp size={22} className="text-content-quaternary" />
                  </div>
                  <p className="text-sm font-medium text-content-primary">
                    {t("bim.upload_drop_here")}
                  </p>
                  <p className="text-[10px] text-content-quaternary">
                    {t("bim.upload_size_hint")}
                  </p>
                  <div className="flex items-center gap-1.5 mt-1">
                    <span className="text-[9px] font-mono px-1 py-0.5 rounded bg-oe-blue/10 text-oe-blue border border-oe-blue/20">
                      .rvt
                    </span>
                    <span className="text-[9px] font-mono px-1 py-0.5 rounded bg-oe-blue/10 text-oe-blue border border-oe-blue/20">
                      .ifc
                    </span>
                    <span className="text-[9px] font-mono px-1 py-0.5 rounded bg-surface-tertiary text-content-quaternary">
                      .csv
                    </span>
                    <span className="text-[9px] font-mono px-1 py-0.5 rounded bg-surface-tertiary text-content-quaternary">
                      .xlsx
              ...
              <input
                id="bim-upload-file-input"
                ref={fileInputRef}
                type="file"
                accept=".rvt,.ifc,.csv,.xlsx,.xls"
                className="hidden"
                onChange={(e) => {
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="flex flex-col items-center gap-2 border-2 border-dashed border-border-medium rounded-xl p-4 text-center cursor-pointer hover:border-oe-blue/50 hover:bg-surface-secondary transition-all">
                ...
                <span className="text-[11px] font-medium text-content-primary">
                  {t("bim.upload_advanced_element_data")}
                </span>
                <span className="text-[9px] text-content-quaternary">
                  {t("bim.upload_advanced_element_data_hint")}
                </span>
                {dataFile && (
              ...
              <label className="flex flex-col items-center gap-2 border-2 border-dashed border-border-medium rounded-xl p-4 text-center cursor-pointer hover:border-oe-blue/50 hover:bg-surface-secondary transition-all">
                ...
                <span className="text-[11px] font-medium text-content-primary">
                  {t("bim.upload_advanced_geometry")}
                </span>
                <span className="text-[9px] text-content-quaternary">
                  {t("bim.upload_advanced_geometry_hint")}
                </span>
                {geometryFile && (
                  <Badge variant="blue" size="sm">
                    {geometryFile.name}
          ...
          <div>
            <label className="block text-[10px] font-semibold text-content-tertiary mb-1.5 uppercase tracking-wider">
              {t("bim.upload_model_name_label")}
            </label>
            <input
              type="text"
              className="w-full text-sm py-2 px-3 rounded-lg border border-border-light bg-surface-secondary text-content-primary placeholder-content-quaternary focus:outline-none focus:ring-1 focus:ring-oe-blue"
              placeholder={t("bim.upload_model_name_placeholder")}
              value={modelName}
          ...
          <div>
            <label className="block text-[10px] font-semibold text-content-tertiary mb-1.5 uppercase tracking-wider">
              {t("bim.upload_discipline_label")}
            </label>
            <select
              className="w-full text-sm py-2 px-3 rounded-lg border border-border-light bg-surface-secondary text-content-primary focus:outline-none focus:ring-1 focus:ring-oe-blue"
              value={discipline}
          ...
          {file && getFileExtension(file.name) === ".rvt" && (
            <>
              <div>
                <label className="block text-[10px] font-semibold text-content-tertiary mb-1.5 uppercase tracking-wider">
                  {t("bim.upload_depth_label", {
                    defaultValue: "Conversion depth",
                  })}
                </label>
                <select
                  className="w-full text-sm py-2 px-3 rounded-lg border border-border-light bg-surface-secondary text-content-primary focus:outline-none focus:ring-1 focus:ring-oe-blue"
                  ...
                  onChange={(e) =>
                    setConversionDepth(
                      e.target.value as "standard" | "medium" | "complete",
                    )
                  }
                >
                  <option value="standard">
                    {t("bim.upload_depth_standard", {
                      defaultValue: "Standard · main categories (fast)",
                    })}
                  </option>
                  <option value="medium">
                    {t("bim.upload_depth_medium", {
                      defaultValue: "Medium · extended categories (balanced)",
                    })}
                  </option>
                  <option value="complete">
                    {t("bim.upload_depth_complete", {
                      defaultValue: "Complete · all categories (slow)",
                    })}
                  </option>
                </select>
                <p className="mt-1 text-[10px] text-content-quaternary leading-relaxed">
                  {t("bim.upload_depth_help", {
                    defaultValue:
                      "Controls how many Revit categories are extracted. Element IDs and full properties are always preserved.",
                  })}
                </p>
              </div>
              <div>
                <label className="flex items-start gap-2 cursor-pointer group">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="block text-[11px] font-medium text-content-primary group-hover:text-oe-blue transition-colors">
                      {t("bim.upload_generate_pdf_label", {
                        defaultValue:
                          "Also export existing project sheets as PDF (background)",
                      })}
                    </span>
                    <span className="block text-[10px] text-content-quaternary leading-relaxed mt-0.5">
                      {t("bim.upload_generate_pdf_help", {
                        defaultValue:
          ...
          {uploading && (
            <div className="space-y-2">
              <div className="flex justify-between text-[11px]">
                <span className="text-content-secondary">{uploadStage}</span>
          ...
          >
            ...
            {advancedMode
              ? t("bim.upload_simple_mode_toggle")
              : t("bim.upload_advanced_mode_toggle")}
          </button>
        </div>
        ...
        <div className="px-5 py-4 border-t border-border-light">
          <button
            ...
            disabled={!canUpload}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all disabled:opacity-30 disabled:cursor-not-allowed bg-oe-blue text-white hover:bg-oe-blue-dark active:scale-[0.98] shadow-sm hover:shadow-md"
          >
            {uploading ? (
...
function NonReadyOverlay({
  ...
  if (!model) {
    return (
      <div
        className="flex flex-col items-center justify-center h-full bg-surface-secondary"
        role="status"
      >
        <div className="text-center max-w-sm px-6">
          ...
          <h2 className="text-lg font-bold text-content-primary mb-2">
            {t("bim.overlay_loading_model_title", {
              defaultValue: "Loading model…",
            })}
          </h2>
          <p className="text-sm text-content-secondary">
            {t("bim.overlay_loading_model_desc", {
              defaultValue:
                "Fetching the model record. This usually takes only a moment.",
            })}
          </p>
        </div>
      </div>
  ...
  const converterId =
    typeof meta.converter_id === "string" ? meta.converter_id : null;
  ...
  const handleInstall = async () => {
    ...
    try {
      await onInstallConverter(converterId);
    } finally {
  ...
  const showInstallButton =
    !isProcessing && errorCode === "ddc_not_found" && !!converterId;
  ...
  return (
    ...
    >
      <div className="text-center max-w-md px-6 w-full">
        <div
          className={`mx-auto w-20 h-20 rounded-2xl ${c.bg} border flex items-center justify-center mb-5`}
        >
          {c.icon}
        </div>
        <h2 className="text-lg font-bold text-content-primary mb-2">
        ...
        <p className="text-sm text-content-secondary mb-2 whitespace-pre-line">
          {description}
        </p>
        <p className="text-[11px] text-content-quaternary mb-6">
          {model.name}
          ...
          <div className="mx-auto max-w-xs mb-6">
            ...
            <div className="h-1.5 w-full rounded-full bg-blue-100 dark:bg-blue-900/30 overflow-hidden relative">
              <div
                ...
                style={{
                  animation: "oeBimIndeterminate 1.6s ease-in-out infinite",
                }}
              />
            </div>
            <p className="text-[11px] text-content-tertiary mt-2.5">
              {t("bim.overlay_processing_hint", {
                defaultValue:
                  "Backend is converting the file. This page will update automatically when ready — feel free to navigate away.",
              })}
            </p>
          </div>
        )}
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            {showInstallButton && (
              <button
                ...
                aria-label={t("bim.overlay_install_converter_btn", {
              ...
              >
                ...
                {isInstalling
                  ? t("bim.overlay_install_in_progress", {
            ...
            {showRetryButton && (
              <button
                onClick={handleRetry}
                disabled={isRetrying}
                aria-label={t("bim.overlay_retry_btn", {
                  defaultValue: "Retry conversion",
                })}
              ...
              >
                ...
                {isRetrying
                  ? t("bim.overlay_retry_in_progress", {
                      defaultValue: "Retrying…",
                    })
                  : t("bim.overlay_retry_btn", {
                      defaultValue: "Retry conversion",
                    })}
              </button>
            )}
            <button
              onClick={onUploadConverted}
              aria-label={t("bim.overlay_upload_converted_btn")}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-surface-primary border border-border-light text-content-secondary text-sm font-medium hover:bg-surface-secondary transition-colors"
            >
              <UploadCloud size={15} /> {t("bim.overlay_upload_converted_btn")}
            </button>
            <button
              onClick={onDelete}
              aria-label={t("bim.overlay_delete_btn")}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-surface-primary border border-border-light text-content-secondary text-sm font-medium hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
            >
...
function LandingPage({
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      const name = modelName || file.name.replace(/\.[^.]+$/, "");
      const uploadType = isCADFile(file.name)
        ? ("cad" as const)
        : ("data" as const);
      startGlobalUpload({
        file,
  ...
  const features = [
    {
      ...
      ic: "text-blue-500",
      title: t("bim.landing_feat_3d_title"),
      desc: t("bim.landing_feat_3d_desc"),
    },
    {
      icon: Layers,
      color:
      ...
      ic: "text-emerald-500",
      title: t("bim.landing_feat_extract_title"),
      desc: t("bim.landing_feat_extract_desc"),
    },
    {
      icon: Link2,
      color:
      ...
      ic: "text-violet-500",
      title: t("bim.landing_feat_boq_title"),
      desc: t("bim.landing_feat_boq_desc"),
    },
    {
      icon: Ruler,
      color:
      ...
      ic: "text-orange-500",
      title: t("bim.landing_feat_qty_title"),
      desc: t("bim.landing_feat_qty_desc"),
    },
    {
      icon: Building2,
      color:
      ...
      ic: "text-pink-500",
      title: t("bim.landing_feat_compare_title"),
      desc: t("bim.landing_feat_compare_desc"),
    },
    {
      icon: Globe2,
      color:
      ...
      ic: "text-cyan-500",
      title: t("bim.landing_feat_format_title"),
      desc: t("bim.landing_feat_format_desc"),
    },
  ];
        ...
        >
          <defs>
            ...
            <symbol id="bimIsoCube" viewBox="-100 -120 200 240">
              <polygon
                points="0,-100 90,-50 0,0 -90,-50"
                fill="url(#bimCubeFadeTop)"
                stroke="currentColor"
                strokeWidth="0.5"
                strokeOpacity="0.12"
              />
              <polygon
                points="-90,-50 0,0 0,100 -90,50"
                fill="url(#bimCubeFadeLeft)"
                stroke="currentColor"
                strokeWidth="0.5"
                strokeOpacity="0.12"
              />
            ...
            >
              <g transform="translate(90 120) scale(0.32)">
                <use href="#bimIsoCube" />
              </g>
              <g transform="translate(270 90) scale(0.28)">
                <use href="#bimIsoCube" />
              </g>
              <g transform="translate(430 160) scale(0.3)">
                <use href="#bimIsoCube" />
              </g>
              <g transform="translate(180 270) scale(0.36)">
                <use href="#bimIsoCube" />
              </g>
              <g transform="translate(380 330) scale(0.26)">
                <use href="#bimIsoCube" />
              </g>
              <g
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch mb-8">
              ...
              <div className="flex flex-col">
                <div className="rounded-2xl bg-white dark:bg-gray-800/60 border border-border-light shadow-lg shadow-black/5 dark:shadow-black/20 p-6 flex flex-col h-full">
                  <label
                    aria-label={t("bim.landing_dropzone_aria", {
                      defaultValue:
                        "Drop a BIM model file here or click to browse. Supported formats: .rvt, .ifc, .csv, .xlsx",
                    })}
                    onDrop={(e) => {
                      e.preventDefault();
                      const f = e.dataTransfer.files?.[0];
                      if (f) {
                        ...
                        if (DWG_EXTENSIONS.has(ext)) {
                          addToast({
                            ...
                            title: t("bim.dwg_redirect_title", {
                              defaultValue:
                                "DWG files are handled in the DWG Takeoff module",
                            }),
                            message: t("bim.dwg_redirect_msg", {
                              defaultValue: "Redirecting to DWG Takeoff...",
                            }),
                        ...
                        if (
                          !CAD_EXTENSIONS.has(ext) &&
                          !DATA_EXTENSIONS.has(ext)
                        ) {
                          addToast({
                            type: "error",
                            title: t("bim.upload_unsupported_format", {
                              defaultValue:
                                "Unsupported file format. Please upload .rvt or .ifc files.",
                            }),
                          });
                    ...
                    {file ? (
                      <>
                        <div className="w-14 h-14 rounded-2xl bg-oe-blue/10 flex items-center justify-center">
                          <CheckCircle2 size={26} className="text-oe-blue" />
                        </div>
                      ...
                      <>
                        ...
                        <div>
                          <p className="text-sm font-semibold text-content-primary">
                            {t("bim.landing_drop_here")}
                          </p>
                          <p className="text-xs text-content-tertiary mt-1">
                            {t("bim.landing_size_hint")}
                          </p>
                        </div>
                  ...
                  {file && (
                    <div className="mt-4 space-y-3">
                      <input
                        type="text"
                        className="w-full text-sm py-2.5 px-4 rounded-xl border border-border-light bg-surface-secondary text-content-primary placeholder-content-quaternary focus:outline-none focus:ring-2 focus:ring-oe-blue/30"
                        placeholder={t("bim.model_name")}
                        value={modelName}
                        onChange={(e) => setModelName(e.target.value)}
                      />
                        ...
                        <>
                          <div>
                            ...
                            >
                              <option value="standard">
                                {t("bim.upload_depth_standard", {
                                  defaultValue:
                                    "Standard · main categories (fast)",
                                })}
                              </option>
                              <option value="medium">
                                {t("bim.upload_depth_medium", {
                                  defaultValue:
                                    "Medium · extended categories (balanced)",
                                })}
                              </option>
                              <option value="complete">
                                {t("bim.upload_depth_complete", {
                                  defaultValue:
                                    "Complete · all categories (slow)",
                                })}
                              </option>
                            </select>
                            <p className="mt-1 text-[10px] text-content-quaternary leading-relaxed">
                          ...
                          <label className="flex items-start gap-2 cursor-pointer group">
                            <input
                              ...
                              checked={generatePdfSheets}
                              onChange={(e) =>
                                setGeneratePdfSheets(e.target.checked)
                              }
                              className="mt-0.5 h-4 w-4 rounded border-border-medium text-oe-blue focus:ring-1 focus:ring-oe-blue cursor-pointer"
                            />
                            <div className="flex-1 min-w-0">
                      ...
                      >
                        ...
                        {uploading
                          ? t("bim.landing_processing")
                          : t("bim.landing_upload_process")}
                      </button>
                    </div>
                  )}
                </div>
                ...
                {activeUploads.length > 0 && (
                  <div className="mt-3 space-y-2">
                    {activeUploads.map((job) => (
                      <div
                        key={job.id}
                        className={`rounded-xl border p-3.5 ${job.status === "ready" ? "border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/20" : "border-oe-blue/30 bg-white dark:bg-gray-900 shadow-sm"}`}
                      >
                        <div className="flex items-center gap-2 mb-1.5">
                          {job.status === "ready" ? (
                            <CheckCircle2
              ...
              <div className="flex flex-col justify-center gap-4">
                <div>
                  ...
                  <p className="text-xs text-content-tertiary mt-3 leading-relaxed">
                    {t("bim.landing_formats_detailed", {
                      defaultValue:
                        "Revit 2015\u20132026 (.rvt) \u00B7 IFC 2x3, 4.0, 4.1, 4.3 (.ifc) \u00B7 CSV \u00B7 Excel. DWG \u2192 DWG Takeoff.",
                    })}
                  </p>
              ...
              <div className="hidden">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {(landingModels ?? []).map((m) => {
                    ...
                    const isError =
                      m.status === "error" || m.status === "needs_converter";
                    ...
                    const statusLabel = isReady
                      ...
                      : m.status === "needs_converter"
                        ? t("bim.status_needs_converter", {
                            defaultValue: "Needs Converter",
                          })
                        : isProcessing
                          ? t("bim.status_processing", {
                    ...
                    const timeAgo = m.created_at
                      ? (() => {
                          ...
                          if (mins < 1)
                            return t("bim.just_now", {
                              defaultValue: "just now",
                            });
                          if (mins < 60)
                            return t("bim.time_mins_ago", {
                              defaultValue: "{{count}}m ago",
                              count: mins,
                            });
                          const hrs = Math.floor(mins / 60);
                          if (hrs < 24)
                            return t("bim.time_hours_ago", {
                              defaultValue: "{{count}}h ago",
                              count: hrs,
                            });
                          const days = Math.floor(hrs / 24);
                          return t("bim.time_days_ago", {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <p
                              className="text-sm font-semibold text-content-primary truncate pe-6"
                              title={m.name}
                            >
                              {m.name}
                            </p>
              ...
              >
                ...
                <div className="px-3 py-2.5">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    {fmt && (
                      <span className="text-[9px] font-mono font-bold px-1 py-0.5 rounded bg-oe-blue/8 text-oe-blue border border-oe-blue/15 shrink-0">
                        .{fmt.toLowerCase()}
                      </span>
                    )}
                  </div>
...
export function BIMPage() {
  ...
  const queryClient = useQueryClient();
  const navigate = useNavigate();
  const { projectId: urlProjectId, modelId: urlModelId } = useParams<{
    projectId?: string;
    modelId?: string;
  }>();
  ...
  const projectId =
    urlProjectId || contextProjectId || projectsList[0]?.id || "";
  ...
  const setActiveProjectInStore = useProjectContextStore(
  ...
  const [uploadConvertedName, setUploadConvertedName] = useState<string | null>(
    null,
  );
  const [showUploadOverride, setShowUploadOverride] = useState<boolean | null>(
    null,
  );
  ...
  >(null);
  const [visibleElementCount, setVisibleElementCount] = useState<number | null>(
    null,
  );
  ...
  const [linkCandidates, setLinkCandidates] = useState<BIMElementData[] | null>(
    null,
  );
  ...
  const [createTaskFor, setCreateTaskFor] = useState<BIMElementData[] | null>(
    null,
  );
  ...
  const highlightedBIMElementIds = useBIMLinkSelectionStore(
    (s) => s.highlightedBIMElementIds,
  );
  ...
  const showFullPageUpload =
    showUploadOverride !== null ? showUploadOverride : !hasModels;

  useEffect(() => {
    if (hasModels && showUploadOverride === false) setShowUploadOverride(null);
  ...
  useEffect(() => {
    ...
    const targetName = deepLinkDocName
      ? decodeURIComponent(deepLinkDocName).toLowerCase()
      : "";
    ...
    if (match) {
      ...
      setUploadOpen(true);
      if (deepLinkDocName)
        setUploadConvertedName(decodeURIComponent(deepLinkDocName));
    }
  ...
  useEffect(() => {
    ...
    fetchBIMModel(urlModelId)
      ...
      .catch(() => {
        ...
        addToast({
          type: "warning",
          title: t("bim.model_not_found_title", {
            defaultValue: "Model not found",
          }),
  ...
  useEffect(() => {
    ...
    const target =
      urlModelId && models.find((m) => m.id === urlModelId)
        ? urlModelId
        : models[0]!.id;
  ...
  const geometryUrl = useMemo(() => {
    if (
      ...
      ((activeModel?.element_count ?? 0) === 0 &&
        !elements.some((el) => !!el.mesh_ref))
    ) {
      return null;
    }
  ...
  const handleSmartFilter = useCallback(
    (
      ...
      const labels: Record<typeof filterId, string> = {
        errors: t("bim.smart_filter_errors", {
          defaultValue: "Validation errors",
        }),
        warnings: t("bim.smart_filter_warnings", {
          defaultValue: "Validation warnings",
        }),
        unlinked_boq: t("bim.smart_filter_unlinked_boq", {
          defaultValue: "Unlinked to BOQ",
        }),
        has_tasks: t("bim.smart_filter_has_tasks", {
          defaultValue: "With tasks",
        }),
        has_docs: t("bim.smart_filter_has_docs", {
  ...
  const handleDeleteGroup = useCallback(
    async (group: BIMElementGroup) => {
      const ok = await confirm({
        title: t("bim.group_delete_confirm_title", {
      ...
      try {
        ...
        addToast({
          type: "success",
          title: t("bim.group_deleted_title", {
  ...
  const handleIsolateGroup = useCallback((group: BIMElementGroup) => {
    const ids = Array.isArray(group.member_element_ids)
  ...
  const handleHighlightGroup = useCallback(
    (group: BIMElementGroup | null) => {
      if (group) {
        setBIMSelection(
          Array.isArray(group.member_element_ids)
            ? group.member_element_ids
            : [],
        );
      } else {
    ...
    async (linkId: string) => {
      try {
        ...
        addToast({
          ...
          message: t("bim.link_removed", {
            defaultValue: "BIM ↔ BOQ link removed",
          }),
        });
        queryClient.invalidateQueries({
          queryKey: ["bim-elements", activeModelId],
  ...
  const handleQuickTakeoff = useCallback(() => {
    ...
    if (subset.length === 0) {
      addToast({
        type: "info",
        title: t("bim.quick_takeoff_empty_title", {
          defaultValue: "Nothing to link",
        }),
  ...
  const handleDeleteModel = useCallback(
    async (modelId: string, name: string) => {
      const ok = await confirm({
        title: t("bim.confirm_delete_model_title", {
          defaultValue: "Delete model?",
        }),
      ...
      try {
        ...
        addToast({
          type: "success",
          title: t("bim.toast_model_deleted_title"),
          message: name,
        });
        ...
        addToast({
          type: "error",
          title: t("bim.toast_delete_failed_title"),
          message: err instanceof Error ? err.message : String(err),
        });
      }
    },
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    if (projectId && contextProjectName)
      items.push({ label: contextProjectName, to: `/projects/${projectId}` });
  ...
  if (!projectId) {
    return (
      <div
        className="flex items-center justify-center -mx-4 sm:-mx-7 -mt-6 -mb-6 border-s border-border-light"
        style={{ height: "calc(100vh - 56px)" }}
      >
        <EmptyState
  ...
  const isModelNonReady =
    ...
    !!(
      activeModel &&
      ["processing", "needs_converter", "error"].includes(activeModel.status)
    );
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-sm font-bold text-content-primary">
                {t("bim.viewer_title", { defaultValue: "BIM Viewer" })}
              </h1>
              {activeModel && (
                <p
                  ...
                  title={(() => {
                    ...
                    const lines = [
                      activeModel.name,
                      "",
                      t("bim.disk_root", { defaultValue: "Storage root" }) +
                        `: ${data.storage_root_label}`,
                      t("bim.disk_models", { defaultValue: "Models" }) +
                        `: ${models.length}`,
                      t("bim.disk_artifacts", {
                        defaultValue: "Conversion artifacts",
                      }) + `: ${fmtMb(artifactMb)}`,
                      hasOriginals
                        ? t("bim.disk_originals_kept", {
                            defaultValue: "Originals (kept)",
                          }) + `: ${fmtMb(originalMb)}`
                        : t("bim.disk_originals_dropped", {
                            defaultValue:
                              "Originals dropped after conversion (production policy)",
                          }),
                      t("bim.disk_total", { defaultValue: "Total on disk" }) +
                        `: ${fmtMb(artifactMb + originalMb)}`,
                    ];
                    return lines.join("\n");
                  })()}
          ...
          {elements.length > 0 && (
            <div className="hidden md:flex items-center gap-2 ms-2">
              <StatPill
                icon={Box}
                label={t("bim.stat_elements", { defaultValue: "Elements" })}
                value={elements.length}
              />
              {storeys.size > 0 && (
                <StatPill
                  icon={Layers}
                  label={t("bim.stat_storeys", { defaultValue: "Levels" })}
                  value={storeys.size}
                />
              )}
              {discips.size > 0 && (
          ...
          {elements.length > 0 && (
            <>
              <button
                ...
                title={t("bim.filter_toggle", {
                  defaultValue: "Toggle filter panel",
                })}
                aria-label={t("bim.filter_toggle", {
                  defaultValue: "Toggle filter panel",
                })}
              ...
              <button
                ...
                title={
                  summaryPanelOpen
                    ? t("bim.summary_hide", {
                        defaultValue: "Hide summary panel",
                      })
                    : t("bim.summary_show", {
                        defaultValue: "Show summary panel",
                      })
                }
                aria-label={t("bim.summary_toggle", {
                  defaultValue: "Toggle summary panel",
                })}
                ...
                title={
                  dimensionsVisible
                    ? t("bim.dimensions_hide", {
                        defaultValue:
                          "Hide bounding-box dimensions on selection",
                      })
                    : t("bim.dimensions_show", {
                        defaultValue:
                          "Show bounding-box dimensions on selection",
                      })
                }
              ...
              >
                <Maximize2 size={13} />
                {t("bim.dimensions_button", {
                  defaultValue: "BBox Dimensions",
                })}
              </button>
              ...
              <button
                ...
                title={t("bim.linked_boq_toggle", {
                  defaultValue: "Toggle linked BOQ panel",
                })}
                aria-label={t("bim.linked_boq_toggle", {
                  defaultValue: "Toggle linked BOQ panel",
                })}
              ...
              >
                <optgroup
                  label={t("bim.color_group_field", {
                    defaultValue: "By field",
                  })}
                >
                  <option value="default">
                    {t("bim.color_default", { defaultValue: "Default" })}
                  </option>
                  <option value="storey">
                    {t("bim.color_storey", { defaultValue: "Storey" })}
                  </option>
                  <option value="type">
                    {t("bim.color_type", { defaultValue: "Category" })}
                  </option>
                </optgroup>
                <optgroup
                  label={t("bim.color_group_status", {
                    defaultValue: "By compliance",
                  })}
                >
                  <option value="validation">
                    {t("bim.color_validation", {
                      defaultValue: "Validation status",
                    })}
                  </option>
                  <option value="boq_coverage">
                    {t("bim.color_boq_coverage", {
                      defaultValue: "BOQ link coverage",
                    })}
                  </option>
                  <option value="document_coverage">
                    {t("bim.color_doc_coverage", {
                      defaultValue: "Document coverage",
                    })}
                  </option>
                </optgroup>
                <optgroup
                  label={t("bim.color_group_cost", { defaultValue: "By cost" })}
                >
                ...
                <optgroup
                  label={t("bim.color_group_schedule", {
                    defaultValue: "By schedule",
                  })}
                >
                  <option value="4d_schedule">
                    {t("bim.color_4d_schedule", {
                      defaultValue: "4D timeline",
                    })}
                  </option>
                </optgroup>
              </select>
              ...
              {selectedElementId && (
                <button
                  ...
                  title={t("bim.isolate_selection", {
                    defaultValue: "Isolate selection",
                  })}
                >
                  {isolatedIds
              ...
              >
                ...
                {t("bim.rules_button", { defaultValue: "Rules" })}
              </a>
            </>
          )}
          <button
        ...
        <div className="flex-1 min-w-0 relative">
          ...
          {dimensionsVisible && selectedDimensions && (
            <div
              ...
              style={{
                insetInlineStart:
                  filterPanelOpen && elements.length > 0 ? 332 : 12,
              }}
            ...
            >
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-wide text-content-tertiary">
                  {t("bim.dimensions_title", {
                    defaultValue: "BBox dimensions",
                  })}
                </span>
              </div>
              <div
                className="text-[11px] font-medium text-content-primary truncate mb-1.5"
                title={selectedDimensions.name}
              >
              ...
              <div className="grid grid-cols-3 gap-2 text-[11px] tabular-nums">
                <div>
                  <div className="text-[9px] uppercase text-content-tertiary">
                    L
                  </div>
                  <div className="font-semibold text-content-primary">
                    {selectedDimensions.L.toFixed(2)}
                    <span className="text-[9px] text-content-tertiary ml-0.5">
                      m
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="font-semibold text-content-primary">
                    {selectedDimensions.W.toFixed(2)}
                    <span className="text-[9px] text-content-tertiary ml-0.5">
                      m
                    </span>
                  </div>
                ...
                <div>
                  ...
                  <div className="font-semibold text-content-primary">
                    {selectedDimensions.H.toFixed(2)}
                    <span className="text-[9px] text-content-tertiary ml-0.5">
                      m
                    </span>
                  </div>
              ...
              <div className="mt-1.5 pt-1.5 border-t border-border-light text-[10px] text-content-tertiary flex items-center justify-between">
                <span>
                  {t("bim.bbox_volume", { defaultValue: "BBox volume" })}
                </span>
          ...
          {(() => {
            ...
            for (const j of globalUploadJobs.values()) {
              if (
                j.modelId === activeModelId &&
                (j.pdfStatus === "generating" || j.pdfStatus === "failed")
              ) {
                pdfJob = j;
                break;
              }
              ...
              <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 pointer-events-none">
                ...
                >
                  ...
                  <span className="text-xs font-medium whitespace-nowrap">
                    {failed
                      ? t("bim.pdf_status_failed", {
                          defaultValue: "PDF sheet export failed",
                        })
                      : t("bim.pdf_status_generating", {
                          defaultValue: "Generating PDF sheets in background…",
                        })}
                  </span>
          ...
          {isModelNonReady ? (
            <NonReadyOverlay
              ...
              onDelete={() => {
                if (activeModel)
                  handleDeleteModel(activeModel.id, activeModel.name);
              }}
              onRetry={async () => {
                ...
                try {
                  ...
                  addToast({
                    type: "success",
                    title: t("bim.retry_scheduled_title", {
                      defaultValue: "Retry scheduled",
                    }),
                    message: t("bim.retry_scheduled_desc", {
                      defaultValue:
                        "The model is being re-processed. This page will update when done.",
                    }),
                  });
                  queryClient.invalidateQueries({
                    queryKey: ["bim-models", projectId],
                  ...
                  queryClient.invalidateQueries({
                    queryKey: ["bim-model", activeModel.id],
                  });
                } catch (e) {
                  addToast({
                    type: "error",
                    title: t("bim.retry_failed_title", {
              ...
              onInstallConverter={async (converterId) => {
                try {
                  ...
                  if (r.installed) {
                    addToast({
                      type: "success",
                      title: t("bim.converter_installed_title", {
                        defaultValue: "Converter installed",
                      }),
                      message:
                        r.message ||
                        t("bim.converter_installed_desc", {
                          defaultValue: "Retrying conversion now…",
                        }),
                    });
                    ...
                    addToast({
                      ...
                      message:
                        r.message ||
                        t("bim.converter_install_problem_desc", {
                          defaultValue:
                            "Install finished but the binary did not pass the smoke test.",
                        }),
                    });
                  }
                } catch (e) {
                  addToast({
                    type: "error",
                    title: t("bim.converter_install_failed_title", {
                      defaultValue: "Converter install failed",
                    }),
                    message: e instanceof Error ? e.message : String(e),
                  });
                }
            ...
            <>
              <BIMViewer
                ...
                highlightedIds={
                  highlightedBIMElementIds.length > 0
                ...
                error={
                  elementsQuery.error
                    ? t("bim.error_load_elements", {
                        defaultValue:
                          "Failed to load model elements. Check the server connection.",
                      })
                    : null
                }
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-4 py-2.5 rounded-xl bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-border-light shadow-lg text-sm">
                  ...
                  <span className="text-content-secondary">
                    {t("bim.group_subset_info", {
                      defaultValue:
                        'Showing {{count}} elements from group "{{name}}"',
                      count: elementsTotal,
                  ...
                  >
                    ...
                    {t("bim.load_full_model", {
                      ...
                      total:
                        activeModel?.element_count?.toLocaleString() ?? "...",
                    })}
                  </button>
                </div>
              )}
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <Cuboid
                  size={40}
                  className="text-content-quaternary mx-auto mb-3"
                />
          ...
          {false && meshMatchRatio !== null && <div className="hidden"></div>}
        </div>
