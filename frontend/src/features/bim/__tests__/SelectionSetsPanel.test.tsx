...
describe("SelectionSetsPanel", () => {
  ...
  it("renders empty state with no-selection hint", () => {
    ...
    render(
      <SelectionSetsPanel modelId={MODEL_ID} selectionManager={manager} />,
    );
  ...
  it("Restore button calls selectByIds with exclusive=true", () => {
    ...
    fireEvent.click(
      screen.getByTestId(`bim-selection-set-restore-${stored.id}`),
    );
  ...
  it("Add button calls selectByIds with exclusive=false", () => {
    ...
    expect(selectByIds).toHaveBeenCalledWith(["w1", "w2"], {
      exclusive: false,
    });
  });
  ...
  it("Delete row shows confirm, then commit removes the set", () => {
    ...
    fireEvent.click(
      screen.getByTestId(`bim-selection-set-delete-${stored.id}`),
    );
    const confirm = screen.getByTestId(
      `bim-selection-set-delete-confirm-${stored.id}`,
    );
  ...
  it("Rename inline: double-click name → input → Enter commits store.update", () => {
    ...
    const input = screen.getByTestId(
      `bim-selection-set-rename-input-${stored.id}`,
    );
  ...
  it("Rename Esc cancels without mutating", () => {
    ...
    fireEvent.doubleClick(
      screen.getByTestId(`bim-selection-set-name-${stored.id}`),
    );
    const input = screen.getByTestId(
      `bim-selection-set-rename-input-${stored.id}`,
    );
  ...
  it("Update button overwrites the set with the current selection", () => {
    ...
    const stored = selectionSetsStore.list(MODEL_ID)[0]!;
    fireEvent.click(
      screen.getByTestId(`bim-selection-set-update-${stored.id}`),
    );
