...
export default function BIMRequirementsImport() {
  ...
  const [importResult, setImportResult] =
    useState<BIMRequirementImportResult | null>(null);
  ...
  const { data: sets = [], isLoading: setsLoading } = useQuery({
    ...
    queryFn: () =>
      projectId ? fetchBIMRequirementSets(projectId) : Promise.resolve([]),
    enabled: !!projectId,
  });

  // Import mutation
  const importMutation = useMutation({
    mutationFn: (file: File) => importBIMRequirements(projectId!, file),
    onSuccess: (result) => {
      setImportResult(result);
      addToast({
        type: "success",
        title: t("bim.requirements.importSuccessTitle", {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      addToast({
        type: "success",
        title: t("bim.requirements.deletedTitle", {
          defaultValue: "Deleted‌⁠‍",
        }),
        message: t("bim.requirements.deleted", {
          defaultValue: "Requirement set deleted‌⁠‍",
        }),
      });
      queryClient.invalidateQueries({ queryKey: ["bim-requirement-sets"] });
    },
    ...
    <div className="space-y-6">
      ...
      <div>
        <h3 className="text-sm font-semibold mb-2 text-zinc-700 dark:text-zinc-200">
          {t("bim.requirements.importTitle", {
            defaultValue: "Import BIM Requirements‌⁠‍",
          })}
        </h3>
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-xs text-zinc-400">
                {t("bim.requirements.formats", {
                  defaultValue:
                    "IDS XML, COBie Excel, Excel/CSV, Revit SP (.txt), BIMQ JSON",
                })}
              </span>
            </div>
          )}
        ...
        <div className="mt-2 flex gap-2">
          ...
          >
            ...
            {t("bim.requirements.downloadTemplate", {
      ...
      <div>
        <h3 className="text-sm font-semibold mb-2 text-zinc-700 dark:text-zinc-200">
          {t("bim.requirements.importedSets", {
            defaultValue: "Imported Requirement Sets",
          })}
          {sets.length > 0 && (
        ...
        {setsLoading ? (
          ...
          <p className="text-sm text-zinc-400">
            {t("bim.requirements.noSets", {
              defaultValue: "No requirement sets imported yet.",
            })}
          </p>
        ) : (
          <div className="space-y-2">
            {sets.map((s) => (
              <RequirementSetCard
                ...
                onToggle={() =>
                  setExpandedSet(expandedSet === s.id ? null : s.id)
                }
      ...
      {result.errors.length > 0 && (
        <div className="mt-2 text-xs text-red-700 dark:text-red-300">
          <div className="flex items-center gap-1 font-medium">
            ...
            {t("bim.requirements.errors", { defaultValue: "error(s)" })}
          </div>
          {result.errors.slice(0, 3).map((e, i) => (
            <div
              key={`err-${(e.msg ?? "").slice(0, 30)}-${i}`}
              className="ml-5 mt-0.5"
            >
              {e.msg}
            </div>
      ...
      >
        ...
        >
          <a
            ...
            title={t("bim.requirements.exportExcel", {
          ...
          <a
            ...
            title={t("bim.requirements.exportIDS", {
              defaultValue: "Export as IDS XML",
            })}
          >
  ...
  if (!data || data.requirements.length === 0) {
    return (
      <div className="p-3 border-t border-zinc-100 dark:border-zinc-700 text-sm text-zinc-400">
        {t("bim.requirements.empty", {
          defaultValue: "No requirements in this set.",
        })}
      </div>
    );
  }
      ...
      <table className="w-full text-xs">
        <thead className="bg-zinc-50 dark:bg-zinc-700/50">
          <tr>
            ...
            <th className="px-3 py-1.5 text-left font-medium text-zinc-500">
              {t("bim.requirements.col.constraint", {
          ...
          {data.requirements.slice(0, 50).map((req) => (
            ...
            >
              ...
              <td className="px-3 py-1.5 text-zinc-500 dark:text-zinc-400">
                {formatConstraint(
                  req.constraint_def as Record<string, unknown>,
                )}
              </td>
            </tr>
          ))}
