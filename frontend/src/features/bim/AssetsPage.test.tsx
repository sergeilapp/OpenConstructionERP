...
vi.mock("./api", () => ({
  ...
  cobieExportUrl: (modelId: string) =>
    `/api/v1/bim_hub/models/${modelId}/export/cobie.xlsx/`,
}));
...
describe("AssetsPage", () => {
  ...
  it("renders a table of tracked assets for the active project", async () => {
    useProjectContextStore
      .getState()
      .setActiveProject("proj-1", "Riverside HQ");
    (listTrackedAssets as any).mockResolvedValue({
      items: [sampleAsset],
      total: 1,
    });
    ...
    await waitFor(() =>
      expect(listTrackedAssets).toHaveBeenCalledWith(
        "proj-1",
        expect.any(Object),
      ),
    );
  ...
  it("renders an empty state when the project has no tracked assets", async () => {
    ...
    renderWithProviders();
    expect(
      await screen.findByText(/No tracked assets yet/i),
    ).toBeInTheDocument();
  });
