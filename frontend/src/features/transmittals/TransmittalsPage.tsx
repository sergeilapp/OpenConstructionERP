...
function CreateTransmittalModal({
  ...
  const addRevision = (rev: CDERevision) => {
    ...
    const container = containers.find(
      (c: CDEContainer) => c.id === pickerContainerId,
    );
    if (!container) return;
    setForm((prev) => ({
      ...prev,
  ...
  const set = <K extends keyof TransmittalFormData>(
    key: K,
    value: TransmittalFormData[K],
  ...
  return (
    <WideModal
      ...
      busy={isPending}
      size="xl"
      title={t("transmittals.new_transmittal", {
      ...
      footer={
        <>
          ...
          >
            ...
            <span>
              {t("transmittals.create_transmittal", {
                defaultValue: "Create Transmittal",
              })}
            </span>
          </Button>
        </>
    ...
    >
      <WideModalSection columns={2}>
        <WideModalField
          label={t("transmittals.field_subject", {
          ...
          error={
            subjectError
              ? t("transmittals.subject_required", {
        ...
        >
          <div className="relative">
            <select
              ...
              value={form.purpose}
              onChange={(e) =>
                set("purpose", e.target.value as TransmittalPurpose)
              }
              className={inputCls + " appearance-none pr-9"}
            >
              {(Object.keys(PURPOSE_LABELS) as TransmittalPurpose[]).map(
                (p) => (
                  <option key={p} value={p}>
                    {t(`transmittals.purpose_${p}`, {
                      defaultValue: PURPOSE_LABELS[p],
                    })}
                  </option>
                ),
              )}
        ...
        <WideModalField
          label={t("transmittals.field_response_due", {
            defaultValue: "Response Due",
          })}
        ...
        <WideModalField
          label={t("transmittals.field_recipients", {
        ...
        <WideModalField
          label={t("transmittals.field_items", {
            defaultValue: "Document Items",
          })}
          htmlFor="tr-items"
        >
          <input
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="relative">
              <select
                aria-label={t("transmittals.picker_container_label", {
              ...
              >
                <option value="">
                  {t("transmittals.picker_select_container", {
            ...
            <div className="relative">
              <select
                aria-label={t("transmittals.picker_revision_label", {
              ...
              >
                <option value="">
                  {t("transmittals.picker_select_revision", {
        ...
        <WideModalField
          label={t("transmittals.field_cover_note", {
            defaultValue: "Cover Note",
          })}
          span={2}
...
const TransmittalRow = React.memo(function TransmittalRow({
  ...
  const purposeCls =
    PURPOSE_COLORS[transmittal.purpose] ?? PURPOSE_COLORS.for_information;

  const acknowledgedCount = transmittal.recipients.filter(
    (r) => r.acknowledged,
  ).length;
  const isOverdue =
  ...
  return (
    <div className="border-b border-border-light last:border-b-0">
      ...
      <div
        ...
        aria-label={t("transmittals.toggle_row", {
          defaultValue: "Toggle details for {{num}}",
          num: transmittal.transmittal_number,
        })}
      ...
      >
        ...
        <Badge
          variant="neutral"
          size="sm"
          className={clsx(purposeCls, "hidden md:inline-flex")}
        >
          ...
          {transmittal.cover_note && (
            <div className="rounded-lg bg-surface-secondary p-3">
              <p className="text-xs text-content-tertiary mb-1 font-medium uppercase tracking-wide">
                {t("transmittals.label_cover_note", {
                  defaultValue: "Cover Note",
                })}
              </p>
                  ...
                  >
                    ...
                    {item.revision && (
                      <Badge variant="blue" size="sm" className="shrink-0">
                        {t("transmittals.revision_prefix", {
            ...
            {transmittal.status === "draft" && (
              <>
                ...
                >
                  ...
                  {t("transmittals.action_issue", {
                    defaultValue: "Issue Transmittal",
                  })}
                </Button>
                <Button
                  variant="secondary"
...
function EditTransmittalModal({
  ...
  const [purpose, setPurpose] = useState<TransmittalPurpose>(
    transmittal.purpose,
  );
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            <div>
              ...
              <select
                value={purpose}
                onChange={(e) =>
                  setPurpose(e.target.value as TransmittalPurpose)
                }
                ...
                {(Object.keys(PURPOSE_LABELS) as TransmittalPurpose[]).map(
                  (k) => (
                    <option key={k} value={k}>
                      {t(`transmittals.purpose_${k}`, {
                        defaultValue: PURPOSE_LABELS[k],
                      })}
                    </option>
                  ),
                )}
            ...
            <div>
              <label className="block text-xs font-medium text-content-secondary mb-1">
                {t("transmittals.field_response_due", {
                  defaultValue: "Response due",
                })}
              </label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium text-content-secondary mb-1">
                {t("transmittals.field_cover_note", {
                  defaultValue: "Cover note",
                })}
              </label>
              <textarea
...
export function TransmittalsPage() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("transmittals.created", {
          defaultValue: "Transmittal created",
        }),
      });
    },
    onError: (e: Error) =>
  ...
  const updateMut = useMutation({
    ...
    onSuccess: async () => {
      ...
      addToast({
        type: "success",
        title: t("transmittals.updated", {
          defaultValue: "Transmittal updated",
        }),
      });
    },
    onError: (e: Error) =>
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: async () => {
      ...
      addToast({
        type: "success",
        title: t("transmittals.deleted", {
          defaultValue: "Transmittal deleted",
        }),
      });
    },
    onError: (e: Error) =>
  ...
  const handleIssue = useCallback(
    async (id: string) => {
      const ok = await confirm({
        title: t("transmittals.confirm_issue_title", {
          defaultValue: "Issue transmittal?",
        }),
        message: t("transmittals.confirm_issue_msg", {
          defaultValue:
            "Once issued, this transmittal will be locked and sent to recipients.",
        }),
    ...
    <div className="w-full animate-fade-in">
      ...
      <div className="flex items-center gap-2 text-2xs text-content-quaternary mb-4">
        ...
        <button
          ...
          aria-label={t("transmittals.flow_upload", { defaultValue: "Upload" })}
        >
        ...
        <button
          ...
          className="hover:text-oe-blue transition-colors"
          aria-label={t("transmittals.flow_organize", {
            defaultValue: "Organize (CDE)",
          })}
        >
        ...
        <div className="flex items-center gap-2 shrink-0">
          {!routeProjectId && projects.length > 0 && (
            <select
              ...
              aria-label={t("transmittals.select_project", {
                defaultValue: "Project...",
              })}
              className={inputCls + " !h-8 !text-xs max-w-[180px]"}
            >
              <option value="" disabled>
                {t("transmittals.select_project", {
                  defaultValue: "Project...",
                })}
              </option>
              {projects.map((p) => (
          ...
          <Button
            ...
            icon={<Plus size={14} />}
          >
            {t("transmittals.new_transmittal", {
              defaultValue: "New Transmittal",
            })}
          </Button>
        </div>
        ...
        <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800 dark:border-blue-700 dark:bg-blue-950/30 dark:text-blue-300 relative">
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="font-semibold">
              {t("transmittals.info_title", {
                defaultValue: "About Transmittals",
              })}
            </span>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        ...
        <Card className="p-4 animate-card-in">
          <p className="text-2xs font-medium text-content-tertiary uppercase tracking-wider">
            {t("transmittals.stat_acknowledged", {
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            aria-label={t("transmittals.search_placeholder", {
              defaultValue: "Search transmittals...",
            })}
        ...
        <div className="relative">
          <select
            value={statusFilter}
            onChange={(e) =>
              setStatusFilter(e.target.value as TransmittalStatus | "")
            }
            aria-label={t("transmittals.filter_all", {
