  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!regRes.ok) {
        ...
        setError(
          data?.detail || t("auth.registration_failed", "Registration failed"),
        );
        return;
      }
  ...
  const benefits = [
    {
      ...
      title: t("login.benefit.local", "Your data stays on your computer"),
      desc: t(
        "login.benefit.local_desc",
        "No cloud. No third-party servers. Full control.",
      ),
    },
    {
      icon: ShieldCheck,
      color: "text-blue-500 bg-blue-500/10",
      title: t("login.benefit.open_source", "100% open source"),
      desc: t(
        "login.benefit.open_source_desc",
        "Transparent code. No vendor lock-in.",
      ),
    },
    {
      icon: Globe2,
      color: "text-violet-500 bg-violet-500/10",
      title: t("login.benefit.standards", "Global cost databases"),
      desc: t(
        "login.benefit.standards_desc",
        "55,000+ cost items, 48 databases.",
      ),
    },
    {
      icon: Brain,
      color: "text-amber-500 bg-amber-500/10",
      title: t("login.benefit.ai", "AI-assisted estimation"),
      desc: t(
        "login.benefit.ai_desc",
        "Smart suggestions. You decide, AI assists.",
      ),
    },
    {
      icon: Zap,
      color: "text-rose-500 bg-rose-500/10",
      title: t("login.benefit.allinone", "BOQ + 4D + 5D + Tendering"),
      desc: t("login.benefit.allinone_desc", "Full workflow in one tool."),
    },
    {
      icon: Users,
      color: "text-cyan-500 bg-cyan-500/10",
      title: t("login.benefit.free", "Free for everyone"),
      desc: t("login.benefit.free_desc", "No fees. No limits. By estimators."),
    },
  ];
  ...
  return (
    <div className="relative flex h-screen bg-surface-secondary overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-[460px] xl:w-[500px] shrink-0 relative z-10 flex-col justify-center pl-14 xl:pl-20 pr-8 xl:pr-10 py-6">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1.5 mb-0.5">
                  <div
                    className={`flex h-5 w-5 shrink-0 items-center justify-center rounded ${item.color}`}
                  >
                    <Icon size={11} />
                  </div>
                  <span className="text-2xs font-semibold text-content-primary leading-tight">
                    {item.title}
                  </span>
                </div>
                <p className="text-2xs text-content-tertiary leading-snug pl-[26px]">
                  {item.desc}
                </p>
              </div>
            );
        ...
        <div
          className="mt-3 flex items-start gap-2 rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-3 py-2 animate-stagger-in"
        ...
        >
          ...
          <div>
            <p className="text-2xs text-content-secondary leading-snug">
              {t(
                "login.privacy",
                "All data is processed and stored locally on your machine. Nothing is sent to external servers. You own your data — always.",
              )}
            </p>
            <p className="mt-0.5 text-[10px] text-content-quaternary leading-snug">
              *{" "}
              {t(
                "login.privacy_ai",
                "If you use built-in AI tools, some data may be sent to the AI provider you configure (OpenAI, Anthropic, etc.). You control which provider to use and what data to share.",
              )}
            </p>
          </div>
        </div>
        ...
        <div
          className="mt-3 flex items-center gap-3 text-2xs text-content-quaternary animate-stagger-in"
      ...
      <div className="flex flex-1 items-center justify-center p-4 sm:p-6 relative z-10">
        <div className="w-full max-w-[400px]">
          {/* Logo */}
          <div
            className="mb-5 flex flex-col items-center animate-stagger-in"
            style={{ animationDelay: "0ms" }}
          >
          ...
          {/* Form */}
          <div
            className="glass-strong rounded-2xl px-6 py-5 shadow-lg animate-form-scale-in"
          ...
          >
            ...
            <form
              ...
              className="space-y-3"
              aria-label={t("auth.register", "Create account")}
            >
              <div
                className="animate-stagger-in"
              ...
              <div
                className="grid grid-cols-2 gap-2 animate-stagger-in"
              ...
              >
                ...
                <div>
                  <label
                    htmlFor="register-how-found"
                    className="text-sm font-medium text-content-primary block mb-1"
                  >
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="">
                        {t("auth.how_found_select", "— Select —")}
                      </option>
                      <option value="google">
                        {t("auth.how_found_google", "Google Search")}
                      </option>
                      <option value="github">
                        {t("auth.how_found_github", "GitHub")}
                      </option>
                      <option value="linkedin">
                        {t("auth.how_found_linkedin", "LinkedIn")}
                      </option>
                      <option value="reddit">
                        {t("auth.how_found_reddit", "Reddit")}
                      </option>
                      <option value="youtube">
                        {t("auth.how_found_youtube", "YouTube")}
                      </option>
                      <option value="recommendation">
                        {t("auth.how_found_recommendation", "Recommendation")}
                      </option>
                      <option value="conference">
                        {t("auth.how_found_conference", "Conference / Event")}
                      </option>
                      <option value="other">
                        {t("auth.how_found_other", "Other")}
                      </option>
                    </select>
                  </div>
                </div>
              ...
              >
                ...
                {password && (
                  <div className="flex items-center gap-2 mt-0.5">
                    <div
                      className={`h-1 flex-1 rounded-full transition-colors duration-normal ${password.length >= 8 ? "bg-semantic-success" : "bg-border"}`}
                    />
                    <div
                      className={`h-1 flex-1 rounded-full transition-colors duration-normal ${password.length >= 12 ? "bg-semantic-success" : "bg-border"}`}
                    />
                    <div
                      className={`h-1 flex-1 rounded-full transition-colors duration-normal ${/[A-Z]/.test(password) && /[0-9]/.test(password) ? "bg-semantic-success" : "bg-border"}`}
                    />
                    <span className="text-2xs text-content-tertiary ml-1">
                      {password.length < 8
                        ? t("auth.password_strength_weak", "Weak")
                        : password.length < 12
                          ? t("auth.password_strength_medium", "Medium")
                          : t("auth.password_strength_strong", "Strong")}
                    </span>
                  </div>
                )}
              </div>
              ...
              >
                <label className="flex items-start gap-2 cursor-pointer group">
                  ...
                  <span className="text-[11px] text-content-secondary leading-snug">
                    ...
                    {". "}
                    {t(
                      "auth.privacy_consent_detail",
                      "Your data is processed in accordance with GDPR. We collect your name, email, company info, and usage data to provide the service. You can delete your account at any time.",
                    )}
                  </span>
                </label>
              </div>
