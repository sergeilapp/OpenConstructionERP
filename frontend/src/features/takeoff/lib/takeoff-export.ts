...
export function summariseByGroupType(
  ...
  rows.sort(
    (a, b) => a.group.localeCompare(b.group) || a.type.localeCompare(b.type),
  );
  return rows;
}
...
export function selectAnnotatedPages(
  ...
  for (const m of measurements) {
    ...
    if (isAnnotationType(m.type) && hiddenGroups.has("__annotations__"))
      continue;
    pages.add(m.page);
  }
...
export async function buildTakeoffPdf(ctx: PdfExportContext): Promise<JsPDF> {
  ...
  for (const pageNum of pageNumbers) {
    ...
    if (!canvasCtx)
      throw new Error("Failed to acquire 2D context for PDF export");
...
function renderPdfSummary(pdf: JsPDF, ctx: PdfExportContext): void {
  ...
  pdf.setFontSize(10);
  pdf.text(`Project: ${ctx.projectName || "Untitled"}`, margin, margin + 28);
  pdf.text(
...
export async function buildTakeoffWorkbook(
  ...
  const ExcelJsCtor = (ExcelJsMod.Workbook ??
    ExcelJsMod.default.Workbook) as typeof ExcelJS.Workbook;
  ...
  for (const groupName of sortedGroups) {
    ...
    const color = (ctx.groupColorMap[groupName] ?? "#3B82F6").replace(
      "#",
      "FF",
    );
    ...
    const subtotalTypes: Array<
      "distance" | "polyline" | "area" | "volume" | "count"
    > = ["distance", "polyline", "area", "volume", "count"];
