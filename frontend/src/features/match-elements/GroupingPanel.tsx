...
export function GroupingPanel({
  ...
  // extra round-trip.
  const sessionData = qc.getQueryData<MatchSession>([
    "match-session",
    sessionId,
  ]);
  ...
  const updateSlot = useCallback(
    (idx: 0 | 1 | 2, value: string) => {
      const next: [string, string, string] = [...slots] as [
        string,
        string,
        string,
      ];
    ...
    <div className="space-y-5" data-testid="grouping-panel">
      ...
      <div>
        <div className="mb-2 text-xs font-medium uppercase tracking-wide text-content-tertiary">
          {t("match.wizard.grouping.presets", {
            defaultValue: "Common groupings",
          })}
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            {t("match.wizard.grouping.preset.custom", {
              defaultValue: "Custom",
            })}
          </span>
        </div>
          ...
          {([0, 1, 2] as const).map((idx) => {
            ...
            return (
              ...
              >
                <option value="">
                  {idx === 0
                    ? t("match.wizard.grouping.slot1", {
                        defaultValue: "Primary key…",
                      })
                    : idx === 1
                      ? t("match.wizard.grouping.slot2", {
                          defaultValue: "Secondary…",
                        })
                      : t("match.wizard.grouping.slot3", {
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-surface-muted text-content-secondary">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left font-medium">
                      {t("match.wizard.grouping.unit", {
                        defaultValue: "Unit",
                      })}
                    </th>
                    <th className="px-3 py-2 text-left font-medium">
                      {t("match.wizard.grouping.samples", {
