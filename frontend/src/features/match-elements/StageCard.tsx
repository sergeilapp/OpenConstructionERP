...
function OutputPreview({ output }: { output: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-1.5 space-y-1.5">
      {summary && <p className="text-xs text-content-secondary">{summary}</p>}
      {chips.length > 0 && (
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex flex-col items-center">
        ...
        {!isLast && <div className="w-px flex-1 my-1 bg-border" aria-hidden />}
      </div>
      ...
      <div className="flex-1 pb-3 min-w-0">
        ...
        >
          ...
          {/* Output preview */}
          {stage.status === "done" && <OutputPreview output={stage.output} />}
