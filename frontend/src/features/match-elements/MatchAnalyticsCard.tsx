...
function Tile({
  ...
  const toneClass = {
    ...
    positive:
      "border-emerald-300/70 dark:border-emerald-800/70 bg-emerald-50/40 dark:bg-emerald-950/20",
    warning:
      "border-amber-300/70 dark:border-amber-700/70 bg-amber-50/40 dark:bg-amber-950/20",
    critical:
      "border-red-300/70 dark:border-red-800/70 bg-red-50/40 dark:bg-red-950/20",
  }[tone];
  return (
    <div className={`rounded-lg border ${toneClass} px-3 py-2`}>
      ...
      <div className="text-lg font-semibold text-content-primary leading-tight">
        {value}
      </div>
      {hint && (
        <div className="text-[11px] text-content-tertiary mt-0.5">{hint}</div>
      )}
    </div>
  );
}
...
function AlertRow({ alert }: { alert: AnalyticsAlert }) {
  ...
  return (
    <div
      className={`rounded-lg border px-3 py-2 flex items-start gap-2 ${tone}`}
    >
        ...
        {rows.map((r) => {
          ...
          return (
            <div key={r.key} className="flex items-center gap-2 text-xs">
              <div
                className="w-24 shrink-0 truncate text-content-secondary"
                title={r.key}
              >
                {r.key}
              </div>
      ...
      <div className="flex items-end gap-1 h-16">
        {entries.map(([k, v]) => {
          ...
          return (
            ...
            >
              <div className="text-[10px] tabular-nums text-content-tertiary">
                {v}
              </div>
              <div
...
export function MatchAnalyticsCard({ projectId }: MatchAnalyticsCardProps) {
  ...
  const q = useQuery<MatchAnalyticsResponse>({
    ...
    queryFn: () =>
      matchElementsApi.getAnalytics({ project_id: projectId ?? null, days }),
    staleTime: 60_000,
          ...
          {!empty && (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <Tile
                  label={t(
                    "match_elements.analytics_tile_searches",
                    "Searches",
                  )}
                ...
                <Tile
                  label={t(
                    "match_elements.analytics_tile_pick_rate",
                    "Pick rate",
                  )}
                ...
                <Tile
                  label={t(
                    "match_elements.analytics_tile_latency",
                    "Latency p95",
                  )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <Tile
                  label={t(
                    "match_elements.analytics_tile_low_score",
                    "Low score (<0.3)",
                  )}
                ...
                <Tile
                  label={t(
                    "match_elements.analytics_tile_zero_hit",
                    "Zero-hit",
                  )}
                ...
                <Tile
                  label={t(
                    "match_elements.analytics_tile_pick_above_4",
                    "Picks at rank > 4",
                  )}
                  ...
                  tone={
                    data.high_picked_rank_pct >= 0.2 ? "warning" : "neutral"
                  }
                />
                <Tile
                  label={t(
                    "match_elements.analytics_tile_rerank",
                    "BGE rerank",
                  )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <BreakdownTable
                  ...
                  caption={t(
                    "match_elements.analytics_by_country",
                    "By country",
                  )}
                  emptyLabel={t(
                ...
                <BreakdownTable
                  ...
                  caption={t(
                    "match_elements.analytics_by_source",
                    "By source type",
                  )}
                  emptyLabel={t(
                    "match_elements.analytics_breakdown_empty",
