...
function ConfidencePill({
  ...
  return (
    <span
      className={`inline-block px-1.5 py-0.5 rounded text-white text-xs ${cls}`}
    >
      {score.toFixed(2)}
    </span>
  );
    ...
    <>
      ...
      >
        ...
        <nav
          ...
          role="tablist"
          aria-label={t(
            "match_elements.detail.tabs_label",
          ...
          {(["methods", "elements", "apply"] as const).map((tabKey) => (
            ...
            >
              {tabKey === "methods"
                ...
                : tabKey === "elements"
                  ? t("match_elements.tab.elements", "Elements ({{count}})", {
                      count: group.element_count,
                    })
        ...
        <div className="flex-1 overflow-auto p-4">
          ...
          {tab === "methods" && detailQ.data && (
            <div
              role="tabpanel"
              id="detail-tabpanel-methods"
              aria-labelledby="detail-tab-methods"
            >
                ...
                <div className="text-center py-12 text-slate-500">
                  ...
                  <p className="text-sm">
                    {t(
                      ...
                      "No matchers run yet for this group.",
                    )}
                  </p>
                  <p className="text-xs mt-1">
                    {t(
                      "match_elements.detail.use_action_bar",
                      "Use the action bar buttons above.",
                    )}
                  </p>
                </div>
              )}
                    ...
                    <thead className="bg-slate-50 dark:bg-slate-800">
                      <tr>
                        <th className="text-left px-2 py-1.5 font-medium">
                          {t("match_elements.detail.col.code", "Code")}
                        </th>
                        <th className="text-left px-2 py-1.5 font-medium">
                          {t(
                            "match_elements.detail.col.description",
                            "Description",
                          )}
                        </th>
                        <th className="text-right px-2 py-1.5 font-medium">
                          {t(
                            "match_elements.detail.col.unit_rate",
                            "Unit · Rate",
                          )}
                        </th>
                        <th className="text-right px-2 py-1.5 font-medium">
                          {t("match_elements.detail.col.conf", "Conf.")}
                        </th>
                      ...
                      {(methods[name] ?? []).slice(0, 10).map((cand, idx) => (
                        ...
                        >
                          ...
                          <td className="px-2 py-1.5 text-right text-xs tabular-nums">
                            {cand.unit_rate.toFixed(2)} {cand.currency}/
                            {cand.unit}
                          </td>
          ...
          {tab === "elements" && detailQ.data && (
            <div
              role="tabpanel"
              id="detail-tabpanel-elements"
              aria-labelledby="detail-tab-elements"
            >
              <p className="text-xs text-slate-500 mb-2">
                {t(
                  "match_elements.detail.element_ids_count",
                  "{{count}} element id(s)",
                  { count: detailQ.data.element_ids.length },
                )}
              </p>
              <div className="font-mono text-xs leading-tight max-h-[60vh] overflow-auto bg-slate-50 dark:bg-slate-800 p-2 rounded">
                ...
                {detailQ.data.element_ids.length > 200 && (
                  <div className="text-slate-500">
                    {t(
                      "match_elements.detail.and_more",
                      "…and {{count}} more",
                      { count: detailQ.data.element_ids.length - 200 },
                    )}
                  </div>
                )}
              </div>
          ...
          {tab === "apply" && (
            <div
              role="tabpanel"
              id="detail-tabpanel-apply"
              aria-labelledby="detail-tab-apply"
            >
              {applyQ.isLoading && (
                <div className="text-center text-slate-500 py-8">
                  <Loader2 className="w-5 h-5 animate-spin inline mr-2" />
                ...
                applyQ.data.positions.map((p) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                      ...
                      {p.unit_rate.toFixed(2)} {p.currency} ={" "}
                      <strong className="tabular-nums">
                        {p.line_total.toFixed(2)} {p.currency}
                      </strong>
                    </div>
                    {p.resources.length > 0 && (
                      <div className="mt-2 pl-3 border-l-2 border-slate-200 dark:border-slate-700">
                        ...
                        {p.resources.map((r, i) => (
                          <div
                            key={i}
                            className="text-xs flex justify-between text-slate-600 dark:text-slate-300"
                          >
                            <span>
                              {r.description} (×{r.factor})
                            </span>
              ...
              {applyQ.data && applyQ.data.positions.length === 0 && (
                <div className="text-center text-slate-500 py-8 text-sm">
                  {t(
                    ...
                    "Confirm a match first to see the BOQ preview.",
                  )}
                </div>
              )}
            </div>
        ...
        <footer className="px-4 py-3 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-500">
            ...
            <strong>
              {t(`match_elements.status.${group.status}`, group.status)}
            </strong>
            <ChevronRight className="w-3 h-3" />
            <CheckCircle2
              className={`w-4 h-4 ${group.status === "confirmed" ? "text-emerald-500" : "text-slate-300"}`}
            />
          </div>
        </footer>
      </aside>
