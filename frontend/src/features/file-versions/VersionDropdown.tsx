...
export function VersionDropdown({
  ...
  if (isError) {
    return (
      ...
      >
        <History size={12} />
        {t("files.versions.load_failed", {
          defaultValue: "Versions unavailable",
        })}
      </div>
    );
  }
    ...
    <div className={clsx("relative inline-block", className)}>
      ...
      >
        ...
        {current ? (
          ...
          <span>
            {t("files.versions.no_history", { defaultValue: "No history" })}
          </span>
        )}
        {total > 1 && (
          <span className="text-content-tertiary tabular-nums">({total})</span>
        )}
        <ChevronDown
          size={11}
      ...
      {open && versions && versions.length > 0 && (
        <div
          role="listbox"
          aria-label={t("files.versions.dropdown_aria", {
            defaultValue: "File version history",
          })}
          className={clsx(
