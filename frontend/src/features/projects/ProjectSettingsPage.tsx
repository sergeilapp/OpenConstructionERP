...
function FxRateModal({
  ...
  const effectiveCode =
    code === "__custom__" ? customCode.trim().toUpperCase() : code;
  ...
  const canSave =
    codeLooksValid && rateLooksValid && !codeIsTaken && !isSameAsBase;
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!resolvedLabel) {
      ...
      if (opt) {
        ...
        resolvedLabel =
          dash >= 0 ? opt.label.slice(dash + 1).trim() : opt.label;
      }
    }
    onSave({
      code: effectiveCode,
      ...
      <div className="relative w-full max-w-md mx-4 rounded-2xl bg-surface-elevated border border-border-light shadow-2xl animate-fade-in">
        <div className="flex items-center justify-between px-6 pt-6 pb-2">
          <h3 className="text-lg font-semibold text-content-primary">
            {isEdit
              ? t("project.settings.fx.edit_title", {
                  defaultValue: "Edit currency‌⁠‍",
                })
              : t("project.settings.fx.add_title", {
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 pt-2 space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-content-primary">
              {t("project.settings.fx.currency", {
            ...
            {isSameAsBase && (
              <p className="text-xs text-semantic-error">
                {t("project.settings.fx.cannot_be_base", {
                  defaultValue:
                    "Additional currencies must differ from the base currency.",
                })}
              </p>
            )}
            {codeIsTaken && (
          ...
          {/* Optional label */}
          <Input
            label={t("project.settings.fx.label", {
...
export function ProjectSettingsPage() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (updated) => {
      ...
      addToast({
        type: "success",
        title: t("project.settings.saved", {
          defaultValue: "Project settings saved",
        }),
      });
    },
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("project.settings.save_failed", {
  ...
  if (!project) {
    return (
      <div className="w-full">
        <EmptyState
          title={t("project.settings.not_found", {
  ...
  const overrideActive =
    (project.default_vat_rate ?? "").toString().trim() !== "";
  ...
  return (
    <div className="w-full space-y-5 animate-fade-in">
      ...
      <Card padding="lg">
        <CardHeader
          title={t("project.settings.base_currency.title", {
        ...
        <div className="mt-4 inline-flex items-center gap-2 rounded-lg border border-border-light bg-surface-secondary/40 px-3 py-2">
          ...
          <span className="text-sm font-medium text-content-primary tabular-nums">
            {baseCurrency ||
              t("project.settings.base_currency.unset", {
      ...
      <Card padding="lg" id="fx-rates">
        <CardHeader
          title={t("project.settings.fx.title", {
            defaultValue: "Additional currencies",
          })}
            ...
            <div className="rounded-lg border border-dashed border-border-light bg-surface-secondary/30 px-4 py-6 text-center">
              <p className="text-sm text-content-tertiary">
                {t("project.settings.fx.empty", {
                  defaultValue:
                    'No additional currencies. Click "Add currency" to set one up.',
                })}
              </p>
            </div>
          ) : (
                ...
                <thead className="bg-surface-secondary/40">
                  <tr className="text-left text-xs uppercase tracking-wide text-content-tertiary">
                    <th className="px-4 py-2 font-medium">
                      {t("project.settings.fx.col_code", {
                        defaultValue: "Code",
                      })}
                    </th>
                    <th className="px-4 py-2 font-medium">
                      {t("project.settings.fx.col_label", {
                        defaultValue: "Label",
                      })}
                    </th>
                      ...
                      <td className="px-4 py-2.5 text-content-secondary">
                        {row.label || (
                          <span className="text-content-tertiary italic">
                            {t("project.settings.fx.no_label", {
                              defaultValue: "—",
                            })}
                          </span>
                        )}
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex justify-end gap-1">
                          <button
                            type="button"
                            onClick={() =>
                              setFxModal({ open: true, initial: row })
                            }
      ...
      <Card padding="lg">
        <CardHeader
          title={t("project.settings.vat.title", {
            defaultValue: "Default VAT rate",
          })}
          subtitle={t("project.settings.vat.subtitle", {
            defaultValue:
              "Used when seeding markups for new BOQs in this project.",
          })}
          action={
      ...
      <Card padding="lg">
        <CardHeader
          title={t("project.settings.units.title", {
            defaultValue: "Custom units",
          })}
        ...
        <div className="mt-4 space-y-3">
          ...
          <form onSubmit={handleAddUnit} className="flex items-end gap-2">
            <div className="flex-1 max-w-xs">
              <Input
                label={t("project.settings.units.add_label", {
                  defaultValue: "Add unit",
                })}
                value={unitInput}
