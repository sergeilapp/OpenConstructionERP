...
describe("PhotosTab", () => {
  ...
  it("opens the lightbox when a tile is clicked and closes on ESC", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("photos-tab-lightbox-image"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("photos-tab-lightbox"),
  ...
  it("filters tiles by filename via the search input", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByTestId("photos-tab-tile-photo-1"),
      ).not.toBeInTheDocument();
    });
    expect(screen.getByTestId("photos-tab-tile-photo-2")).toBeInTheDocument();
    expect(
      screen.queryByTestId("photos-tab-tile-photo-3"),
    ).not.toBeInTheDocument();
  });
});
