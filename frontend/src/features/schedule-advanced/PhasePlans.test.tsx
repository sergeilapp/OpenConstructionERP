...
describe("PhasePlans tab", () => {
  ...
  it("renders the empty-state CTAs when there are no phases", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /new phase/i }).length,
    ).toBeGreaterThan(0);
    expect(
  ...
  it("opens the create-phase modal when the empty-state CTA is clicked", async () => {
    ...
    expect(
      await screen.findByText(/^new phase$/i, { selector: "h2,h3" }),
    ).toBeInTheDocument();
    // Form fields visible
  ...
  it("renders cards + status filter chips when phases exist", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /^in planning/i }),
    ).toBeInTheDocument();
  });
  ...
  it("submits createPhasePlan with the entered name", async () => {
    (listPhasePlans as any).mockResolvedValue([]);
    (createPhasePlan as any).mockResolvedValue({
      ...samplePhase,
      name: "New Phase A",
    });
    renderPage();
