...
describe("NlRuleBuilderPanel", () => {
  ...
  it("renders the YAML preview after a successful generation", async () => {
    ...
    expect(screen.getByTestId("dsl-preview-yaml").textContent).toContain(
      "custom.wall.has_fire_rating",
    );
  });
  ...
  it("calls saveDslRule with the generated YAML when Save is clicked", async () => {
    ...
    await waitFor(() => {
      expect(saveDslRule).toHaveBeenCalledWith(SAMPLE_RESULT.dsl_yaml, true);
    });
  });
