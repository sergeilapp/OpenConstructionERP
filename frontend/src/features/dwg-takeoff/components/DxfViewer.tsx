...
export function DxfViewer({
  ...
  const [textPinPopup, setTextPinPopup] = useState<TextPinPopupState | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const visibleEntities =
        hidden.size > 0 ? entities.filter((e) => !hidden.has(e.id)) : entities;
      ...
      if (primarySelId) {
        ...
        if (
          selEnt?.type === "LWPOLYLINE" &&
          selEnt.vertices &&
          selEnt.vertices.length >= 2
        ) {
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (activeTool === "calibrate") {
        ...
        const snapHit = activeSnapRef.current;
        const world = snapHit
          ? { x: snapHit.point.x, y: snapHit.point.y }
          : rawWorld;
      ...
      {extentsRef.current && (
        <div className="absolute bottom-3 right-3 flex items-center gap-1.5">
          ...
          <button
            ...
            title={t("dwg_takeoff.fit_all", {
              defaultValue: "Fit to drawing bounds‌⁠‍",
            })}
          >
            <svg
              width="14"
...
function TextPinPopup({
  ...
  const left = Math.min(
    screenPt.x + 12,
    (popupRef.current?.parentElement?.clientWidth ?? 9999) - popupWidth - 8,
  );
  const top = Math.min(
    screenPt.y + 12,
    (popupRef.current?.parentElement?.clientHeight ?? 9999) - popupHeight - 8,
  );
...
function renderPolylineMeasurements(
  ...
  if (closed && area > 0) {
    ...
    roundRect(
      ctx,
      centroidScreen.x - atw / 2,
      centroidScreen.y - ath / 2,
      atw,
      ath,
      6,
    );
  ...
  for (const ent of entities) {
    ...
    if (ent.type === "LWPOLYLINE" && ent.vertices && ent.vertices.length >= 2) {
      ...
      if (ent.closed && ent.vertices.length >= 3) {
        const sd = pointToSegmentDistance(
          world,
          ent.vertices[ent.vertices.length - 1]!,
          ent.vertices[0]!,
        );
        if (sd < d) d = sd;
