...
export function MarkupCommentsDrawer({
  markupId,
  onClose,
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("takeoff.markup.comment_added", {
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("takeoff.markup.comment_deleted", {
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {isLoading ? (
          ...
          <ul className="space-y-3">
            {data.map((c) => {
              const isMine =
                !!currentUserEmail && currentUserEmail === c.user_id;
              return (
                <li
                  key={c.id}
