...
describe("collectHitCandidates — RFC 11 §4.2", () => {
  ...
  it("scoreOf: among inside-hits, smaller areas score lower", () => {
    const small: HitCandidate = {
      id: "small",
      distance: 0,
      inside: true,
      area: 10,
    };
    const big: HitCandidate = {
      id: "big",
      distance: 0,
      inside: true,
      area: 10000,
...
describe("selection Set semantics — RFC 11 §4.3", () => {
  ...
  it("shift-click adds a new id to the selection", () => {
    const s0 = applySelection(new Set(["a"]), {
      kind: "click",
      id: "b",
      shiftKey: true,
    });
  ...
  it("shift-click toggles an already-selected id off", () => {
    const s0 = applySelection(new Set(["a", "b"]), {
      kind: "click",
      id: "a",
      shiftKey: true,
    });
