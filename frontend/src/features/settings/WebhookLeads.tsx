...
export function WebhookLeads() {
  ...
  const createSource = useMutation({
    ...
    onSuccess: (created) => {
      ...
      addToast({
        type: "success",
        title: t("webhook_leads.created", {
          defaultValue: "Webhook source created",
        }),
      });
    },
    onError: (e: unknown) =>
      addToast({
        type: "error",
        title: t("webhook_leads.create_failed", {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4">
        <h3 className="font-medium mb-3 flex items-center gap-2">
          ...
          {t("webhook_leads.new_source", {
            defaultValue: "New webhook source",
          })}
        </h3>
        ...
        <>
              ...
              {(mappingsQ.data ?? []).length === 0 && (
                <p className="text-sm text-gray-500">
                  {t("webhook_leads.no_mappings", {
                    defaultValue:
                      "No mappings — add at least one required rule.",
                  })}
                </p>
              )}
            </div>
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button size="sm" variant="ghost" onClick={() => logsQ.refetch()}>
                <RefreshCw className="w-4 h-4" />
              </Button>
            </div>
