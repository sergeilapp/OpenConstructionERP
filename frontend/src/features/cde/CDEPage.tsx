...
const STATE_CONFIG: Record<
  CDEState,
  {
    variant: "neutral" | "blue" | "success" | "warning";
    cls: string;
    label: string;
  }
> = {
...
function CreateCDEModal({
  ...
  const canSubmit =
    form.container_code.trim().length > 0 && form.title.trim().length > 0;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-lg animate-fade-in">
      <div
        ...
        role="dialog"
        aria-label={t("cde.new_container", {
      ...
      >
        ...
        <div className="px-6 py-4 space-y-4">
            ...
            <div>
              ...
              {codeError && (
                <p className="mt-1 text-xs text-semantic-error">
                  {t("cde.code_required", {
                    defaultValue: "Container code is required‌⁠‍",
                  })}
                </p>
              )}
            </div>
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) =>
                    set("discipline", e.target.value as CDEDiscipline)
                  }
                  className={inputCls + " appearance-none pr-9"}
                >
                  {(Object.keys(DISCIPLINE_LABELS) as CDEDiscipline[]).map(
                    (d) => (
                      <option key={d} value={d}>
                        {t(`cde.discipline_${d}`, {
                          defaultValue: DISCIPLINE_LABELS[d],
                        })}
                      </option>
                    ),
                  )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  {STATE_ORDER.map((s) => (
                    <option key={s} value={s}>
                      {t(`cde.state_${s}`, {
                        defaultValue: STATE_CONFIG[s].label,
                      })}
                    </option>
                  ))}
                </select>
            ...
            <div>
              ...
              >
                {t("cde.field_suitability", {
                  defaultValue: "Suitability Code",
                })}
              </label>
              <div className="relative">
                <select
                  ...
                  className={inputCls + " appearance-none pr-9"}
                  aria-label={t("cde.field_suitability", {
                    defaultValue: "Suitability Code",
                  })}
                >
                  <option value="">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border-light">
          ...
          <Button variant="primary" onClick={handleSubmit} disabled={isPending}>
            ...
            <span>
              {t("cde.create_container", { defaultValue: "Create Container" })}
            </span>
          </Button>
        </div>
      </div>
  ...
  const { data: docs = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGet<DocItem[] | { items: DocItem[] }>(
        ...
        Array.isArray(res) ? res : ((res as { items: DocItem[] }).items ?? []),
      ),
  });
      ...
      >
        ...
        <div className="px-5 py-3 border-b border-border shrink-0">
          ...
          {selected.length > 0 && (
            <div className="mt-2 text-xs text-oe-blue font-medium">
              {selected.length}{" "}
              {t("cde.selected", { defaultValue: "selected" })}
            </div>
          )}
        </div>
              ...
              <p>
                {docs.length === 0
                  ? t("cde.no_documents_in_project", {
                      defaultValue:
                        "No documents in this project. Upload documents first.",
                    })
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-border shrink-0">
          <span className="text-xs text-content-tertiary">
            {filtered.length}{" "}
            {t("cde.documents_available", {
              defaultValue: "documents available",
            })}
          </span>
    ...
    <div className="border-b border-border-light last:border-b-0">
      ...
      >
        ...
        <Badge
          variant="neutral"
          size="sm"
          className={clsx(disciplineCls, "hidden md:inline-flex")}
        >
          {t(`cde.discipline_${containerDiscipline}`, {
            defaultValue:
              DISCIPLINE_LABELS[containerDiscipline] ?? containerDiscipline,
          })}
        </Badge>

        {/* CDE State badge */}
        <span
          title={t("cde.iso19650_states_tooltip", {
            defaultValue:
              "ISO 19650 document states: WIP = Work in Progress (being authored), Shared = shared with team for review, Published = formally approved and issued, Archived = superseded or no longer current",
          })}
        >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation();
                navigate(
                  `/transmittals?create=true&container_id=${container.id}`,
                );
              }}
            >
              <Send size={13} className="mr-1" />
              {t("cde.send_transmittal", {
            ...
            <Button
              ...
              title={t("cde.view_history", {
                defaultValue: "View state transition history",
              })}
            >
            ...
            {revisionsLoading ? (
              ...
              <div className="px-4 py-3 bg-surface-secondary/30 rounded-lg">
                <p className="text-xs text-content-quaternary">
                  {t("cde.no_revisions_hint", {
                    defaultValue:
                      "No revisions yet. Upload documents and link them to this container.",
                  })}
                </p>
              </div>
            ) : (
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg bg-surface-secondary p-2.5 text-sm">
      ...
      <DateDisplay
        ...
        className="text-xs text-content-tertiary w-24 shrink-0"
      />
...
export function CDEPage() {
    ...
    return containers.filter(
      (c) =>
        ...
        (c.classification_code &&
          c.classification_code.toLowerCase().includes(q)),
    );
  ...
  const createMut = useMutation({
    ...
    onError: (e: Error) => {
      ...
      addToast({
        type: "error",
        title: t("cde.create_failed", {
          defaultValue: "Failed to create container",
        }),
        message: detail,
      });
    },
  ...
  const [approvalTarget, setApprovalTarget] = useState<CDEContainer | null>(
    null,
  );
  ...
  const handlePromote = useCallback(
    async (container: CDEContainer) => {
      ...
      const ok = await confirm({
        title: t("cde.confirm_promote_title", {
          defaultValue: "Promote container?",
        }),
        message: t("cde.confirm_promote_msg", {
          defaultValue:
            'This will move the container to the "{{state}}" state.',
          state: nextState,
        }),
          ...
          <div className="flex items-center justify-between gap-2 mb-1">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold">
                {t("cde.info_title", {
                  defaultValue: "About Document Containers",
                })}
              </span>
            </div>
            <button
          ...
          <EmptyState
            ...
            title={
              searchQuery || stateFilter
                ? t("cde.no_results", {
                    defaultValue: "No matching containers",
                  })
        ...
        <LinkDocumentModal
          ...
          onLinked={() => {
            qc.invalidateQueries({
              queryKey: ["cde-revisions", linkTarget.id],
            });
            setLinkTarget(null);
          }}
        />
