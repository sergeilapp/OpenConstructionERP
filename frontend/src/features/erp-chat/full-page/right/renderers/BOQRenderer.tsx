...
export default function BOQRenderer({ data }: { data: unknown }) {
  ...
  const grandTotal = items.reduce(
    (sum, it) => sum + (it.total ?? (it.quantity ?? 0) * (it.unit_rate ?? 0)),
    0,
  );
      ...
      >
        <thead>
          <tr style={{ background: "var(--chat-surface-2)" }}>
            ...
            <th style={{ ...cellBase, fontWeight: 600, textAlign: "left" }}>
              Description
            </th>
            <th
              style={{
        ...
        <tbody>
          {items.map((item, i) => {
            const total =
              item.total ?? (item.quantity ?? 0) * (item.unit_rate ?? 0);
            const isZeroPrice =
              (item.unit_rate ?? 0) === 0 && (item.quantity ?? 0) > 0;
            return (
              <tr
                key={item.ordinal ?? i}
                style={{
                  background:
                    i % 2 === 0 ? "transparent" : "var(--chat-surface-1)",
                  borderLeft: isZeroPrice
                    ? "3px solid var(--chat-tool-error)"
                    : "3px solid transparent",
                }}
              >
                <td style={{ ...cellBase, color: "var(--chat-text-tertiary)" }}>
                  {item.ordinal ?? i + 1}
                </td>
                ...
                <td
                  style={{
                    ...numCell,
                    color: isZeroPrice ? "var(--chat-tool-error)" : undefined,
                  }}
                >
                  {item.unit_rate != null
                    ? item.unit_rate.toLocaleString(undefined, {
