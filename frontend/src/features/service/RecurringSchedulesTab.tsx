...
import { Repeat, PlayCircle, Plus, Power, Loader2, Trash2 } from "lucide-react";
import {
  Button,
  Card,
  Badge,
...
export function RecurringSchedulesTab({ contracts }: Props) {
  ...
  const runNow = useMutation({
    mutationFn: (id: string) =>
      materializeRecurringSchedule(id, { force: true }),
    onSuccess: (result) => {
      void qc.invalidateQueries({
        queryKey: ["service", "recurring-schedules"],
      });
            ...
            <table className="w-full text-sm">
              <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
                <tr>
                  ...
                  <th className="px-4 py-2.5 text-left">
                    {t("service.recurring.col_rrule", {
                      defaultValue: "RRULE",
                    })}
                  </th>
                  <th className="px-4 py-2.5 text-left">
                    {t("service.recurring.col_next", {
                      defaultValue: "Next run",
                    })}
                  </th>
                  <th className="px-4 py-2.5 text-left">
                    {t("service.recurring.col_last", {
                      defaultValue: "Last run",
                    })}
                  </th>
                  <th className="px-4 py-2.5 text-left">
                    {t("service.recurring.col_status", {
                      defaultValue: "Status",
                    })}
                  </th>
...
function CreateRecurringModal({
  ...
  const canSubmit =
    name.trim().length > 0 && rrule.trim().length > 0 && !!contractId;

  return (
    <WideModal
      open
      onClose={onClose}
      title={t("service.recurring.new", {
        defaultValue: "New Recurring Schedule",
      })}
      footer={
        <>
