...
export function ServicePage() {
  ...
  const [selected, setSelected] = useState<{ kind: Tab; id: string } | null>(
    null,
  );
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-content-secondary">
            {t("service.subtitle", {
              defaultValue:
                "Manage service contracts, assets, tickets and work orders.",
            })}
          </p>
        </div>
        {tab !== "recurring" && (
        ...
        <nav className="flex gap-1 -mb-px">
          {(
            [
              {
                id: "tickets",
                label: t("service.tickets", { defaultValue: "Tickets" }),
                icon: Wrench,
              },
              {
                id: "work_orders",
                label: t("service.work_orders", {
                  defaultValue: "Work Orders",
                }),
                icon: ClipboardList,
              },
              {
                id: "contracts",
                label: t("service.contracts", { defaultValue: "Contracts" }),
                icon: FileText,
              },
              {
                id: "assets",
                label: t("service.assets", { defaultValue: "Assets" }),
                icon: Box,
              },
              {
                id: "recurring",
                label: t("service.recurring_tab", {
    ...
    return (
      <EmptyState
        ...
        description={t("service.empty_tickets_desc", {
          defaultValue:
            "Log a service ticket against a contract to dispatch a technician.",
        })}
        action={{
        ...
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("service.ticket", { defaultValue: "Ticket" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.title_col", { defaultValue: "Title" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.priority", { defaultValue: "Priority" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.status", { defaultValue: "Status" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.sla_chip", { defaultValue: "SLA" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.reported_at", { defaultValue: "Reported" })}
            </th>
          </tr>
        </thead>
        <tbody>
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        ...
        title={t("service.empty_work_orders", {
          defaultValue: "No work orders yet",
        })}
        description={t("service.empty_work_orders_desc", {
          defaultValue:
            "Work orders are created from dispatched tickets to track on-site work.",
        })}
        action={{
          label: t("service.new_work_order", {
            defaultValue: "New Work Order",
          }),
          onClick: emptyAction,
        }}
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("service.wo_number", { defaultValue: "WO #" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.scheduled_for", { defaultValue: "Scheduled" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.technician", { defaultValue: "Technician" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.status", { defaultValue: "Status" })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("service.billed", { defaultValue: "Billed" })}
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            ...
            >
              ...
              <td className="px-4 py-2 text-xs text-content-secondary">
                {r.scheduled_for ? (
                  <DateDisplay value={r.scheduled_for} />
                ) : (
                  "—"
                )}
              </td>
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        ...
        title={t("service.empty_contracts", {
          defaultValue: "No service contracts yet",
        })}
        description={t("service.empty_contracts_desc", {
          defaultValue:
            "Create a contract to associate assets, SLA tiers and recurring work.",
        })}
        action={{
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("service.contract_number", { defaultValue: "Contract #" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.title_col", { defaultValue: "Title" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.period", { defaultValue: "Period" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.sla_tier", { defaultValue: "SLA" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.status", { defaultValue: "Status" })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("service.value", { defaultValue: "Value" })}
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            ...
            >
              ...
              <td className="px-4 py-2 font-medium truncate max-w-[280px]">
                {r.title || "—"}
              </td>
              <td className="px-4 py-2 text-xs text-content-secondary">
...
function AssetTable({
  ...
  if (!hasContract) {
    return (
      <EmptyState
        icon={<FileText size={22} />}
        title={t("service.no_contract_selected", {
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={<Box size={22} />}
        title={t("service.empty_assets", {
          defaultValue: "No assets under this contract",
        })}
        description={t("service.empty_assets_desc", {
          defaultValue:
            "Register the equipment you maintain — HVAC, lifts, generators, etc.",
        })}
        action={{
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("service.asset_tag", { defaultValue: "Tag" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.name", { defaultValue: "Name" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("service.asset_type", { defaultValue: "Type" })}
            </th>
            <th className="px-4 py-2.5 text-left">
...
function DetailDrawer({
  ...
  const wo =
    kind === "work_orders" ? workOrders.find((x) => x.id === id) : null;
  const contract =
    kind === "contracts" ? contracts.find((x) => x.id === id) : null;
  ...
  const dispatchMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("service.dispatched", { defaultValue: "Ticket dispatched" }),
      });
    },
  ...
  const resolveMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("service.resolved", { defaultValue: "Ticket resolved" }),
      });
    },
    onError: (err) => addToast({ type: "error", title: getErrorMessage(err) }),
  });
  ...
  const closeMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("service.ticket_closed", { defaultValue: "Ticket closed" }),
      });
    },
    onError: (err) => addToast({ type: "error", title: getErrorMessage(err) }),
  });
  ...
  const billMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("service.billed_ok", { defaultValue: "Work order billed" }),
      });
    },
    onError: (err) => addToast({ type: "error", title: getErrorMessage(err) }),
  });
  ...
  const advanceWoMut = useMutation({
    mutationFn: (next: WorkOrderStatus) =>
        ...
        <div className="space-y-4 p-5">
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <Field
                  ...
                  value={
                    <Badge variant={TICKET_STATUS_VARIANT[ticket.status]} dot>
                      {ticket.status}
                    </Badge>
                  }
                />
                <Field
                  label={t("service.reported_at")}
                  value={<DateDisplay value={ticket.reported_at} />}
                />
                <Field
                  label={t("service.assigned_to", {
                    defaultValue: "Assigned to",
                  })}
                  value={ticket.assigned_to || "—"}
                />
                <Field
                  label={t("service.sla_due", { defaultValue: "SLA due" })}
                  value={
                    ticket.sla_due_at ? (
                ...
                {ticket.resolved_at && (
                  <Field
                    label={t("service.resolved_at", {
                      defaultValue: "Resolved at",
                    })}
                    value={<DateDisplay value={ticket.resolved_at} />}
                  />
                )}
              </div>
              ...
              {ticket.status === "new" && (
                <Card padding="sm">
                  <p className="text-xs font-semibold text-content-secondary uppercase tracking-wide mb-2">
                    {t("service.dispatch_ticket", {
                      defaultValue: "Dispatch to technician",
                    })}
                  </p>
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder={t("service.technician_id", {
                        defaultValue: "Technician ID / email",
                      })}
                      className={inputCls}
                    />
                    <Button
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                ...
                {ticket.status === "in_progress" && (
                  ...
                  >
                    {t("service.mark_resolved", {
                      defaultValue: "Mark Resolved",
                    })}
                  </Button>
                )}
            ...
            <>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <Field
                  label={t("service.status")}
                  value={
                    <Badge variant={WO_STATUS_VARIANT[wo.status]} dot>
                      {wo.status}
                    </Badge>
                  }
                />
                <Field
                  label={t("service.scheduled_for")}
                  value={
                    wo.scheduled_for ? (
                      <DateDisplay value={wo.scheduled_for} />
                    ) : (
                      "—"
                    )
                  }
                ...
                />
                <Field
                  label={t("service.billed")}
                  value={
                    <MoneyDisplay
                      amount={Number(wo.billed_amount) || 0}
                      currency={wo.currency || undefined}
                    />
                  }
                />
              </div>
              ...
              {wo.status === "scheduled" && (
                ...
                >
                  {t("service.dispatch_wo", {
                    defaultValue: "Dispatch Work Order",
                  })}
                </Button>
              )}
              ...
              {wo.status === "in_progress" && (
                <Card padding="sm">
                  <p className="text-xs font-semibold uppercase tracking-wide text-content-secondary mb-2">
                    {t("service.complete_wo", {
                      defaultValue: "Complete with debrief",
                    })}
                  </p>
                  <div className="space-y-2">
                    <textarea
                      ...
                      onChange={(e) =>
                        setDebrief((s) => ({ ...s, problem: e.target.value }))
                      }
                      placeholder={t("service.problem", {
                    ...
                    <textarea
                      value={debrief.cause}
                      onChange={(e) =>
                        setDebrief((s) => ({ ...s, cause: e.target.value }))
                      }
                      placeholder={t("service.cause", {
                        defaultValue: "Cause",
                      })}
                      rows={2}
                      className={clsx(inputCls, "h-auto py-2")}
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setDebrief((s) => ({ ...s, solution: e.target.value }))
                      }
                      placeholder={t("service.solution", {
          ...
          {contract && (
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              />
              <Field
                label={t("service.period_start", { defaultValue: "Start" })}
                value={contract.period_start}
              />
              <Field
                label={t("service.period_end", { defaultValue: "End" })}
                value={contract.period_end}
              />
              ...
              <Field
                ...
                value={
                  <MoneyDisplay
                    amount={Number(contract.value) || 0}
                    currency={contract.currency || undefined}
                  />
                }
              />
              <Field
                label={t("service.auto_renew", { defaultValue: "Auto renew" })}
          ...
          {asset && (
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <Field
                label={t("service.manufacturer", {
                  defaultValue: "Manufacturer",
                })}
                value={asset.manufacturer || "—"}
              />
              <Field
                label={t("service.model", { defaultValue: "Model" })}
                value={asset.model || "—"}
              />
              <Field
                label={t("service.serial", { defaultValue: "Serial" })}
                value={asset.serial || "—"}
              />
              <Field
              ...
              />
              <Field
                label={t("service.install_date", { defaultValue: "Installed" })}
                value={asset.install_date || "—"}
              />
              <Field
                label={t("service.warranty_until", {
...
function CreateModal({
  ...
  const submit = async () => {
    ...
    try {
      if (kind === "tickets") {
        ...
        addToast({
          type: "success",
          title: t("service.ticket_created", {
        ...
        addToast({
          type: "success",
          title: t("service.contract_created", {
            defaultValue: "Contract created",
          }),
        });
        ...
        addToast({
          type: "success",
          title: t("service.asset_created", { defaultValue: "Asset created" }),
        });
  ...
  const ASSET_TYPE_OPTIONS: Array<{ value: string; label: string }> = [
    {
      value: "hvac",
      label: t("service.asset_type_hvac", { defaultValue: "HVAC" }),
    },
    {
      value: "lift",
      label: t("service.asset_type_lift", { defaultValue: "Lift / elevator" }),
    },
    {
      value: "generator",
      label: t("service.asset_type_generator", { defaultValue: "Generator" }),
    },
    {
      value: "fire_protection",
      label: t("service.asset_type_fire", { defaultValue: "Fire protection" }),
    },
    {
      value: "plumbing",
      label: t("service.asset_type_plumbing", { defaultValue: "Plumbing" }),
    },
    {
      value: "electrical",
      label: t("service.asset_type_electrical", { defaultValue: "Electrical" }),
    },
    {
      value: "cctv",
      label: t("service.asset_type_cctv", {
        defaultValue: "CCTV / surveillance",
      }),
    },
    {
      value: "access_control",
      label: t("service.asset_type_access", { defaultValue: "Access control" }),
    },
    {
      value: "bms",
      label: t("service.asset_type_bms", { defaultValue: "BMS / automation" }),
    },
    {
      value: "other",
      label: t("service.asset_type_other", { defaultValue: "Other" }),
    },
  ];
  ...
  const titleByKind: Record<Tab, string> = {
    ...
    work_orders: t("service.new_work_order", {
      defaultValue: "New work order",
    }),
    contracts: t("service.new_contract", {
      defaultValue: "New service contract",
    }),
  ...
  const subtitleByKind: Record<Tab, string> = {
    tickets: t("service.new_ticket_subtitle", {
      defaultValue:
        "Log a customer request under one of the active service contracts.",
    }),
    work_orders: t("service.new_work_order_subtitle", {
      defaultValue:
        "Schedule a technician visit to deliver on a logged ticket.",
    }),
        ...
        <WideModalSection columns={2}>
          ...
          >
            <select
              ...
              onChange={(e) =>
                setTicketForm({ ...ticketForm, contract_id: e.target.value })
              }
              className={inputCls}
            >
              <option value="">
          ...
          <WideModalField
            ...
            hint={t("service.description_hint", {
              defaultValue:
                "Anything the technician needs to know before arriving.",
            })}
            span={2}
          >
            <textarea
          ...
          >
            ...
            >
              <option value="low">
                {t("service.priority_low", { defaultValue: "Low" })}
              </option>
              <option value="med">
                {t("service.priority_med", { defaultValue: "Medium" })}
              </option>
              <option value="high">
                {t("service.priority_high", { defaultValue: "High" })}
              </option>
              <option value="critical">
                {t("service.priority_critical", { defaultValue: "Critical" })}
              </option>
            </select>
          </WideModalField>
        </WideModalSection>
        ...
        <WideModalSection columns={2}>
          ...
          >
            <select
              ...
              onChange={(e) =>
                setWoForm({ ...woForm, ticket_id: e.target.value })
              }
              className={inputCls}
            >
              <option value="">
          ...
          >
            <input
              ...
              onChange={(e) =>
                setWoForm({ ...woForm, scheduled_for: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setWoForm({ ...woForm, technician_id: e.target.value })
              }
              className={inputCls}
      ...
      {kind === "contracts" && (
        <>
          ...
          >
            ...
            <WideModalField
              ...
              hint={t("service.contract_title_hint", {
                defaultValue:
                  'Short label shown in lists, e.g. "HVAC maintenance — HQ 2026".',
              })}
              span={2}
            >
              <input
          ...
          >
            ...
            <WideModalField
              label={t("service.period_end", { defaultValue: "Period end" })}
            >
              <input
                type="date"
            ...
            >
              <select
                ...
                onChange={(e) =>
                  setContractForm({ ...contractForm, sla_tier: e.target.value })
                }
                className={inputCls}
              >
                <option value="standard">
                  {t("service.sla_standard", {
                    defaultValue: "Standard (next business day)",
                  })}
                </option>
                <option value="priority">
                  {t("service.sla_priority", {
                    defaultValue: "Priority (same business day)",
                  })}
                </option>
                <option value="critical">
                  {t("service.sla_critical", {
                    defaultValue: "Critical (4 hours)",
                  })}
                </option>
                <option value="24x7">
                  {t("service.sla_24x7", { defaultValue: "24×7 emergency" })}
                </option>
              </select>
            </WideModalField>
          </WideModalSection>
        ...
        <WideModalSection columns={2}>
          ...
          >
            <select
              ...
              onChange={(e) =>
                setAssetForm({ ...assetForm, contract_id: e.target.value })
              }
              className={inputCls}
            >
              <option value="">
