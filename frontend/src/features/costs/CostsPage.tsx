    ...
    return (
      <div className="mb-6 rounded-xl border-2 border-dashed border-border-light bg-surface-secondary/50 p-6 text-center">
        ...
        <p className="text-sm font-medium text-content-primary mb-1">
          {t("costs.no_database_loaded", {
            defaultValue: "No database loaded‌⁠‍",
          })}
        </p>
        <p className="text-xs text-content-tertiary mb-3">
          {t("costs.import_first_hint", {
            defaultValue:
              "Import a regional cost database to start searching 55,000+ items.‌⁠‍",
          })}
        </p>
        <Button
          variant="primary"
...
export function CostsPage() {
    ...
    queryFn: async () => {
      ...
      if (semanticSearch && debouncedQuery.length >= 2) {
        try {
          ...
          if (region) params.set("region", region);
          const results = await apiGet<Array<Record<string, unknown>>>(
            `/v1/costs/vector/search/?${params}`,
          );
          ...
          return {
            items: results.map((r) => ({
              ...
              classification: (r.classification ?? {}) as Record<
                string,
                string
              >,
              components: [],
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("costs.item_deleted", { defaultValue: "Item deleted" }),
      });
    },
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("costs.delete_failed", { defaultValue: "Delete failed" }),
        message: err.message,
      });
    },
  });
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: () => {
      addToast({
        ...
        message: t("costs.export_success_msg", {
  ...
  return (
    <div className="w-full animate-fade-in">
      ...
      <div className="mb-5 flex items-start justify-between">
            ...
            <span>
              {regionInfo
                ? (() => {
                    ...
                    const tabCount =
                      regionStats?.find((r) => r.region === activeRegion)
                        ?.count ?? null;
                    const display =
                      total > 0
                        ? total
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className={
              showRegionalAdjust
                ? "border-oe-blue/40 text-oe-blue bg-oe-blue-subtle/20"
                : ""
            }
          >
            {t("costs.regional_adjust.toggle", {
              defaultValue: "Regional Adjust",
            })}
          </Button>
          <Button
            variant="secondary"
      ...
      <div className="mb-4 flex items-center gap-2">
        <button
          onClick={() =>
            setSpecialTab(specialTab === "favourites" ? "" : "favourites")
          }
        ...
        >
          <Star
            size={14}
            className={specialTab === "favourites" ? "fill-yellow-400" : ""}
          />
            ...
            <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-end">
              ...
              <div className="relative flex-1 flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder={
                      semanticSearch
                        ? t(
                            ...
                            "Describe what you need (AI finds similar)...",
                          )
                        : regionInfo
                          ? t("costs.search_in_region", {
                              defaultValue: "Search in {{name}}...",
                              name: regionInfo.name,
                            })
                          : t(
                              "costs.search_placeholder",
                              "Search by description or code...",
                            )
                    }
                    aria-label={t("costs.search_placeholder", {
                      defaultValue: "Search cost items",
                    })}
                    className={`h-10 w-full rounded-lg border bg-surface-primary pl-10 ${query ? "pr-8" : "pr-3"} text-sm text-content-primary placeholder:text-content-tertiary transition-all duration-fast ease-oe focus:outline-none focus:ring-2 focus:border-transparent hover:border-content-tertiary ${
                      semanticSearch
                        ? "border-purple-400 focus:ring-purple-400/30"
                ...
                <button
                  ...
                  title={
                    semanticSearch
                      ? t("costs.switch_to_text_search", {
                          defaultValue: "Switch to text search",
                        })
                      : t("costs.switch_to_ai_search", {
                          defaultValue: "Switch to AI semantic search",
                        })
                  }
              ...
              <div className="relative">
                ...
                >
                  <option value="">
                    {t("costs.all_sources", "All sources")}
                  </option>
          ...
          {activeFilterCount > 0 && (
            <div className="flex items-center gap-2 mb-4 -mt-3">
              <Badge variant="blue" size="sm">
                {activeFilterCount}{" "}
                {t("costs.filters_active", { defaultValue: "filters active" })}
              </Badge>
              <button
                      ...
                      <tr className="border-b border-border-light bg-surface-tertiary text-left">
                        <th className="px-2 py-3 w-16">
                          <div className="flex items-center gap-0.5">
                            <Star
                              size={14}
                              className="text-content-quaternary ml-1"
                            />
                            <button
                        ...
                        <th
                          className="px-4 py-3 font-medium text-content-secondary w-28 cursor-pointer select-none"
                        ...
                        <th
                          className="px-4 py-3 font-medium text-content-secondary cursor-pointer select-none"
                        ...
                        <th
                          className="px-4 py-3 font-medium text-content-secondary w-32 text-right cursor-pointer select-none"
            ...
            <Button
              ...
              onClick={() => {
                const text = selectedItems
                  .map(
                    (i) => `${i.code}\t${i.description}\t${i.unit}\t${i.rate}`,
                  )
                  .join("\n");
                navigator.clipboard
                  .writeText(text)
                  .then(() => {
                    addToast({
                      ...
                      message: t("costs.items_copied", {
                        defaultValue: "{{count}} items copied to clipboard",
                        count: selectedIds.size,
                      }),
                    });
                  })
                  .catch((err) => {
                    addToast({
                      ...
                      message: err?.message || "Clipboard access denied",
                    });
                  });
              }}
            >
...
function AddToBOQModal({
  ...
  const { data: sections } = useQuery({
    ...
    queryFn: async () => {
      const boqData = await apiGet<{ positions?: BOQSection[] }>(
        `/v1/boq/boqs/${boqId}`,
      );
    ...
    try {
      ...
      for (const item of items) {
        const section = String(
          Math.floor((nextOrdinal - 1) / 999) + 1,
        ).padStart(2, "0");
        ...
        if (item.components && item.components.length > 0) {
          ...
          if (m.labor_cost && m.labor_cost > 0) {
            synth.push({
              name: t("costs.component_labor", { defaultValue: "Labor" }),
              code: "",
              type: "labor",
              unit: item.unit,
              quantity: 1,
          ...
          if (m.material_cost && m.material_cost > 0) {
            synth.push({
              name: t("costs.component_material", { defaultValue: "Material" }),
              code: "",
              type: "material",
              unit: item.unit,
              quantity: 1,
          ...
          if (m.equipment_cost && m.equipment_cost > 0) {
            synth.push({
              name: t("costs.component_equipment", {
        ...
        if (meta.resources) {
          ...
          for (const r of meta.resources as Array<{
            type: string;
            total: number;
          }>) {
      ...
      addToast({
        type: "success",
        title: t("costs.items_added_to_boq", {
          defaultValue: "{{count}} items added to BOQ",
          count: items.length,
        }),
        message: t("costs.positions_created_hint", {
          defaultValue: "Positions created with unit rates from cost database",
        }),
      });
      onSuccess();
    } catch (err) {
      addToast({
        type: "error",
        title: t("costs.add_items_failed", {
          defaultValue: "Failed to add items",
        }),
        message:
    ...
    >
      ...
      >
          ...
          {boqId && sections && sections.length > 0 && (
            <div>
              <label className="text-xs font-medium text-content-secondary mb-1.5 block">
                {t("boq.section_optional", {
                  defaultValue: "Section (optional)",
                })}
              </label>
              <select
                value={sectionId}
              ...
              >
                <option value="">
                  {t("boq.no_section", "No section (top level)")}
                </option>
                {sections.map((s) => (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-surface-tertiary text-content-secondary">
                    ...
                    <th className="px-3 py-1.5 text-center font-medium w-14">
                      {t("boq.unit")}
                    </th>
                ...
                <tbody className="divide-y divide-border-light">
                  {items.slice(0, 10).map((item) => (
                    <tr key={item.id}>
                      <td className="px-3 py-1.5 text-content-primary truncate max-w-[250px]">
                        {item.description}
                      </td>
                      <td className="px-3 py-1.5 text-center text-content-tertiary">
                        {item.unit}
                      </td>
                      <td className="px-3 py-1.5 text-right tabular-nums font-medium text-content-primary">
                        {fmt(item.rate)}
                      </td>
                    </tr>
                  ))}
                  {items.length > 10 && (
                    <tr>
                      <td
                        colSpan={3}
                        className="px-3 py-1.5 text-center text-content-quaternary"
                      >
                        {t("costs.and_n_more", {
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border-light bg-surface-secondary/30">
          <span className="text-xs text-content-tertiary">
            {t("costs.n_positions_will_be_created", {
              defaultValue: "{{count}} positions will be created",
              count: items.length,
            })}
          </span>
          <div className="flex items-center gap-2">
...
function CreateAssemblyFromCostsModal({
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        type: "success",
        title: t("assemblies.assembly_created", {
      ...
      addToast({
        type: "error",
        title: t("assemblies.create_failed", {
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-light">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2
                id="create-assembly-modal-title"
                className="text-base font-semibold text-content-primary"
              >
                {t("assemblies.create_assembly", {
                  defaultValue: "Create Assembly",
              ...
              <p className="text-xs text-content-tertiary">
                {t("costs.n_cost_items_to_recipe", {
                  defaultValue: "{{count}} cost items → reusable recipe",
                  count: items.length,
                })}
              </p>
            </div>
        ...
        <div className="px-6 py-4 space-y-4">
          <div>
            <label className="text-xs font-medium text-content-secondary mb-1.5 block">
              {t("assemblies.assembly_name", { defaultValue: "Assembly Name" })}
            </label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder={t("assemblies.assembly_name_placeholder", {
                defaultValue: "e.g. Reinforced Concrete Wall C30/37 24cm",
              })}
          ...
          <div>
            ...
            <div className="rounded-lg border border-border-light overflow-hidden max-h-40 overflow-y-auto">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="flex items-center justify-between px-3 py-2 text-xs border-b border-border-light/50 last:border-0"
                >
                  <span className="text-content-primary truncate flex-1 mr-2">
                    {item.description || item.code}
                  </span>
                  <span className="text-content-secondary shrink-0 tabular-nums">
                    {fmt(item.rate)} / {item.unit}
                  </span>
                </div>
              ))}
            </div>
            <div className="flex items-center justify-between mt-2 text-xs">
              <span className="text-content-tertiary">
                {t("assemblies.total_rate_sum", {
                  defaultValue: "Total rate (sum of components)",
                })}
              </span>
...
function CreateCostItemModal({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const code =
        form.code.trim() || `CUSTOM-${Date.now().toString(36).toUpperCase()}`;
      ...
      addToast({
        type: "success",
        title: t("costs.item_created", { defaultValue: "Cost item created" }),
      });
      onCreated();
    } catch (err) {
      addToast({
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-light">
          <div>
            <h2
              id="create-cost-item-modal-title"
              className="text-base font-semibold text-content-primary"
            >
              {t("costs.create_item", { defaultValue: "Add Custom Cost Item" })}
            </h2>
            <p className="text-xs text-content-tertiary">
              {t("costs.create_item_desc", {
                defaultValue: "Create your own cost item for this project",
              })}
            </p>
          </div>
          <button
        ...
        <div className="px-6 py-4 space-y-3">
          <div>
            <label className="text-xs font-medium text-content-secondary mb-1 block">
              {t("costs.code", "Code")}
              <span className="text-content-quaternary ml-1">
                ({t("costs.optional", "optional")})
              </span>
            </label>
            <input
              type="text"
              ...
              placeholder={t("costs.code_placeholder", {
                defaultValue: "e.g. WALL-001",
              })}
          ...
          <div>
            ...
            <input
              ...
              placeholder={t("costs.description_placeholder", {
                defaultValue: "e.g. Reinforced concrete wall C30/37, 25cm",
              })}
              className="h-9 w-full rounded-lg border border-border bg-surface-primary px-3 text-sm focus:outline-none focus:ring-2 focus:ring-oe-blue"
            />
          </div>
        ...
        <div className="flex items-center justify-end gap-2 px-6 py-3 border-t border-border-light bg-surface-secondary/30">
          ...
          >
            {isSubmitting
              ? t("costs.creating", { defaultValue: "Creating..." })
              : t("costs.create", { defaultValue: "Create Item" })}
          </Button>
        </div>
      </div>
    </div>
...
function CostVariantDetail({
  ...
  const visible = useMemo(() => {
    ...
    if (
      medianIdx >= 0 &&
      medianIdx < sorted.length &&
      !indices.includes(medianIdx)
    ) {
    ...
    return indices
      ...
      .filter(
        (x): x is { v: import("./api").CostVariant; i: number } => x !== null,
      );
    ...
    <div className="mb-4 rounded-lg border border-oe-blue-subtle bg-oe-blue-subtle/10 p-2 animate-fade-in">
      ...
      <div className="mb-2 flex flex-wrap items-center gap-x-1 gap-y-1 text-2xs tabular-nums text-content-secondary">
        <span className="rounded bg-surface-primary/70 px-1.5 py-0.5">
          <span className="text-content-tertiary">
            {t("costs.variant_min", { defaultValue: "Min" })}
          </span>
        ...
        <span className="rounded bg-surface-primary/70 px-1.5 py-0.5">
          <span className="text-content-tertiary">
            {t("costs.variant_median", { defaultValue: "Median" })}
          </span>
          <span className="ml-1 font-semibold text-content-primary">
            {fmt(stats.median)}
          </span>
        ...
        <span className="rounded bg-surface-primary/70 px-1.5 py-0.5">
          <span className="text-content-tertiary">
            {t("costs.variant_mean", { defaultValue: "Mean" })}
          </span>
          <span className="ml-1 font-semibold text-content-primary">
            {fmt(stats.mean)}
          </span>
        ...
        <span className="rounded bg-surface-primary/70 px-1.5 py-0.5">
          <span className="text-content-tertiary">
            {t("costs.variant_max", { defaultValue: "Max" })}
          </span>
          <span className="ml-1 font-semibold text-content-primary">
            {fmt(stats.max)}
          </span>
          ...
          <>
            ...
            <span className="rounded bg-surface-primary/70 px-1.5 py-0.5">
              <span className="font-semibold text-content-primary">
                {stats.position_count.toLocaleString()}
              </span>
              <span className="ml-1 text-content-tertiary">
                {t("costs.variant_position_count_label", {
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-surface-tertiary text-left">
              <th className="px-2 py-1 w-10 text-center text-2xs font-medium text-content-secondary">
                #
              </th>
            ...
            {visible.map(({ v, i }) => {
              ...
              return (
                <tr
                  ...
                  className={
                    isMedian
                      ? "bg-oe-blue-subtle/15"
                      : "hover:bg-surface-secondary/30"
                  }
                >
                  <td className="px-2 py-1 text-center font-mono text-2xs text-content-tertiary">
                    {v.index + 1}
                  </td>
                  <td className="px-2 py-1 text-content-primary">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isMedian && (
                        ...
                        >
                          {t("costs.variant_default_median_chip", {
                            defaultValue: "Median",
                          })}
                        </Badge>
                      )}
...
function CostItemRow({
  ...
  const materials = (detail.components ?? []).filter(
    (c) => c.type === "material",
  );
  const machines = (detail.components ?? []).filter(
    (c) =>
      c.type === "equipment" ||
      c.type === "operator" ||
      c.type === "electricity",
        ...
        <td className="px-2 py-3 w-10">
          <div className="flex items-center gap-0.5">
            <button
              ...
              title={
                isFavourite
                  ? t("costs.remove_from_favourites", {
                      defaultValue: "Remove from favourites",
                    })
                  : t("costs.add_to_favourites", {
        ...
        <td className="px-4 py-3 text-right font-semibold text-content-primary tabular-nums">
          <div className="inline-flex items-center gap-1.5">
            ...
            {hasVariants && variantStats && (
              ...
              >
                <span
                  title={`${fmt(variantStats.min)} – ${fmt(variantStats.max)}`}
                >
              ...
              <button
                onClick={async (e) => {
                  ...
                  const ok = await confirm({
                    title: t("costs.confirm_delete_title", {
                      defaultValue: "Delete cost item?",
                    }),
                    message: t("costs.confirm_delete", {
                      defaultValue: "Delete this custom cost item?",
                    }),
                  });
            ...
            <div className="bg-surface-secondary/30 border-t border-b border-border-light px-6 py-4 animate-fade-in">
                  ...
                  {[
                    ...
                    .map((part, i, arr) => (
                      ...
                      >
                        ...
                        {i < arr.length - 1 && (
                          <span className="text-2xs text-content-quaternary/50">
                            &rsaquo;
                          </span>
                        )}
                      </span>
                      ...
                      <div className="h-2 w-full rounded-full overflow-hidden flex bg-surface-tertiary">
                        ...
                        {equipmentCost > 0 && (
                          <div
                            ...
                            style={{
                              width: `${(equipmentCost / item.rate) * 100}%`,
                            }}
                        ...
                        {materialCost > 0 && (
                          <div
                            ...
                            style={{
                              width: `${(materialCost / item.rate) * 100}%`,
                            }}
                      ...
                      <div className="flex gap-4 mt-1.5 text-2xs text-content-tertiary">
                        ...
                        {equipmentCost > 0 && (
                          <span className="flex items-center gap-1">
                            ...
                            Equipment{" "}
                            {((equipmentCost / item.rate) * 100).toFixed(0)}%
                          </span>
                        )}
                        {materialCost > 0 && (
                          <span className="flex items-center gap-1">
                            ...
                            Materials{" "}
                            {((materialCost / item.rate) * 100).toFixed(0)}%
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <table className="w-full text-xs table-fixed">
                      ...
                      <tbody className="divide-y divide-border-light">
                        {detail.components.map((comp, i) => {
                          ...
                          const typeColor =
                            TYPE_COLOR_MAP[comp.type] ||
                            "text-gray-600 bg-gray-50";
                          const typeLabel = t(`costs.component_${comp.type}`, {
                            defaultValue:
                          ...
                          return (
                            <tr
                              key={`${comp.name}-${comp.type}-${i}`}
                              className="hover:bg-surface-secondary/30"
                            >
                              <td
                                className="px-3 py-2 text-content-primary truncate"
                                title={comp.name}
                              >
                                {comp.name}
                              </td>
                              <td className="px-3 py-2">
                                <span
                                  className={`inline-block text-2xs font-medium px-1.5 py-0.5 rounded ${typeColor}`}
                                >
                                  {typeLabel}
                                </span>
                              </td>
              ...
              <details className="mt-4">
                <summary className="text-2xs font-medium text-content-tertiary cursor-pointer hover:text-content-secondary transition-colors select-none">
                  All properties (
                  {Object.keys(cls).length + Object.keys(meta).length + 5}{" "}
                  fields)
                </summary>
                <div className="mt-2 grid grid-cols-2 gap-x-6 gap-y-1.5 text-2xs">
                  {/* Basic fields */}
                  <div className="flex justify-between">
                    <span className="text-content-quaternary">Code</span>
                    <span className="text-content-secondary font-mono">
                      {item.code}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-quaternary">Unit</span>
                    <span className="text-content-secondary">{item.unit}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-quaternary">Rate</span>
                    <span className="text-content-secondary font-semibold">
                      {fmt(item.rate)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-quaternary">Region</span>
                    <span className="text-content-secondary">
                      {item.region || "—"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-content-quaternary">Source</span>
                    <span className="text-content-secondary">
                      {item.source}
                    </span>
                  </div>
                  ...
                  {Object.entries(cls).map(([k, v]) => (
                    <div key={k} className="flex justify-between">
                      ...
                      <span
                        className="text-content-secondary truncate ml-2 max-w-[200px]"
                        title={String(v)}
                      >
                        {String(v)}
                      </span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-content-quaternary">Components</span>
                    <span className="text-content-secondary">
                      {item.components_count ?? detail.components?.length ?? 0}{" "}
                      resources
                    </span>
                  </div>
                </div>
