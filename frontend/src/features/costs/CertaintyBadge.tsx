...
import {
  fetchCertainty,
  type CertaintyBadge as CertaintyBadgeData,
} from "./api";
...
export function CertaintyBadge({
  ...
  if (isLoading) {
    ...
    return (
      <span
        ...
        aria-label={t("costs.certainty.loading", {
          defaultValue: "Loading certainty…",
        })}
      />
    );
  }
  ...
  const tooltip = t("costs.certainty.tooltip", {
    defaultValue: "Used {{frequency}}× · last {{age}} · source {{source}}",
