        ...
        <nav className="flex gap-1 -mb-px" role="tablist">
          {(
            [
              {
                id: "assets",
                label: t("equipment.tab_assets", { defaultValue: "Assets" }),
                icon: Truck,
              },
              {
                id: "types",
                label: t("equipment.tab_types", { defaultValue: "Types" }),
                icon: Tags,
              },
      ...
      {pageTab === "types" ? (
        ...
        <>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <option value="">
                {t("equipment.all_ownership", {
                  defaultValue: "All ownership",
                })}
              </option>
...
function DetailDrawer({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !deleting && !editOpen && !deleteOpen) {
        e.preventDefault();
        onClose();
      }
    };
        ...
        {!eq && !eqQ.isLoading && (
          <div className="p-5">
            <EmptyState
              ...
              description={eqQ.isError ? getErrorMessage(eqQ.error) : undefined}
              action={
                eqQ.isError
                  ? {
                      label: t("common.retry", { defaultValue: "Retry" }),
...
function MaintenanceTab({
  ...
  const invalidate = () =>
    qc.invalidateQueries({
      queryKey: ["equipment", "workOrders", equipmentId],
    });
  ...
  return (
    <div className="space-y-3">
      <SectionHeader
        ...
        addLabel={t("equipment.workorder.add", {
          defaultValue: "Add work order",
        })}
  ...
  return (
    <div className="space-y-3">
      ...
      {!loading && rows.length === 0 && (
        <EmptyState
          icon={<AlertTriangle size={20} />}
          title={t("equipment.no_damage", {
            defaultValue: "No damage reports",
          })}
        />
      )}
      {!loading &&
...
function _toPayload(form: EquipmentFormState): CreateEquipmentPayload {
        ...
        <div className="space-y-4">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                <label className={labelCls}>
                  {t("equipment.odometer_km", {
                    defaultValue: "Odometer (km)",
                  })}
                </label>
                <input
                  type="text"
