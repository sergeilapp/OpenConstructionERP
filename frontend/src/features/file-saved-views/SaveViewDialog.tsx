...
export function SaveViewDialog({
  ...
  const submit = async () => {
    ...
    if (!trimmed) {
      setError(
        t("files.views.error_name_required", {
  ...
  return (
    <WideModal
      open={open}
      onClose={onClose}
      title={t("files.views.dialog_title", {
        defaultValue: "Save current filter as view",
      })}
      subtitle={t("files.views.dialog_subtitle", {
        defaultValue:
          "Re-apply this filter from the saved-views rail with one click.",
      })}
      size="md"
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2 text-sm">
          <label className="flex items-center gap-2">
            ...
            />
            <span>
              {t("files.views.dialog_pin", {
                defaultValue: "Pin to top of rail",
              })}
            </span>
          </label>
          {projectId && (
