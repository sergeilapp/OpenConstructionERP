...
describe("SavedViewsRail", () => {
  ...
  it("clicking duplicate calls the duplicate mutation", async () => {
    duplicateSavedViewMock.mockResolvedValue(
      view({ id: "dup", name: "Drawings (copy)" }),
    );
  ...
  it("disables write actions for views the caller does not own", async () => {
    ...
    fireEvent.contextMenu(row, { clientX: 10, clientY: 20 });
    expect(
      (screen.getByTestId("saved-view-action-rename") as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    expect(
      (screen.getByTestId("saved-view-action-pin") as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    expect(
      (screen.getByTestId("saved-view-action-share") as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    expect(
      (screen.getByTestId("saved-view-action-delete") as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    // Duplicate is always allowed — copy lands in the user's own list.
    expect(
      (screen.getByTestId("saved-view-action-duplicate") as HTMLButtonElement)
        .disabled,
    ).toBe(false);
  });
});
