...
export function SavedViewsRail({
  ...
  const targetView = useMemo<SavedViewResponse | null>(
    () =>
      contextMenu
        ? (views.find((v) => v.id === contextMenu.viewId) ?? null)
        : null,
    [contextMenu, views],
  );
  ...
  const handleRename = async (view: SavedViewResponse) => {
    const next = window.prompt(
      t("files.views.rename_prompt", { defaultValue: "New name" }),
      view.name,
    );
    if (next === null) return;
    ...
    <div className="flex flex-col" onClick={closeMenu}>
      ...
      {expanded && (
        <ul
          ...
          data-testid="saved-views-rail-list"
        >
          {isLoading && (
              ...
              >
                ...
                {view.is_shared && (
                  <Users
                    ...
                    aria-label={t("files.views.shared_label", {
                      defaultValue: "Shared",
                    })}
                  />
                )}
      ...
      {contextMenu && targetView && (
        ...
        >
          ...
          <MenuButton
            ...
            label={
              targetView.is_shared
                ? t("files.views.action_unshare", {
                    defaultValue: "Stop sharing",
                  })
                : t("files.views.action_share", {
                    defaultValue: "Share with project",
                  })
            }
          ...
          <MenuButton
            ...
            label={t("files.views.action_duplicate", {
