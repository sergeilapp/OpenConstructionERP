...
const BUILTIN_TASK_TYPES: BuiltinTaskType[] = [
  "task",
  "topic",
  "information",
  "decision",
...
const CUSTOM_CATEGORY_COLORS = [
  ...
  {
    bg: "bg-lime-100 text-lime-700 dark:bg-lime-900/40 dark:text-lime-300",
    hex: "#84cc16",
  },
  {
  ...
  {
    bg: "bg-fuchsia-100 text-fuchsia-700 dark:bg-fuchsia-900/40 dark:text-fuchsia-300",
    hex: "#d946ef",
  },
];
...
const BUILTIN_STATUSES: TaskStatus[] = [
  "draft",
  "open",
  "in_progress",
  "completed",
...
function getTypeColor(
  ...
  if (custom)
    return CUSTOM_CATEGORY_COLORS[
      custom.colorIdx % CUSTOM_CATEGORY_COLORS.length
    ]!.bg;
...
const PRIORITY_BADGE: Record<
  TaskPriority,
  { variant: "neutral" | "blue" | "warning" | "error"; cls: string }
> = {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-lg animate-fade-in">
      <div
        ...
        role="dialog"
        aria-label={t("tasks.new_task", { defaultValue: "New Task‌⁠‍" })}
      >
        {/* Header */}
        ...
        <div className="px-6 py-4 space-y-5">
                    ...
                    className={clsx(
                      ...
                      selected
                        ? (BUILTIN_TYPE_COLOR[tt] ?? "") +
                            " border-current ring-2 ring-oe-blue/30"
              ...
              {customCategories.map((cat) => {
                ...
                const colorCls =
                  CUSTOM_CATEGORY_COLORS[
                    cat.colorIdx % CUSTOM_CATEGORY_COLORS.length
                  ]?.bg ?? "";
                return (
                  <button
                    key={cat.name}
          ...
          <div>
            ...
            />
            {errors.title && (
              <p className="mt-1 text-xs text-semantic-error">{errors.title}</p>
            )}
          </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border-light">
          ...
          >
            ...
            <span>
              {t("tasks.create_task", { defaultValue: "Create Task" })}
            </span>
          </Button>
        </div>
      </div>
...
const TaskCard = React.memo(function TaskCard({
  ...
  const pb =
    PRIORITY_BADGE[task.priority as TaskPriority] ?? PRIORITY_BADGE.normal;
  ...
  return (
    ...
    >
      ...
      {(task.meeting_id ||
        (task.metadata && typeof task.metadata.source === "string") ||
        ...
        <div className="flex items-center gap-0 opacity-0 group-hover:opacity-100 transition-opacity">
          {task.status !== "completed" ? (
            <>
              <Button
                ...
                className="!p-0.5 text-green-600 hover:text-green-700 h-auto"
                title={t("tasks.mark_complete", { defaultValue: "Complete" })}
              >
              ...
              <select
                ...
                aria-label={t("tasks.change_status", {
          ...
          {task.status !== "completed" && (
            <Button
              ...
              size="sm"
              onClick={() => onEdit(task)}
              className="!p-0.5 text-content-quaternary hover:text-oe-blue h-auto"
          ...
          <Button
            ...
            className="!p-0.5 text-content-quaternary hover:text-red-500 h-auto"
          >
            <Trash2 size={10} />
          </Button>
        </div>
...
export function TasksPage() {
  ...
  const [customStatuses, setCustomStatuses] =
    useState<CustomStatus[]>(loadCustomStatuses);
  ...
  const handleAddStatus = useCallback(() => {
    ...
    if (
      !slug ||
      BUILTIN_STATUSES.includes(slug as TaskStatus) ||
      customStatuses.some((s) => s.name === slug)
    ) {
      addToast({
        type: "warning",
        title: t("tasks.status_exists", {
          defaultValue: "This status already exists",
        }),
      });
      return;
    }
    const updated = [
      ...customStatuses,
      { name: slug, label, colorIdx: newStatusColorIdx },
    ];
    ...
    addToast({
      type: "success",
      title: t("tasks.status_created", {
        defaultValue: 'Column "{{name}}" created',
        name: label,
      }),
    });
  ...
  const handleAddCategory = useCallback(() => {
    ...
    // Avoid duplicates
    if (
      BUILTIN_TASK_TYPES.includes(slug as BuiltinTaskType) ||
      customCategories.some((c) => c.name === slug)
    ) {
      addToast({
        type: "warning",
        title: t("tasks.category_exists", {
    ...
    const updated = [
      ...customCategories,
      { name: slug, label, colorIdx: newCategoryColorIdx },
    ];
    setCustomCategories(updated);
    saveCustomCategories(updated);
    setNewCategoryName("");
    setNewCategoryColorIdx(
      (prev) => (prev + 1) % CUSTOM_CATEGORY_COLORS.length,
    );
    setShowAddCategory(false);
    addToast({
      type: "success",
      title: t("tasks.category_created", {
        defaultValue: 'Category "{{name}}" created',
        name: label,
      }),
    });
  ...
  const exportMut = useMutation({
    ...
    onSuccess: () =>
      addToast({
        type: "success",
        title: t("tasks.export_success", {
          defaultValue: "Tasks exported successfully",
        }),
        message: t("tasks.export_success_msg", {
    ...
    onError: (e: Error) =>
      addToast({
        type: "error",
        title: t("tasks.export_failed", {
          defaultValue: "Failed to export tasks",
        }),
        message: e.message,
      }),
  });
    ...
    onSuccess: () => {
      ...
      addToast({
        ...
        title: t("tasks.completed", {
          defaultValue: "Task marked as completed",
        }),
      });
    },
    onError: (e: Error) =>
  ...
  const handleCreateSubmit = useCallback(
    (formData: TaskFormData) => {
      if (!projectId) {
        addToast({
          type: "error",
          title: t("tasks.no_project_error", {
  ...
  const editMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("tasks.updated", { defaultValue: "Task updated" }),
      });
    },
    onError: () => {
      addToast({
        type: "error",
        title: t("tasks.update_failed", { defaultValue: "Update failed" }),
      });
    },
  });
  ...
  const handleComplete = useCallback(
    async (id: string) => {
      const ok = await confirm({
        ...
        message: t("tasks.confirm_complete_msg", {
          defaultValue: "This task will be marked as completed.",
        }),
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("tasks.deleted", { defaultValue: "Task deleted" }),
      });
    },
    onError: () => {
      addToast({
        type: "error",
        title: t("tasks.delete_failed", { defaultValue: "Delete failed" }),
      });
    },
  });
    ...
    async (id: string) => {
      const ok = await confirm({
        ...
        message: t("tasks.confirm_delete_msg", {
          defaultValue: "This task will be permanently deleted.",
        }),
  ...
  const statusMut = useMutation({
    ...
    onError: (_err, _vars, ctx) => {
      ...
      addToast({
        type: "error",
        title: t("tasks.status_update_failed", {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const response = await fetch("/api/v1/tasks/template/", {
        method: "GET",
        headers,
      });
      ...
      addToast({
        type: "error",
        title: t("tasks.template_download_failed", {
          defaultValue: "Failed to download template",
        }),
        message:
          e instanceof Error
            ? e.message
            : t("tasks.template_error_generic", {
                defaultValue:
                  "An unexpected error occurred while downloading the import template.",
              }),
      });
    }
  };
    ...
    <div className="w-full animate-fade-in">
      ...
      <div className="mb-3 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-content-secondary">
            {t("tasks.subtitle", {
              defaultValue:
                "Track assignments, deadlines, and progress across your team",
            })}
          </p>
        </div>
      ...
      {projectId || myTasksOnly ? (
                ...
                <div className="absolute left-0 top-full mt-1 z-20 w-64 rounded-xl border border-border bg-surface-elevated shadow-lg p-3 animate-fade-in">
                  ...
                  <input
                    ...
                    placeholder={t("tasks.category_name_placeholder", {
                      defaultValue: "e.g. Safety, QA Review...",
                    })}
          ...
          >
            {isLoading ? (
                ...
                description={
                  searchQuery || typeFilter || myTasksOnly
                    ...
                    : t("tasks.no_tasks_hint", {
                        defaultValue:
                          "Create your first task to track assignments, deadlines, and progress across your team.",
                      })
                }
                action={
                  !searchQuery && !typeFilter && !myTasksOnly
              ...
              <div
                ...
                role="list"
                aria-label={t("tasks.kanban_board", {
              ...
              >
                {allStatuses.map((status) => {
                  ...
                  const isDropTarget =
                    draggedTaskId != null && dropTargetStatus === status;
                  const isBuiltin = BUILTIN_STATUSES.includes(
                  ...
                  const headerCls =
                    STATUS_HEADER_CLS[status as TaskStatus] ??
                    (customStatus
                      ? (CUSTOM_CATEGORY_COLORS[
                          customStatus.colorIdx % CUSTOM_CATEGORY_COLORS.length
                  ...
                  const isColumnDropTarget =
                    draggedColumn != null && columnDropTarget === status;
                  ...
                  return (
                    <div
                      ...
                      onDrop={(e) => {
                        if (
                          e.dataTransfer.types.includes("application/x-column")
                        ) {
                      ...
                      role="group"
                      aria-label={t("tasks.column_label", {
                        defaultValue: "{{name}} column, {{count}} tasks",
                        name: displayLabel,
                    ...
                    >
                      ...
                      <div
                        draggable={!isBuiltin}
                        onDragStart={(e) =>
                          handleColumnReorderDragStart(e, status)
                        }
                        className={clsx(
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {!isBuiltin && (
                            <button
                              ...
                              title={t("tasks.remove_column", {
                                defaultValue: "Remove column",
                              })}
                              aria-label={t("tasks.remove_column_named", {
                                defaultValue: "Remove {{name}} column",
                                name: displayLabel,
                              })}
                            >
                              <X size={10} />
                      ...
                      <div
                        className={clsx(
                          ...
                          isDropTarget &&
                            "bg-oe-blue/5 border-2 border-dashed border-oe-blue/30",
                        )}
                  ...
                  {showAddStatus ? (
                    <div className="rounded-lg border-2 border-dashed border-oe-blue/30 bg-oe-blue/5 p-3 space-y-2">
                      <input
                        ...
                        placeholder={t("tasks.new_column_name", {
                          defaultValue: "Column name...",
                        })}
                        aria-label={t("tasks.new_column_name", {
                          defaultValue: "Column name...",
                        })}
                        ...
                        {CUSTOM_CATEGORY_COLORS.map((c, i) => (
                          <button
                            ...
                            className={clsx(
                              ...
                              newStatusColorIdx === i
                                ? "scale-125 border-content-primary"
                                : "border-transparent",
                            )}
                            style={{ backgroundColor: c.hex }}
                          />
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 text-[10px] font-semibold py-1.5 rounded-md bg-oe-blue text-white hover:bg-oe-blue-dark transition-colors"
                        >
                          {t("common.add", { defaultValue: "Add" })}
                        </button>
                        <button
                          onClick={() => setShowAddStatus(false)}
                          className="flex-1 text-[10px] font-semibold py-1.5 rounded-md bg-surface-secondary text-content-secondary hover:bg-surface-tertiary transition-colors"
                        >
                          {t("common.cancel", { defaultValue: "Cancel" })}
                        </button>
                      </div>
        ...
        <EmptyState
          ...
          description={t("tasks.select_project", {
            defaultValue:
              "Select a project from the header to view and manage tasks, assignments, and deadlines.",
          })}
        />
      )}
            ...
            editingTask
              ...
              : typeFilter
                ? // When the create dialog opens while a type tab is active
                  ...
                  {
                    ...EMPTY_FORM,
                    task_type: typeFilter as TaskFormData["task_type"],
                  }
                : null
          }
          customCategories={customCategories}
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-lg animate-fade-in">
          <div
            ...
            role="dialog"
            aria-label={t("tasks.import_tasks", {
              defaultValue: "Import Tasks",
            })}
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-border-light">
              <h2 className="text-lg font-semibold text-content-primary">
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="flex items-center gap-2 rounded-lg bg-surface-secondary p-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-content-secondary">
                    {t("tasks.import_template_hint", {
                      defaultValue:
                        "Download the import template to see the expected format.",
                    })}
                  </p>
                </div>
                <Button
              ...
              >
                ...
                <p className="text-xs text-content-quaternary mt-1">
                  {t("tasks.import_columns_hint", {
                    defaultValue:
                      "Columns: Title, Type, Status, Priority, Due Date, Description",
                  })}
                </p>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border-light">
              ...
              {!importResult && (
                ...
                >
                  ...
                  <span>
                    {t("tasks.import_btn", { defaultValue: "Import" })}
                  </span>
                </Button>
              )}
            </div>
