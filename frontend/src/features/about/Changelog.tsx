...
const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: "2026-05-17",
    changes: [
      "Fixed: project photos and file thumbnails now load reliably",
      "Added: demo projects now include a Revit structural model next to the IFC model",
    ],
  },
  {
    version: "3.3.0",
    date: "2026-05-16",
    changes: [
      "Added: reuse a BOQ code to create a linked copy — edit the master once and every copy updates, while each keeps its own quantity",
      'Added: "Show linked positions" panel with one-click unlink, plus master/instance badges in the grid',
      "Fixed: unlinking a position no longer fails",
      "Fixed: a broad correctness pass across BOQ, costs, schedule, CAD/BIM and takeoff",
    ],
  },
  {
    version: "2.5.0",
  ...
  },
  {
    version: "1.3.19",
    date: "2026-04-10",
    changes: ["Fix: BIM camera correctly frames model on first load"],
  },
  {
    version: "1.3.18",
    date: "2026-04-10",
          ...
          {CHANGELOG.map((entry, idx) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 ${isCurrent ? "bg-emerald-50 border-emerald-500 dark:bg-emerald-900/20" : "bg-oe-blue/10 border-oe-blue"}`}
                >
                  {isCurrent && (
                    <span className="absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-30 animate-ping" />
                  )}
                  <div
                    className={`h-2.5 w-2.5 rounded-full ${isCurrent ? "bg-emerald-500" : "bg-oe-blue"}`}
                  />
                </div>
