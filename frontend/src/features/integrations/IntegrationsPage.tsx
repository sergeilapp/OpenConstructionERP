...
const CONNECTORS: ConnectorDef[] = [
  // ── Notifications ──────────────────────────────────────────────────
  {
    ...
    defaultDesc:
      "Send notifications to your Teams channel via Incoming Webhook",
    icon: MessageSquare,
    color: "bg-[#6264A7]",
  ...
  },
  {
    ...
    defaultDesc:
      "Self-hosted workflow automation. Use our webhook URL as a trigger node.",
    icon: Workflow,
    color: "bg-[#EA4B71]",
    category: "automation",
    status: "info_only",
  ...
  },
  {
    ...
    defaultDesc:
      "Connect 5,000+ apps. Use our webhook events as a Zapier trigger.",
    icon: Zap,
    color: "bg-[#FF4A00]",
    category: "automation",
    status: "info_only",
  ...
  },
  {
    ...
    defaultDesc:
      "Export BOQ and cost data in formats compatible with Google Sheets",
    icon: Sheet,
    color: "bg-[#0F9D58]",
    category: "data",
    status: "info_only",
  ...
  },
  {
    ...
    defaultDesc:
      "Connect BI tools to our REST API for custom dashboards and analytics",
    icon: BarChart3,
    color: "bg-[#F2C811]",
    category: "data",
    status: "info_only",
...
> = {
  notifications: {
    key: "integrations.cat_notifications",
    defaultLabel: "Notifications",
  },
  automation: {
    key: "integrations.cat_automation",
...
function ConnectModal({
  ...
  const selectAllEvents = useCallback(() => {
    if (connector.eventOptions) {
      setSelectedEvents((prev) =>
        prev.length === connector.eventOptions!.length
          ? []
          : [...connector.eventOptions!],
      );
    }
      ...
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl bg-surface-primary p-6 shadow-xl">
        ...
        {testResult && (
          ...
          >
            {testResult.success
              ? t(
                  "integrations.test_success_msg",
                  "Connection test passed successfully!",
                )
              : testResult.message ||
                t(
                  "integrations.test_failed_msg",
...
export function IntegrationsPage() {
  ...
  const [connectingType, setConnectingType] = useState<ConnectorDef | null>(
    null,
  );
  ...
  const configsByType = configs.reduce<Record<string, IntegrationConfig[]>>(
    (acc, c) => {
  ...
  const deleteMut = useMutation({
    ...
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("integrations.disconnect_failed", {
  ...
  return (
    <div className="w-full animate-fade-in">
      ...
      {CATEGORY_ORDER.map((category) => {
        const categoryConnectors = CONNECTORS.filter(
                          ...
                          <button
                            onClick={async () => {
                              const ok = await confirm({
                                title: t(
                                  "integrations.confirm_disconnect_title",
                                  {
                                    defaultValue: "Disconnect integration?‌⁠‍",
                                  },
                                ),
                                message: t(
