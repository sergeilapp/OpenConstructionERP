...
function fmtDays(
  ...
  if (n === null || n === undefined) return "—";
  return t("risk.montecarlo.days_value", {
...
export function MonteCarloTab({ projectId, currency }: MonteCarloTabProps) {
  ...
  const showCostChips =
    result && (result.mode === "cost" || result.mode === "both");
  const showScheduleChips =
    result && (result.mode === "schedule" || result.mode === "both");
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap items-end gap-4">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (Number.isFinite(v)) {
                  setIterations(
                    Math.max(ITERATIONS_MIN, Math.min(v, ITERATIONS_MAX)),
                  );
                }
              }}
          ...
          <div>
            ...
            >
              <option value="both">
                {t("risk.montecarlo.mode_both", {
                  defaultValue: "Cost + Schedule",
                })}
              </option>
              <option value="cost">
              ...
              <option value="schedule">
                {t("risk.montecarlo.mode_schedule", {
                  defaultValue: "Schedule only",
                })}
              </option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-content-primary">
              {t("risk.montecarlo.last_run", {
                defaultValue:
                  "Last run — {{risks}} risks, {{iterations}} iterations",
          ...
          {result.risk_count === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {showCostChips && (
                <>
                  <Chip
                    label={t("risk.montecarlo.p50_cost", {
                      defaultValue: "P50 Cost",
                    })}
                    value={fmtCurrencyOrPlain(result.p50_cost, currency)}
                    tone="success"
                  />
                  <Chip
                    label={t("risk.montecarlo.p80_cost", {
                      defaultValue: "P80 Cost",
                    })}
                    value={fmtCurrencyOrPlain(result.p80_cost, currency)}
                    tone="warning"
                  />
                  <Chip
                    label={t("risk.montecarlo.p95_cost", {
                      defaultValue: "P95 Cost",
                    })}
      ...
      {result && tornadoData.length > 0 && (
        <Card className="p-4">
          ...
          <ResponsiveContainer
            width="100%"
            height={Math.max(200, tornadoData.length * 32)}
          >
