...
export function TransmittalLogPage() {
  ...
  const rows = useMemo(() => {
    ...
    return data.filter((r) => {
      ...
      if (reasonFilter !== "all" && r.reason_code !== reasonFilter)
        return false;
      return true;
    });
            ...
            <thead className="text-xs text-content-secondary border-b border-border-light bg-surface-secondary/20 sticky top-0">
              <tr>
                <th className="text-left px-6 py-2 font-medium">
                  {t("files.transmittals.col_number", {
                    defaultValue: "Number",
                  })}
                </th>
                <th className="text-left px-3 py-2 font-medium">
                  {t("files.transmittals.col_subject", {
                    defaultValue: "Subject",
                  })}
                </th>
                <th className="text-left px-3 py-2 font-medium">
                  {t("files.transmittals.col_reason", {
                    defaultValue: "Reason",
                  })}
                </th>
                ...
                <th className="text-left px-3 py-2 font-medium">
                  {t("files.transmittals.col_sent_at", {
                    defaultValue: "Sent",
                  })}
                </th>
                <th className="text-left px-3 py-2 font-medium">
                  {t("files.transmittals.col_status", {
                    defaultValue: "Status",
                  })}
                </th>
