...
export async function fetchPhotoGallery(
  ...
  if (!projectId) return [];
  return apiGet<PhotoItem[]>(
    `/v1/documents/photos/gallery/?project_id=${projectId}`,
  );
}

export async function fetchPhotoTimeline(
  projectId: string,
): Promise<PhotoTimelineGroup[]> {
  if (!projectId) return [];
  return apiGet<PhotoTimelineGroup[]>(
    `/v1/documents/photos/timeline/?project_id=${projectId}`,
  );
}
...
export async function uploadPhoto(
  ...
  if (metadata.gps_lat != null)
    formData.append("gps_lat", String(metadata.gps_lat));
  if (metadata.gps_lon != null)
    formData.append("gps_lon", String(metadata.gps_lon));
  ...
  const res = await fetch(
    `/api/v1/documents/photos/upload/?project_id=${projectId}`,
    {
      method: "POST",
      headers: {
