...
const CDE_STATE_COLORS: Record<
  string,
  "warning" | "blue" | "success" | "neutral"
> = {
  wip: "warning",
  shared: "blue",
  published: "success",
...
function fileIcon(mime: string, name?: string) {
  ...
  if (
    ...
    /\.(png|jpe?g|gif|webp|bmp|tiff?)$/.test(lower)
  ) {
 ...
 */
function routeForDocument(doc: DocItem): {
  path: string;
  module: "takeoff" | "dwg-takeoff" | "bim" | "preview" | "download";
} {
  const sourceModule = doc.metadata?.source_module;
...
function PreviewModal({ doc, onClose }: { doc: DocItem; onClose: () => void }) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("documents.preview_title", {
            ...
            <div className="flex flex-wrap gap-1">
              {linkedElements.items.slice(0, 12).map((link) => (
                <button
                  ...
                  onClick={() => {
                    navigate(
                      `/bim?element=${encodeURIComponent(link.bim_element_id)}`,
                    );
                    onClose();
                  }}
...
function SortDropdown({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        {t("documents.sort_by", { defaultValue: "Sort by‌⁠‍" })}:{" "}
        {labels[value]}
        <ChevronDown
          size={14}
      ...
      {open && (
        <div
          ...
          role="listbox"
          aria-label={t("documents.sort_options", {
...
export function DocumentsPage() {
  ...
  const [fileTypeFilter, setFileTypeFilter] = useState<
    "all" | "pdf" | "dwg" | "ifc" | "rvt" | "other"
  >("all");
  const [revisionFilter, setRevisionFilter] = useState<
    "all" | "latest" | "versioned"
  >("all");
  ...
  const sortedDocuments = useMemo(() => {
    ...
    const filtered = docs.filter((d) => {
      if (fileTypeFilter !== "all" && classifyFileType(d) !== fileTypeFilter)
        return false;
      if (revisionFilter === "latest" && latestByName.get(d.name)?.id !== d.id)
        return false;
      if (
        revisionFilter === "versioned" &&
        (versionCountByName.get(d.name) ?? 1) < 2
      )
        return false;
      return true;
    });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("documents.delete_failed", {
  ...
  const handleUpload = useCallback(
    async (files: FileList | File[]) => {
      if (!projectId) {
        addToast({
          type: "error",
          title: t("documents.no_project_error", {
            defaultValue: "No project selected",
          }),
          message: t("documents.select_project_first", {
            defaultValue: "Please select a project first before uploading.",
          }),
        });
        return;
      }
        ...
        (async () => {
          try {
            ...
            const progressTimer = setInterval(() => {
              const task = useUploadQueueStore
                .getState()
      ...
      addToast({
        type: "info",
        title: t("documents.upload_queued", {
          defaultValue: "{{count}} file(s) queued for upload",
  ...
  const renameMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("documents.renamed", {
          defaultValue: "Document renamed successfully",
        }),
      });
      setRenameDoc(null);
    },
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("documents.rename_failed", {
  ...
  const editMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("documents.properties_saved", {
          defaultValue: "Document properties saved successfully",
        }),
      });
      setEditDoc(null);
    },
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("documents.save_failed", {
          defaultValue: "Failed to save document properties",
        }),
        message: err.message,
      });
    },
    ...
    return (
      <div className="w-full px-5 py-4 space-y-4 animate-fade-in">
        ...
        <EmptyState
          ...
          title={t("documents.select_project", {
            defaultValue: "Select a project",
          })}
          description={t("documents.select_project_hint", {
            defaultValue:
              "Use the project switcher in the header to select a project first.",
          })}
        />
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-content-secondary">
            {t("documents.subtitle", {
              defaultValue:
                "Upload and manage project files — drawings, contracts, specifications",
            })}
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          ...
          <input
            ...
            aria-label={t("documents.upload_input", {
          ...
          <Button
            ...
            title={
              !projectId
                ? t("documents.select_project_first", {
      ...
      <div
        ...
        role="region"
        aria-label={t("documents.drop_zone", {
          defaultValue: "File drop zone",
        })}
        onClick={() => {
      ...
      >
        <div
          className={`mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl transition-all ${
            dragOver
              ? "bg-oe-blue/15"
        ...
        >
          <Upload
            size={22}
            className={dragOver ? "text-oe-blue" : "text-oe-blue"}
          />
        </div>
        <p className="text-sm font-semibold text-content-primary">
          {dragOver
            ? t("documents.drop_now", { defaultValue: "Drop files to upload" })
            : t("documents.drop_hint", {
                defaultValue: "Drag & drop files here",
              })}
        </p>
        <p className="text-xs text-content-tertiary mt-1">
          {t("documents.supported_types", {
            defaultValue: "PDF, images, Excel, DWG, IFC — any file type",
          })}
        </p>
        <div className="flex items-center justify-center gap-2 mt-3">
          <span className="text-[10px] font-mono px-2 py-1 rounded-md bg-red-500/8 text-red-500 border border-red-500/15 font-semibold">
            .pdf
          </span>
          <span className="text-[10px] font-mono px-2 py-1 rounded-md bg-emerald-500/8 text-emerald-600 border border-emerald-500/15 font-semibold">
            .img
          </span>
          <span className="text-[10px] font-mono px-2 py-1 rounded-md bg-green-500/8 text-green-600 border border-green-500/15 font-semibold">
            .xlsx
          </span>
          <span className="text-[10px] font-mono px-2 py-1 rounded-md bg-orange-500/8 text-orange-500 border border-orange-500/15 font-semibold">
            .dwg
          </span>
          <span className="text-[10px] font-mono px-2 py-1 rounded-md bg-blue-500/8 text-blue-500 border border-blue-500/15 font-semibold">
            .ifc
          </span>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          <select
            ...
            aria-label={t("documents.filter_file_type", {
              defaultValue: "Filter by file type",
            })}
          ...
          >
            <option value="all">
              {t("documents.type_all", { defaultValue: "All types" })}
            </option>
            <option value="pdf">
              {t("documents.type_pdf", { defaultValue: "PDF" })}
            </option>
            <option value="dwg">
              {t("documents.type_dwg", { defaultValue: "DWG" })}
            </option>
            <option value="ifc">
              {t("documents.type_ifc", { defaultValue: "IFC" })}
            </option>
            <option value="rvt">
              {t("documents.type_rvt", { defaultValue: "RVT" })}
            </option>
            <option value="other">
              {t("documents.type_other", { defaultValue: "Other" })}
            </option>
          </select>
        ...
        <div className="relative">
          <select
            ...
            onChange={(e) =>
              setRevisionFilter(
                e.target.value as "all" | "latest" | "versioned",
              )
            }
            aria-label={t("documents.filter_revision", {
              defaultValue: "Filter by revision",
            })}
            className="h-10 appearance-none rounded-lg border border-border bg-surface-primary pl-3 pr-9 text-sm text-content-primary focus:outline-none focus:ring-2 focus:ring-oe-blue/30"
          >
            <option value="all">
              {t("documents.rev_all", { defaultValue: "All revisions" })}
            </option>
            <option value="latest">
              {t("documents.rev_latest", { defaultValue: "Latest only" })}
            </option>
            <option value="versioned">
              {t("documents.rev_versioned", {
                defaultValue: "Has multiple versions",
              })}
            </option>
          </select>
          <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2.5 text-content-tertiary">
            ...
            {cadSessions.map((s) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    navigate(`/data-explorer?session=${s.session_id}`)
                  }
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-content-tertiary mt-1 flex items-center gap-1.5">
                        <span
                          className={`px-1.5 py-0.5 rounded font-mono text-[10px] font-semibold ${fmtColor[fmt] || "bg-surface-secondary text-content-tertiary"}`}
                        >
                          {fmt || "—"}
                        </span>
                  ...
                  <div className="mt-3 pt-2.5 border-t border-border-light/60 flex items-center justify-between text-[10px] text-content-quaternary">
                    ...
                    {s.is_permanent ? (
                      <Badge variant="success" size="sm">
                        {t("documents.saved", { defaultValue: "Saved" })}
                      </Badge>
                    ) : (
                      <Badge variant="neutral" size="sm">
                        {t("documents.temporary", { defaultValue: "24h" })}
                      </Badge>
                    )}
                  </div>
                </button>
      ...
      {(bimModels.length > 0 ||
        dwgDrawings.length > 0 ||
        takeoffDocs.length > 0) && (
        <div className="space-y-3">
          <h3 className="text-xs font-semibold text-content-primary flex items-center gap-1.5">
            <File size={13} className="text-oe-blue" />
            {t("documents.module_files", { defaultValue: "Module Files" })}
            <Badge variant="blue" size="sm">
              {bimModels.length + dwgDrawings.length + takeoffDocs.length}
            </Badge>
          </h3>
          <div className="grid gap-2 md:gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[10px] text-content-tertiary">
                      {(m.model_format || m.format || "BIM").toUpperCase()}{" "}
                      &middot; {(m.element_count ?? 0).toLocaleString()} elem
                    </p>
                  </div>
                </div>
              </button>
            ...
            {dwgDrawings.map((d) => (
              <button
                ...
                type="button"
                onClick={() =>
                  navigate(`/dwg-takeoff?drawingId=${encodeURIComponent(d.id)}`)
                }
                className="group text-left rounded-lg border border-border-light bg-surface-primary px-3 py-2 hover:border-oe-blue/30 hover:shadow-sm transition-all"
                title={d.name}
              >
            ...
            {takeoffDocs.map((td) => (
              <button
                ...
                onClick={() =>
                  navigate(
                    `/takeoff?tab=measurements&doc=${encodeURIComponent(td.id)}&name=${encodeURIComponent(td.filename)}`,
                  )
                }
                className="group text-left rounded-lg border border-border-light bg-surface-primary px-3 py-2 hover:border-oe-blue/30 hover:shadow-sm transition-all"
                title={td.filename}
      ...
      {isLoading ? (
        <div className="grid gap-3 md:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="rounded-xl border border-border-light bg-surface-primary p-4 shadow-sm"
            >
        ...
        debouncedQuery.trim() ? (
          <EmptyState
            ...
            title={t("documents.no_results", {
              defaultValue: "No results found",
            })}
            description={t("documents.no_results_hint", {
              defaultValue:
                'No documents match "{{query}}". Try a different search term or clear filters.',
              query: debouncedQuery,
            })}
            action={{
              label: t("documents.clear_search", {
                defaultValue: "Clear search",
              }),
              onClick: () => setQuery(""),
            }}
          ...
          <EmptyState
            ...
            description={t("documents.empty_hint", {
              defaultValue:
                "Upload your first file — drawings, contracts, photos, or any project document.",
            })}
          />
        )
      ) : (
          ...
          {sortedDocuments.map((doc) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    className={`flex h-9 w-9 items-center justify-center rounded-lg shrink-0 ${fileIconBg(doc.mime_type, doc.name)}`}
                  >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mt-2">
                      ...
                      {doc.cde_state && (
                        <Badge
                          variant={CDE_STATE_COLORS[doc.cde_state] ?? "neutral"}
                          size="sm"
                        >
                    ...
                    {doc.tags && doc.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {doc.tags.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="rounded-full bg-content-primary/[0.04] px-2 py-[1px] text-[10px] font-medium text-content-tertiary"
                          >
                            {tag}
                          </span>
                        ))}
                          ...
                          >
                            ...
                            {previewKind === "pdf" && (
                              <button
                                ...
                                onClick={() => {
                                  setOpenMenuId(null);
                                  navigate(
                                    `/takeoff?doc=${doc.id}&name=${encodeURIComponent(doc.name)}`,
                                  );
                                }}
                                className="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-content-primary hover:bg-surface-secondary transition-colors"
                              >
                                <Ruler size={14} className="text-oe-blue" />
                                {t("documents.open_in_takeoff", {
                                  defaultValue: "Measure & Takeoff",
                                })}
                              </button>
                            )}
                            {(doc.name.toLowerCase().endsWith(".dwg") ||
                              doc.name.toLowerCase().endsWith(".dxf") ||
                              doc.name.toLowerCase().endsWith(".dgn")) && (
                              <button
                                role="menuitem"
                                onClick={() => {
                                  setOpenMenuId(null);
                                  navigate(
                                    `/dwg-takeoff?docId=${encodeURIComponent(doc.id)}&docName=${encodeURIComponent(doc.name)}`,
                                  );
                                }}
                                className="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-content-primary hover:bg-surface-secondary transition-colors"
                              >
                                <Ruler size={14} className="text-oe-blue" />
                                {t("documents.open_in_dwg_takeoff", {
                                  defaultValue: "Open in DWG Takeoff",
                                })}
                              </button>
                            )}
                            {(doc.name.toLowerCase().endsWith(".rvt") ||
                              doc.name.toLowerCase().endsWith(".ifc") ||
                              doc.name.toLowerCase().endsWith(".nwd") ||
                              doc.name.toLowerCase().endsWith(".nwc")) && (
                              <button
                                role="menuitem"
                                onClick={() => {
                                  setOpenMenuId(null);
                                  navigate(
                                    `/bim?docId=${encodeURIComponent(doc.id)}&docName=${encodeURIComponent(doc.name)}`,
                                  );
                                }}
                                className="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-content-primary hover:bg-surface-secondary transition-colors"
                              >
                                <Ruler size={14} className="text-oe-blue" />
                                {t("documents.open_in_bim", {
                                  defaultValue: "Open in BIM Viewer",
                                })}
                              </button>
                            )}
                            <a
                            ...
                            <button
                              ...
                              onClick={() => {
                                setOpenMenuId(null);
                                navigate(
                                  "/transmittals?create=true&doc_ids=" + doc.id,
                                );
                              }}
                              className="flex w-full items-center gap-2.5 px-3 py-2 text-xs text-content-primary hover:bg-surface-secondary transition-colors"
                            >
                              <Send
                                ...
                                className="text-content-tertiary"
                              />
                              {t("documents.send_transmittal", {
                                defaultValue: "Send via Transmittal",
                              })}
                            </button>
      ...
      {editDoc && (
        ...
        >
          <div
            className="relative w-full max-w-md mx-4 rounded-xl bg-surface-elevated shadow-xl border border-border-light overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-2xs text-content-tertiary">
                    {formatSize(editDoc.file_size)} ·{" "}
                    <DateDisplay value={editDoc.created_at} />
                  </p>
                </div>
              </div>
              <button
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t("documents.description_placeholder", {
                    defaultValue: "Add notes about this document...",
                  })}
                  rows={3}
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <input
                    ...
                    placeholder={t("documents.tag_placeholder", {
                ...
                {editForm.tags.length === 0 && (
                  <p className="text-2xs text-content-quaternary">
                    {t("documents.no_tags", {
                      defaultValue:
                        "No tags yet. Tags help organize and find documents faster.",
                    })}
                  </p>
                )}
              </div>
      ...
      {renameDoc && (
        ...
        >
          <div
            className="relative w-full max-w-sm mx-4 rounded-xl bg-surface-elevated shadow-xl border border-border-light p-5"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <Button
                ...
                disabled={
                  !renameValue.trim() ||
                  renameValue.trim() === renameDoc.name ||
