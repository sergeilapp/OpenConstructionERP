...
const ROLE_OPTIONS: DistributionMemberRole[] = [
  "for_review",
  "fyi",
  "for_construction",
];
...
export function DistributionListModal({
  ...
  const submitCreate = async () => {
    ...
    if (!trimmed) {
      setError(
        t("files.distribution.error_name_required", {
  ...
  const submitAddMember = async () => {
    ...
    if (!trimmed) {
      setError(
        t("files.distribution.error_email_required", {
    ...
    >
      {!active && (
        <div
          ...
          data-testid="distribution-list-overview"
        >
          <div className="flex items-center justify-between">
            <span className="text-sm text-content-secondary">
              {lists.length === 0
                ? t("files.distribution.empty", {
                    defaultValue: "No lists yet — create one.",
                  })
      ...
      {active && (
        <div
          ...
          data-testid="distribution-list-detail"
        >
          {active.description && (
          ...
          <div className="flex items-end gap-2">
            <label className="flex flex-1 flex-col gap-1 text-sm">
              <span className="text-xs font-semibold uppercase tracking-wide text-content-tertiary">
                {t("files.distribution.member_email", {
                  defaultValue: "Email",
                })}
              </span>
              <Input
