    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="flex-1 truncate font-medium text-content-primary">
          {hit.canonical_name ||
            t("files.global_search.unnamed", { defaultValue: "(unnamed)" })}
        </span>
        ...
        <div className="flex flex-1 items-center gap-2 rounded-lg border border-border bg-surface-primary px-3 py-2 focus-within:border-oe-blue">
          ...
          {query && (
            <button
              type="button"
              onClick={() => setQuery("")}
              aria-label={t("files.global_search.clear", {
                defaultValue: "Clear query",
              })}
        ...
        >
          <section className="flex flex-col gap-2">
            <h2 className="text-xs uppercase tracking-wide text-content-tertiary">
              {t("files.global_search.kind_filter_label", {
                defaultValue: "File types",
              })}
            </h2>
            <div
              className="flex flex-wrap gap-1.5"
              data-testid="global-search-kind-filters"
            >
              {ALL_KINDS.map((kind) => {
          ...
          <section className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1">
              {(["relevance", "name_asc", "project_asc"] as const).map(
                (key) => {
                  const on = sort === key;
