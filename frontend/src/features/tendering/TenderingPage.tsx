        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {visibleRows.map((row, idx) => (
              ...
              >
                <td
                  className={`px-3 py-2.5 ${stickyColClass} group-hover:bg-surface-secondary/30`}
                >
...
function PackageDetail({
  ...
  const {
    ...
    queryFn: () =>
      apiGet<PackageWithBids>(`/v1/tendering/packages/${packageId}`),
  });
  ...
  const {
    ...
    queryFn: () =>
      apiGet<BidComparison>(`/v1/tendering/packages/${packageId}/comparison/`),
  });
  ...
  const handleExport = useCallback(() => {
    ...
    const headers = [
      ...
      "Unit",
      "Budget Rate",
      ...comparison.bid_companies.map((c) => `${c} Rate`),
    ];
    ...
    addToast({
      type: "success",
      title: t("tendering.exported", { defaultValue: "Comparison exported" }),
    });
        ...
        <Card className="py-12">
          <EmptyState
            ...
            description={t("tendering.package_load_error", {
              defaultValue:
                "Failed to load this tender package. Please try again.",
            })}
          />
        </Card>
      </div>
  ...
  const awardablePackage =
    pkg.status === "collecting" || pkg.status === "evaluating";
          ...
          {pkg.bids.map((bid) => (
            <Card key={bid.id} padding="none">
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <Badge
                  variant={STATUS_COLORS[bid.status] || "neutral"}
                  size="sm"
                >
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        const ok = await confirm({
                          title: t("tendering.award_confirm_title", {
                            defaultValue: "Award contract?",
                          }),
                          message: t("tendering.award_confirm", {
                            defaultValue:
                              "Award this contract to {{company}}? Winning rates are written back to the BOQ and other bids are rejected. This action cannot be undone.",
          ...
          {comparisonLoading ? (
            ...
            <EmptyState
              ...
              description={t("tendering.comparison_load_error", {
                defaultValue:
                  "Failed to load the bid comparison. Please try again.",
              })}
            />
          ) : comparison ? (
            <>
...
export function TenderingPage() {
  ...
  return (
    <div className="w-full animate-fade-in">
      ...
      <InfoHint
        className="mb-6"
        text={t("tendering.workflow_desc", {
          defaultValue:
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end">
          ...
          {projectsLoading ? (
            ...
            <div>
              ...
              <SelectDropdown
                ...
                placeholder={t(
                  "tendering.select_project_placeholder",
                  "Choose a project...",
                )}
              />
            </div>
        ...
        <span
          title={
            !selectedProjectId
              ? t("tendering.select_project_first", {
      ...
      {!selectedProjectId && (
        <EmptyState
          ...
          title={t("tendering.select_project_title", {
      ...
      {selectedProjectId &&
        ...
        packages.length === 0 && (
          <EmptyState
            icon={<FileText size={28} strokeWidth={1.5} />}
            title={t("tendering.no_packages", {
              defaultValue: "No tenders yet",
            })}
      ...
      {packages && packages.length > 0 && (
        <div>
          ...
          {selectedPackageId && (
            <PackageDetail packageId={selectedPackageId} currency={currency} />
          )}
        </div>
      )}
