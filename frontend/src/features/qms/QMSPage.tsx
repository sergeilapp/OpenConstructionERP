...
const ITP_STATUS_VARIANT: Record<
  string,
...
const NCR_STATUS_VARIANT: Record<
  string,
  "neutral" | "blue" | "success" | "warning" | "error"
> = {
  open: "error",
...
const PUNCH_STATUS_VARIANT: Record<
  string,
  "neutral" | "blue" | "success" | "warning" | "error"
> = {
  open: "error",
...
const AUDIT_STATUS_VARIANT: Record<
  string,
  "neutral" | "blue" | "success" | "warning" | "error"
> = {
  planned: "blue",
...
export function QMSPage() {
  ...
  const filteredItp = useMemo(
    () =>
      filterByText(
        itpQ.data ?? [],
        search,
        (r) => `${r.name} ${r.work_type} ${r.wbs_ref ?? ""}`,
      ),
    [itpQ.data, search],
  );
  ...
  const filteredPunch = useMemo(() => {
    ...
    const list = categoryFilter
      ? base.filter((p) => p.category === categoryFilter)
      : base;
    return filterByText(
      list,
      search,
      (r) => `${r.title} ${r.description ?? ""} ${r.room_ref ?? ""}`,
    );
  ...
  const filteredAudits = useMemo(
    () =>
      filterByText(
        auditQ.data ?? [],
        search,
        (r) => `${r.audit_type} ${r.audit_scope ?? ""} ${r.standard_ref ?? ""}`,
      ),
    [auditQ.data, search],
  );
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb
        items={[
          { label: t("qms.title", { defaultValue: "Quality Management‌⁠‍" }) },
        ]}
      />
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-1 text-sm text-content-secondary">
            {t("qms.subtitle", {
              defaultValue:
                "ITP plans, inspections, NCRs, punch list and audits in one place.‌⁠‍",
            })}
          </p>
        </div>
        <Button
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="grid grid-cols-3 gap-4 text-xs">
              <KvBlock
                ...
                value={
                  <MoneyDisplay
                    amount={Number(copqQ.data.ncr_cost_total) || 0}
                    currency={copqQ.data.currency || undefined}
                  />
                }
              />
              <KvBlock
                ...
                value={
                  <MoneyDisplay
                    amount={Number(copqQ.data.rework_cost_estimate) || 0}
                    currency={copqQ.data.currency || undefined}
                  />
                }
              />
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {tab === "punch" && (
          <select
            ...
            onChange={(e) =>
              setCategoryFilter(e.target.value as PunchCategory | "")
            }
            className={clsx(inputCls, "max-w-[180px]")}
          >
            <option value="">
              {t("qms.all_categories", { defaultValue: "All categories" })}
            </option>
            {(
              [
                "architectural",
      ...
      <Card padding="none">
        {!projectId ? (
          <EmptyState
            ...
            description={t("common.no_project_desc", {
              defaultValue: "Create or select a project to view QMS data.",
            })}
          />
        ) : isLoading ? (
          <div className="p-4">
          ...
          <EmptyState
            ...
            title={t("qms.load_error", {
              defaultValue: "Could not load QMS data",
            })}
...
function tabsDef(t: (k: string, opts?: Record<string, unknown>) => string) {
  return [
    {
      id: "itp" as const,
      label: t("qms.tab_itp", { defaultValue: "ITP Plans" }),
      icon: FileCheck,
    },
    {
      id: "inspections" as const,
      label: t("qms.tab_inspections", { defaultValue: "Inspections" }),
      icon: ClipboardCheck,
    },
    {
      id: "ncrs" as const,
      label: t("qms.tab_ncrs", { defaultValue: "NCRs" }),
      icon: AlertOctagon,
    },
    {
      id: "punch" as const,
      label: t("qms.tab_punch", { defaultValue: "Punch List" }),
      icon: ListChecks,
    },
    {
      id: "audits" as const,
      label: t("qms.tab_audits", { defaultValue: "Audits" }),
      icon: Award,
    },
  ];
}

function tabCreateLabel(
  tab: Tab,
...
function ITPTable({
  ...
  const activate = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("qms.itp_activated", { defaultValue: "ITP plan activated" }),
      });
    },
    ...
    return (
      <EmptyState
        ...
        description={t("qms.empty_itp_desc", {
          defaultValue:
            "Inspection & Test Plans define quality gates for each work package.",
        })}
        action={{
          label: t("qms.new_itp", { defaultValue: "New ITP Plan" }),
          onClick: onAction,
        }}
      />
    );
        ...
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("qms.name", { defaultValue: "Name" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.work_type", { defaultValue: "Work type" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.wbs", { defaultValue: "WBS" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.version", { defaultValue: "Ver" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.status", { defaultValue: "Status" })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("qms.actions", { defaultValue: "Actions" })}
            </th>
          </tr>
        </thead>
        <tbody>
...
function InspectionTable({
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={<ClipboardCheck size={22} />}
        title={t("qms.empty_inspections", {
          defaultValue: "No inspections scheduled",
        })}
        description={t("qms.empty_inspections_desc", {
          defaultValue:
            "Schedule an inspection against an ITP control point to record hold/witness sign-offs.",
        })}
        action={{
          label: t("qms.new_inspection", {
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("qms.location", { defaultValue: "Location" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.scheduled_at", { defaultValue: "Scheduled" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.performed_at", { defaultValue: "Performed" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.status", { defaultValue: "Status" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.notes", { defaultValue: "Notes" })}
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            ...
            >
              ...
              <td className="px-4 py-2 text-content-secondary text-xs truncate max-w-[320px]">
                {r.notes || "—"}
              </td>
            </tr>
          ))}
...
function NCRTable({
  ...
  if (rows.length === 0) {
    return (
      <EmptyState
        icon={<AlertOctagon size={22} />}
        title={t("qms.empty_ncrs", {
          defaultValue: "No non-conformance reports",
        })}
        description={t("qms.empty_ncrs_desc", {
          defaultValue:
            "NCRs capture defects with cost impact and feed the COPQ rollup.",
        })}
        action={{
          label: t("qms.new_ncr", { defaultValue: "Raise NCR" }),
          onClick: onAction,
        }}
      />
    );
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("qms.title_col", { defaultValue: "Title" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.severity", { defaultValue: "Severity" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.status", { defaultValue: "Status" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.raised_at", { defaultValue: "Raised" })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("qms.cost_impact", { defaultValue: "Cost impact" })}
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            <tr
              ...
              className="border-t border-border-light hover:bg-surface-secondary cursor-pointer"
            >
              <td className="px-4 py-2 font-medium text-content-primary truncate max-w-[360px]">
                {r.title}
              </td>
              <td className="px-4 py-2">
...
function PunchTable({
  ...
  const closeMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("qms.punch_closed", { defaultValue: "Punch item closed" }),
      });
    },
    onError: (e) => addToast({ type: "error", title: getErrorMessage(e) }),
  });
    ...
    return (
      <EmptyState
        ...
        description={t("qms.empty_punch_desc", {
          defaultValue:
            "Snag items captured on walkthroughs land here for assignment and close-out.",
        })}
        action={{
          label: t("qms.new_punch", { defaultValue: "Add Punch Item" }),
          onClick: onAction,
        }}
      />
    );
      ...
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            ...
            <th className="px-4 py-2.5 text-left">
              {t("qms.category", { defaultValue: "Category" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.severity", { defaultValue: "Severity" })}
            </th>
            ...
            <th className="px-4 py-2.5 text-left">
              {t("qms.due_date", { defaultValue: "Due" })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("qms.actions", { defaultValue: "Actions" })}
            </th>
        ...
        <tbody>
          {rows.map((r) => (
            ...
            >
              ...
              <td className="px-4 py-2">
                <Badge
                  variant={PUNCH_STATUS_VARIANT[r.status] || "neutral"}
                  dot
                >
                  {r.status}
                </Badge>
...
function AuditTable({
  ...
  const completeMut = useMutation({
    mutationFn: ({ id, rating }: { id: string; rating?: number }) =>
      completeAudit(id, rating),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["qms", "audits"] });
      addToast({
        type: "success",
        title: t("qms.audit_completed", { defaultValue: "Audit completed" }),
      });
    },
    onError: (e) => addToast({ type: "error", title: getErrorMessage(e) }),
  });
    ...
    return (
      <EmptyState
        ...
        description={t("qms.empty_audits_desc", {
          defaultValue:
            "ISO 9001 internal, external and supplier audits with finding registers.",
        })}
        action={{
          label: t("qms.new_audit", { defaultValue: "Plan Audit" }),
          onClick: onAction,
        }}
      />
    );
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-surface-secondary text-content-tertiary text-xs uppercase tracking-wide">
          <tr>
            <th className="px-4 py-2.5 text-left">
              {t("qms.audit_type", { defaultValue: "Type" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.standard", { defaultValue: "Standard" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.planned_date", { defaultValue: "Planned" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.status", { defaultValue: "Status" })}
            </th>
            <th className="px-4 py-2.5 text-left">
              {t("qms.rating", { defaultValue: "Rating" })}
            </th>
            <th className="px-4 py-2.5 text-right">
              {t("qms.actions", { defaultValue: "Actions" })}
            </th>
        ...
        <tbody>
          {rows.map((r) => (
            <tr
              ...
              className="border-t border-border-light hover:bg-surface-secondary"
            >
              <td className="px-4 py-2 font-medium text-content-primary capitalize">
                {r.audit_type}
              </td>
              <td className="px-4 py-2 text-content-secondary text-xs">
                {r.standard_ref || "—"}
              ...
              <td className="px-4 py-2">
                <Badge
                  variant={AUDIT_STATUS_VARIANT[r.status] || "neutral"}
                  dot
                >
                  {r.status}
                </Badge>
...
function NCRDrawer({
  ...
  const addAction = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("qms.action_added", { defaultValue: "Action assigned" }),
      });
      setActionDesc("");
      setResponsible("");
    },
  ...
  const escalate = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("qms.ncr_escalated", {
          defaultValue: "NCR escalated to variation",
        }),
      });
      onClose();
    },
  ...
  const closeMut = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("qms.ncr_closed", { defaultValue: "NCR closed" }),
      });
      onClose();
    },
    onError: (e) => addToast({ type: "error", title: getErrorMessage(e) }),
      ...
      >
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <Field
              label={t("qms.severity")}
              value={
                <Badge variant={SEVERITY_VARIANT[ncr.severity]}>
                  {ncr.severity}
                </Badge>
              }
            />
            <Field
              ...
              value={
                <Badge
                  variant={NCR_STATUS_VARIANT[ncr.status] || "neutral"}
                  dot
                >
                  {ncr.status}
                </Badge>
            ...
            <Field
              ...
              value={
                ncr.raised_at ? <DateDisplay value={ncr.raised_at} /> : "—"
              }
            />
            <Field
              ...
              value={
                ncr.cost_impact_amount != null ? (
                  <MoneyDisplay
                    amount={Number(ncr.cost_impact_amount)}
                    currency={ncr.cost_impact_currency || undefined}
                  />
                ) : (
                  "—"
                )
            ...
            {ncr.root_cause && (
              <Field
                label={t("qms.root_cause", { defaultValue: "Root cause" })}
                value={ncr.root_cause}
              />
            )}
            {ncr.linked_variation_id && (
              <Field
                label={t("qms.linked_variation", { defaultValue: "Variation" })}
                value={ncr.linked_variation_id}
              />
            )}
          </div>
          ...
          <Card padding="sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-content-secondary mb-2">
              {t("qms.add_action", {
                defaultValue: "Assign corrective action",
              })}
            </p>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {ncr.status !== "closed" && ncr.status !== "cancelled" && (
              ...
              >
                {t("qms.escalate_to_variation", {
                  defaultValue: "Escalate to Variation",
                })}
              </Button>
            )}
...
function InspectionDrawer({
  ...
  const [result, setResult] = useState<"passed" | "failed" | "conditional">(
    "passed",
  );
  ...
  const complete = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("qms.inspection_completed", {
          defaultValue: "Inspection completed",
        }),
      });
      onClose();
    },
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border-light bg-surface-elevated px-5 py-3">
          <h2 id="qms-insp-drawer-title" className="text-base font-semibold">
            {insp.location_ref ||
              t("qms.inspection", { defaultValue: "Inspection" })}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-4 p-5">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <Field
              ...
              value={
                <Badge
                  variant={INSPECTION_VARIANT[insp.status] || "neutral"}
                  dot
                >
                  {insp.status}
                </Badge>
            ...
            <Field
              ...
              value={
                insp.scheduled_at ? (
                  <DateDisplay value={insp.scheduled_at} />
                ) : (
                  "—"
                )
              }
            ...
            <Field
              ...
              value={
                insp.performed_at ? (
                  <DateDisplay value={insp.performed_at} />
                ) : (
                  "—"
                )
              }
            />
            <Field
              label={t("qms.drawing_ref", { defaultValue: "Drawing" })}
              value={insp.drawing_ref || "—"}
            />
          ...
          <Card padding="sm">
            ...
            <p className="text-xs text-content-tertiary">
              {t("qms.signatures_hint", {
                defaultValue:
                  "Sign-offs are captured per role (GC, designer, client, inspector).",
              })}
            </p>
          </Card>
          ...
          {(insp.status === "scheduled" || insp.status === "in_progress") && (
            <Card padding="sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-content-secondary mb-2">
                {t("qms.complete_inspection", {
                  defaultValue: "Complete inspection",
                })}
              </p>
              <div className="space-y-2">
                <select
                  ...
                  onChange={(e) =>
                    setResult(
                      e.target.value as "passed" | "failed" | "conditional",
                    )
                  }
                  className={inputCls}
                >
                ...
                <textarea
                  ...
                  placeholder={t("qms.notes_placeholder", {
...
function CreateModal({
  ...
  const submit = async () => {
    ...
    try {
      if (kind === "itp") {
        if (!itpForm.name.trim() || !itpForm.work_type.trim())
          throw new Error("Name and work type required");
        await createITPPlan({
          project_id: projectId,
          name: itpForm.name,
        ...
        addToast({
          type: "success",
          title: t("qms.itp_created", { defaultValue: "ITP plan created" }),
        });
        qc.invalidateQueries({ queryKey: ["qms", "itp"] });
        ...
        addToast({
          type: "success",
          title: t("qms.inspection_scheduled", {
            defaultValue: "Inspection scheduled",
          }),
        });
        qc.invalidateQueries({ queryKey: ["qms", "inspections"] });
      } else if (kind === "ncrs") {
        if (!ncrForm.title.trim() || !ncrForm.description.trim())
        ...
        await createNCR({
          ...
          cost_impact_amount: ncrForm.cost_impact_amount
            ? Number(ncrForm.cost_impact_amount)
            : undefined,
        ...
        addToast({
          type: "success",
          title: t("qms.ncr_created", { defaultValue: "NCR raised" }),
        });
        qc.invalidateQueries({ queryKey: ["qms", "ncrs"] });
      } else if (kind === "punch") {
        ...
        addToast({
          type: "success",
          title: t("qms.punch_created", { defaultValue: "Punch item added" }),
        });
        qc.invalidateQueries({ queryKey: ["qms", "punch"] });
      } else if (kind === "audits") {
        await createAudit({
        ...
        addToast({
          type: "success",
          title: t("qms.audit_created", { defaultValue: "Audit planned" }),
        });
        qc.invalidateQueries({ queryKey: ["qms", "audits"] });
      }
      onClose();
    ...
    >
      {kind === "itp" && (
        <WideModalSection columns={2}>
          <WideModalField
            label={t("qms.name", { defaultValue: "Name" })}
            required
            span={2}
          >
            <input
          ...
          <WideModalField
            label={t("qms.work_type", { defaultValue: "Work type" })}
            required
          >
            <input
              value={itpForm.work_type}
              onChange={(e) =>
                setItpForm({ ...itpForm, work_type: e.target.value })
              }
              className={inputCls}
          ...
          <WideModalField label={t("qms.wbs", { defaultValue: "WBS ref" })}>
            <input
              value={itpForm.wbs_ref}
              onChange={(e) =>
                setItpForm({ ...itpForm, wbs_ref: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
      ...
      {kind === "inspections" && (
        <WideModalSection columns={2}>
          <WideModalField
            label={t("qms.itp_item", {
              defaultValue: "ITP control point (optional)",
            })}
            span={2}
          >
            <select
              ...
              onChange={(e) =>
                setInspForm({ ...inspForm, itp_item_id: e.target.value })
              }
              className={inputCls}
            >
              <option value="">—</option>
          ...
          <WideModalField
            label={t("qms.location", { defaultValue: "Location" })}
          >
            <input
              value={inspForm.location_ref}
              onChange={(e) =>
                setInspForm({ ...inspForm, location_ref: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          <WideModalField
            label={t("qms.scheduled_at", { defaultValue: "Scheduled at" })}
          >
            <input
              type="datetime-local"
              value={inspForm.scheduled_at}
              onChange={(e) =>
                setInspForm({ ...inspForm, scheduled_at: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          <WideModalField
            label={t("qms.notes", { defaultValue: "Notes" })}
            span={2}
          >
            <textarea
              value={inspForm.notes}
              onChange={(e) =>
                setInspForm({ ...inspForm, notes: e.target.value })
              }
              rows={2}
              className={clsx(inputCls, "h-auto py-2")}
            />
      ...
      {kind === "ncrs" && (
        <WideModalSection columns={2}>
          <WideModalField
            label={t("qms.title_col", { defaultValue: "Title" })}
            required
            span={2}
          >
            <input
          ...
          <WideModalField
            label={t("qms.description", { defaultValue: "Description" })}
            required
            span={2}
          >
            <textarea
          ...
          <WideModalField
            label={t("qms.severity", { defaultValue: "Severity" })}
          >
            <select
          ...
          <WideModalField
            label={t("qms.linked_inspection", {
              defaultValue: "Linked inspection",
            })}
          >
            <select
              value={ncrForm.linked_inspection_id}
              onChange={(e) =>
                setNcrForm({ ...ncrForm, linked_inspection_id: e.target.value })
              }
              className={inputCls}
            >
              <option value="">—</option>
          ...
          <WideModalField
            label={t("qms.cost_impact", { defaultValue: "Cost impact" })}
          >
            <input
              type="number"
              value={ncrForm.cost_impact_amount}
              onChange={(e) =>
                setNcrForm({ ...ncrForm, cost_impact_amount: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setNcrForm({ ...ncrForm, cost_impact_currency: e.target.value })
              }
              className={inputCls}
              maxLength={3}
            />
        ...
        <WideModalSection columns={2}>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setPunchForm({ ...punchForm, title: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          ...
          >
            <textarea
              ...
              onChange={(e) =>
                setPunchForm({ ...punchForm, description: e.target.value })
              }
              rows={2}
              className={clsx(inputCls, "h-auto py-2")}
            />
          ...
          <WideModalField
            label={t("qms.room", { defaultValue: "Room / area" })}
          >
            <input
              value={punchForm.room_ref}
              onChange={(e) =>
                setPunchForm({ ...punchForm, room_ref: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          <WideModalField label={t("qms.category")}>
            <select
              ...
              onChange={(e) =>
                setPunchForm({
                  ...punchForm,
                  category: e.target.value as PunchCategory | "",
                })
              }
              className={inputCls}
            >
          ...
          <WideModalField label={t("qms.due_date")}>
            <input
              ...
              onChange={(e) =>
                setPunchForm({ ...punchForm, due_date: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
      ...
      {kind === "audits" && (
        <WideModalSection columns={2}>
          ...
          <WideModalField
            label={t("qms.standard", { defaultValue: "Standard" })}
          >
            <input
              value={auditForm.standard_ref}
              onChange={(e) =>
                setAuditForm({ ...auditForm, standard_ref: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          <WideModalField
            label={t("qms.planned_date", { defaultValue: "Planned date" })}
            span={2}
          >
            <input
              type="date"
              ...
              onChange={(e) =>
                setAuditForm({ ...auditForm, planned_date: e.target.value })
              }
              className={inputCls}
            />
          </WideModalField>
          <WideModalField
            label={t("qms.audit_scope", { defaultValue: "Scope" })}
            span={2}
          >
            <textarea
              value={auditForm.audit_scope}
              onChange={(e) =>
                setAuditForm({ ...auditForm, audit_scope: e.target.value })
              }
              rows={2}
              className={clsx(inputCls, "h-auto py-2")}
            />
