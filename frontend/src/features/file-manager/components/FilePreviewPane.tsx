...
import {
  useFileVersions,
  useRestoreVersion,
} from "@/features/file-versions/hooks";
...
function isPdf(row: FileRow): boolean {
  if (row.extension && row.extension.toLowerCase().replace(/^\./, "") === "pdf")
    return true;
  if (row.mime_type && row.mime_type.toLowerCase() === "application/pdf")
    return true;
  return false;
}
    ...
    if (ok) {
      ...
      addToast({
        ...
        title: t("files.toast.copy_failed", {
          defaultValue: "Could not copy path‌⁠‍",
        }),
      });
    }
  }
  ...
  return (
    <aside className="w-80 shrink-0 border-l border-border-light bg-surface-elevated overflow-y-auto">
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          <h3
            className="text-sm font-semibold text-content-primary break-words"
            title={row.name}
          >
            {row.name}
          </h3>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <button
            ...
            title={t(primary.descriptionI18nKey, {
          ...
          >
            <Send size={13} />
            {t("files.transmittals.send_action", {
              defaultValue: "Send transmittal",
            })}
          </button>
          ...
          >
            ...
            {t("files.approvals.submit_action", {
          ...
          >
            <LinkIcon size={13} />
            {t("files.references.link_action", {
              defaultValue: "Link to entity",
            })}
          </button>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-medium border border-border-light text-content-primary hover:bg-surface-secondary transition-colors"
            title={t("files.activity.open", {
              defaultValue: "View activity history",
            })}
          >
            <Activity size={13} />
            {t("files.activity.open", {
              defaultValue: "View activity history",
            })}
          </button>
          {onManageAccess && (
        ...
        <dl className="space-y-2 text-xs">
          ...
          {row.category && (
            <Row
              label={t("files.detail.category", { defaultValue: "Category" })}
            >
              {row.category}
            </Row>
          )}
          {row.discipline && (
            <Row
              label={t("files.detail.discipline", {
          ...
          <Row label={t("files.detail.path", { defaultValue: "Path" })}>
            <button
              ...
              title={t("files.actions.copy_path", {
    ...
    <div className="border-t border-border-light pt-3">
      ...
      {isLoading ? (
        ...
        <ol className="relative space-y-2 ps-3 before:absolute before:top-1 before:bottom-1 before:start-1 before:w-px before:bg-border-light">
          {events.map((ev) => {
            const chipStyle =
              ACTIVITY_ACTION_STYLE[ev.action] ??
              ...
              <div className="flex flex-col items-end gap-1 shrink-0">
                {v.is_current ? (
                  ...
                  >
                    ...
                    {t("files.versions.make_current", {
                      defaultValue: "Make current",
                    })}
                  </button>
                )}
              </div>
