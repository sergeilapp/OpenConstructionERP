...
export function UploadDialog({
  ...
  const categoryForKind = useCallback((kind: FileKind | null): string => {
    ...
    if (kind === "sheet" || kind === "dwg_drawing" || kind === "bim_model")
      return "drawing";
    return "other";
  }, []);
  ...
  const doUpload = useCallback(
    async (files: FileList | File[]) => {
      if (!projectId) {
        addToast({
          type: "error",
          title: t("files.upload_no_project", {
          ...
          try {
            ...
            if (!res.ok) {
              ...
              queryClient.invalidateQueries({
                queryKey: [fileManagerKeys.tree, projectId],
              });
              queryClient.invalidateQueries({
                queryKey: [fileManagerKeys.list, projectId],
              });
            }
          } catch (err) {
            updateQueueTask(taskId, {
