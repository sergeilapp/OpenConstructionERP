...
export function FileManagerPage() {
  ...
  const [selectedKind, setSelectedKind] = useState<FileKind | null>(
    initialKind,
  );
  const [query, setQuery] = useState(initialQuery);
  const [sort, setSort] =
    useState<NonNullable<FileFilters["sort"]>>(initialSort);
  ...
  useEffect(() => {
    ...
    const urlKindClean = urlKindRaw
      ? urlKindRaw.replace(/^category:/, "")
      : null;
    const urlKind: FileKind | null =
      urlKindClean && VALID_KINDS.has(urlKindClean)
        ? (urlKindClean as FileKind)
        : null;
  ...
  useEffect(() => {
    ...
    if (selectedTagIds.length > 0)
      next.set("tag_ids", selectedTagIds.join(","));
    ...
    if (target.setsActiveProject) {
      ...
      const name =
        ctx.activeProjectId === row.project_id
          ...
          : (projectsLite.find((p) => p.id === row.project_id)?.name ??
            ctx.activeProjectName);
    ...
    if (target.setsActiveProject) {
      ...
      const name =
        ctx.activeProjectId === item.project_id
          ? ctx.activeProjectName
          : (projectsLite.find((p) => p.id === item.project_id)?.name ??
            ctx.activeProjectName);
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <EmptyState
          ...
          title={t("files.no_project_title", {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-border-light bg-surface-elevated">
        ...
        >
          ...
          {!showFolderGrid && (
            <>
              ...
              />
              <span
                className="px-2 py-1 text-content-primary font-semibold truncate"
                title={activeKindLabel}
              >
                {activeKindLabel}
              </span>
            </>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            title={t("files.global_search.title", {
              defaultValue: "Search across projects",
            })}
          >
            <Search size={13} />
            <span className="hidden md:inline">
              {t("files.global_search.short", {
                defaultValue: "Search all projects",
              })}
            </span>
          </Link>
          ...
          <Link
            ...
            className="hidden sm:inline-flex items-center gap-1.5 h-9 px-2.5 rounded-lg text-xs font-medium text-content-secondary hover:text-content-primary hover:bg-surface-secondary transition-colors"
            title={t("files.transmittals.open_log", {
              defaultValue: "Transmittal log",
            })}
          >
            <Send size={13} />
            <span className="hidden md:inline">
              {t("files.transmittals.open_log", {
                defaultValue: "Transmittal log",
              })}
            </span>
          </Link>
          ...
          {showFolderGrid ? (
            ...
            <>
              <FileActionsBar
                ...
                totalCount={
                  tagFilterActive ? tagFilteredItems.length : (list?.total ?? 0)
                }
                extension={extension}
      ...
      <FolderPermissionsModal
        ...
        folderLabel={
          permsKind
            ? t(`files.category.${permsKind}`, { defaultValue: permsKind })
            : undefined
        }
