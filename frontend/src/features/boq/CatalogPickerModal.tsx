...
const TYPE_FILTERS = [
  ...
  {
    key: "material",
    labelKey: "catalog.material",
    defaultLabel: "Material",
    icon: Package,
  },
  {
    key: "labor",
    labelKey: "catalog.labor",
    defaultLabel: "Labor",
    icon: Hammer,
  },
  {
    key: "equipment",
    labelKey: "catalog.equipment",
    defaultLabel: "Equipment",
    icon: Cpu,
  },
  {
    key: "operator",
    labelKey: "catalog.operator",
...
export function CatalogPickerModal({
  ...
  useEffect(() => {
    ...
    apiGet<{ items: CatalogResource[]; total: number }>(
  ...
  return createPortal(
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label={t("boq.pick_from_catalog", {
        ...
        <div className="flex-1 overflow-y-auto">
          {initialLoad && loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center px-6">
              ...
              <p className="text-sm font-medium text-content-secondary mb-1">
                {debouncedQuery || typeFilter
                  ? t("boq.no_catalog_results", {
                      defaultValue: "No resources match your search",
                    })
                  : t("boq.catalog_empty", {
                      defaultValue: "No resources in catalog",
                    })}
              </p>
              <p className="text-xs text-content-tertiary">
                {debouncedQuery || typeFilter
                  ? t("boq.try_different_search", {
                      defaultValue: "Try a different search term or filter",
                    })
            ...
            <div className="divide-y divide-border-light">
              {results.map((res) => {
                const badgeStyle =
                  TYPE_BADGE_STYLE[res.resource_type] ?? TYPE_BADGE_STYLE.other;
                return (
                  <button
                    key={res.id}
                    type="button"
