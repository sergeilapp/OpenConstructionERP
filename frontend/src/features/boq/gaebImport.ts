...
function walkBoQBody(
  ...
  for (const child of Array.from(el.children)) {
    ...
    if (tag === "BoQCtgy") {
      ...
      const ctgyNo =
        child.getAttribute("RNoPart") ?? child.getAttribute("OrdinalNo") ?? "";
      ...
      const lblTxEl = child.querySelector(
        ":scope > LblTx, :scope > Description > LblTx",
      );
      const label =
        lblTxEl?.textContent?.replace(/\s+/g, " ").trim() || sectionLabel;
 ...
 */
export async function importGAEBToBOQ(
  file: File,
  boqId: string,
  ...
  for (const pos of positions) {
    ...
    try {
      ...
      const label = pos.ordinal
        ? `${pos.ordinal} — ${pos.description}`
        : pos.description;
