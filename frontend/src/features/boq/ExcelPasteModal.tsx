...
export function ExcelPasteModal({
  ...
  const totalSum = useMemo(
    () => rows.reduce((s, r) => s + r.quantity * r.unit_rate, 0),
    [rows],
  );
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold">
                {t("boq.paste_from_excel", {
                  defaultValue: "Paste from Excel‌⁠‍",
                })}
              </h2>
              <p className="text-xs text-content-secondary">
                {t("boq.paste_excel_hint", {
                  defaultValue:
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <textarea
            ...
            placeholder={t("boq.paste_placeholder", {
              defaultValue:
          ...
          {rows.length > 0 && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-content-secondary">
                {t("boq.paste_detected", {
                  defaultValue: "{{count}} rows detected‌⁠‍",
                  count: rows.length,
                })}
              </span>
              {detectedHeaders.length > 0 && (
                <span className="text-xs text-content-tertiary">
                  ({t("boq.paste_columns", { defaultValue: "Columns‌⁠‍" })}:{" "}
            ...
            <div className="rounded-xl border border-border-light overflow-hidden">
                ...
                <table className="w-full text-xs">
                  <thead className="bg-surface-secondary sticky top-0">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-center font-semibold text-content-secondary">
                        {t("boq.unit", { defaultValue: "Unit" })}
                      </th>
                      <th className="px-3 py-2 text-right font-semibold text-content-secondary">
                  ...
                  <tbody className="divide-y divide-border-light">
                    {rows.slice(0, 50).map((r, i) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-1.5 text-center font-mono uppercase">
                          {r.unit}
                        </td>
                        ...
                        <td className="px-3 py-1.5 text-right tabular-nums">
                          {r.unit_rate.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                          })}
                        </td>
                        <td className="px-3 py-1.5 text-right tabular-nums font-medium">
                          {(r.quantity * r.unit_rate).toLocaleString(
                            undefined,
                            { minimumFractionDigits: 2 },
                          )}
                        </td>
              ...
              {rows.length > 50 && (
                <div className="px-3 py-1.5 text-xs text-content-tertiary bg-surface-secondary text-center">
                  {t("boq.paste_showing", {
                    defaultValue: "Showing first 50 of {{total}} rows",
                    total: rows.length,
                  })}
                </div>
              )}
