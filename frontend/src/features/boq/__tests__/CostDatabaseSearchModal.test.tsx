...
describe("CostDatabaseSearchModal — paginated catalog", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (
      fetchCategoryTree as unknown as ReturnType<typeof vi.fn>
  ...
  it("fetches the first page + the category tree on mount", async () => {
    ...
    const calls = (fetchCostSearch as unknown as ReturnType<typeof vi.fn>).mock
      .calls;
  ...
  it("combines selected category + text query in a single fetch", async () => {
    ...
    await waitFor(() => {
      const last = (
        fetchCostSearch as unknown as ReturnType<typeof vi.fn>
      ).mock.calls.at(-1)?.[0];
