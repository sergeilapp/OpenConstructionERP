...
function buildPreview(
  ...
  const fmtSection = (idx: number) =>
    pad ? String(idx).padStart(2, "0") : String(idx);
    ...
    for (const child of children) {
      ...
      if (isSection(child)) {
        ...
        if (parentOrd) {
          ...
          newOrd = useDotted
            ? String(leafIdx * step)
            : String(leafIdx * step).padStart(
                scheme === "gap10" || scheme === "gap100" ? 4 : 2,
                "0",
              );
        }
      }
          ...
          <label className="flex items-center justify-between gap-3 rounded-xl border border-border bg-surface-base px-3 py-2.5 cursor-pointer hover:bg-surface-secondary/40 transition-colors">
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium text-content-primary">
                {t("boq.renumber_pad", {
                  defaultValue: "Zero-pad section numbers‌⁠‍",
                })}
              </div>
              <div className="text-[11px] text-content-tertiary">
                {pad
                  ? t("boq.renumber_pad_on", {
                      defaultValue:
                        "Sections will be 01, 02, 03 (two-digit padded)‌⁠‍",
                    })
                  : t("boq.renumber_pad_off", {
                      defaultValue: "Sections will be 1, 2, 3 (no padding)",
                    })}
              </div>
            </div>
            <button
          ...
          <div className="rounded-xl border border-border bg-surface-base overflow-hidden">
            <div className="px-3 py-2 border-b border-border bg-surface-secondary/40">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-content-tertiary">
                {t("boq.renumber_preview", {
                  defaultValue: "Preview (first 5 positions)",
                })}
              </div>
            </div>
            {preview.length === 0 ? (
              <div className="px-3 py-4 text-xs text-content-tertiary text-center">
                {t("boq.renumber_no_preview", {
                  defaultValue: "No positions to preview yet.",
                })}
              </div>
            ) : (
