...
import {
  boqApi,
  type Markup,
  type CreateMarkupData,
  type UpdateMarkupData,
} from "./api";
import { fmtWithCurrency } from "./boqHelpers";
import { useToastStore } from "@/stores/useToastStore";
import clsx from "clsx";
import { ChevronDown, Plus, Trash2, Globe, GripVertical } from "lucide-react";
...
export function MarkupPanel({
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("boq.markup_added", { defaultValue: "Markup added‌⁠‍" }),
      });
    },
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("boq.markup_add_failed", {
          defaultValue: "Failed to add markup‌⁠‍",
        }),
        message: err.message,
      });
    },
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("boq.markup_update_failed", {
          defaultValue: "Failed to update markup‌⁠‍",
        }),
        message: err.message,
      });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("boq.markup_deleted", { defaultValue: "Markup deleted‌⁠‍" }),
      });
    },
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("boq.markup_delete_failed", {
          defaultValue: "Failed to delete markup‌⁠‍",
        }),
        message: err.message,
      });
    },
  ...
  const applyDefaultsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast({
        type: "success",
        title: t("boq.template_applied", {
          defaultValue: "Regional template applied",
        }),
      });
    },
    onError: (err: Error) => {
      addToast({
        type: "error",
        title: t("boq.apply_defaults_failed", {
  ...
  const calculated = (Array.isArray(markups) ? markups : [])
    ...
    .map((m) => {
      ...
      if (m.markup_type === "fixed") {
        amount =
          typeof m.fixed_amount === "number" && Number.isFinite(m.fixed_amount)
            ? m.fixed_amount
            : 0;
  ...
  return (
    <div className="mt-4 rounded-xl border border-border-light bg-surface-elevated shadow-xs">
      ...
      <button
        ...
        aria-label={t("boq.markups_title", {
          defaultValue: "Markups & Overheads",
        })}
        ...
        <div className="border-t border-border-light">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-2.5 bg-surface-secondary/30">
            ...
            <div className="relative">
              <button
                ...
                aria-haspopup="true"
                aria-label={t("boq.apply_template", {
                  defaultValue: "Apply Regional Template",
                })}
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (
                          markups.length > 0 &&
                          !confirm(
                            t("boq.confirm_replace_markups", {
                              defaultValue:
                                "This will replace existing markups. Continue?",
                            }),
                          )
                        ) {
                          setShowRegionMenu(false);
          ...
          {markups.length > 0 ? (
                ...
                <thead>
                  <tr className="border-b border-border-light bg-surface-secondary/20 text-content-tertiary text-xs">
                    ...
                    <th className="text-left px-3 py-2 font-medium">
                      {t("boq.markup_name", { defaultValue: "Name" })}
                    </th>
                    <th className="text-left px-3 py-2 font-medium">
                      {t("boq.markup_category", { defaultValue: "Category" })}
                    </th>
                    <th className="text-right px-3 py-2 font-medium w-20">
                      {t("boq.markup_percentage", { defaultValue: "%" })}
                    </th>
                    <th className="text-right px-3 py-2 font-medium w-32">
                      {t("boq.markup_amount", { defaultValue: "Amount" })}
                    </th>
                    <th className="text-center px-3 py-2 font-medium w-16">
                      {t("boq.markup_active", { defaultValue: "Active" })}
                    </th>
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          {isEditing && editState.field === "name" ? (
                            ...
                            <span
                              ...
                              onClick={() =>
                                handleStartEdit(markup.id, "name", markup.name)
                              }
                            >
                              {markup.name}
                            </span>
                          ...
                          {isEditing && editState.field === "category" ? (
                            ...
                            <span
                              className={clsx(
                                ...
                                CATEGORY_COLORS[markup.category] ??
                        ...
                        <td className="px-3 py-2 text-center">
                          <button
                            ...
                            aria-label={t("boq.markup_active", {
                              defaultValue: "Active",
                            })}
                          >
                            <span
            ...
            <div className="px-5 py-6 text-center text-sm text-content-tertiary">
              {t("boq.no_markups", {
                defaultValue:
                  "No markups yet. Add one or apply a regional template.",
              })}
            </div>
          )}
            ...
            <div className="border-t border-border-light px-5 py-3 bg-surface-secondary/20">
              <div className="flex items-center justify-between gap-4 text-sm">
                <span className="text-content-tertiary whitespace-nowrap">
                  {t("boq.direct_cost", { defaultValue: "Direct Cost" })}
                </span>
                <span className="tabular-nums text-content-secondary whitespace-nowrap shrink-0">
                  {fmtWithCurrency(directCost, locale, currencyCode)}
                </span>
              </div>
              {calculated.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="flex items-center justify-between gap-4 text-sm mt-1"
                  >
                    <span className="text-content-tertiary min-w-0 truncate">
                      + {m.name} ({fmt.format(m.percentage)}%)
                    </span>
                    <span className="tabular-nums text-content-secondary whitespace-nowrap shrink-0">
                      {fmtWithCurrency(c.amount, locale, currencyCode)}
                    </span>
                  </div>
                );
              })}
              <div className="flex items-center justify-between gap-4 text-sm font-semibold mt-2 pt-2 border-t border-border-light">
                <span className="text-content-primary whitespace-nowrap">
                  {t("boq.net_total", { defaultValue: "Net Total" })}
                </span>
                <span className="tabular-nums text-content-primary whitespace-nowrap shrink-0">
                  {fmtWithCurrency(netTotal, locale, currencyCode)}
                </span>
              </div>
            </div>
          )}
