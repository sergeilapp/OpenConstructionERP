...
describe("boqResourceTypes — getResourceTypeLabel", () => {
  ...
  it("passes the canonical i18n key + English defaultValue to `t`", () => {
    ...
    expect(getResourceTypeLabel("equipment", fakeT)).toBe(
      "[boq.resource_type_equipment]",
    );
    expect(calls).toEqual([
      ["boq.resource_type_equipment", { defaultValue: "Equipment‌⁠‍" }],
    ]);
  });
});
...
describe("boqResourceTypes — getResourceTypeI18nKey", () => {
  it("maps canonical values to their i18n key", () => {
    expect(getResourceTypeI18nKey("material")).toBe(
      "boq.resource_type_material",
    );
    expect(getResourceTypeI18nKey("composite")).toBe(
      "boq.resource_type_composite",
    );
  });

  it("falls back to the `other` key for unknown values", () => {
    expect(getResourceTypeI18nKey("legacy_unmapped")).toBe(
      "boq.resource_type_other",
    );
...
describe("integration — getResourceTypeLabel hooked up to live bundles", () => {
  ...
  // when key is missing.
  const makeT =
    (locale: string) => (key: string, opts?: Record<string, string>) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const bundle = (fallbackResources as any)[locale]?.translation as
        | Record<string, string>
        | undefined;
