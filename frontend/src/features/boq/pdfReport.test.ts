...
vi.mock("jspdf-autotable", () => ({
  default: vi
    .fn()
    .mockImplementation((doc: { lastAutoTable: { finalY: number } }) => {
...
function baseOptions(
  overrides: Partial<PdfReportOptions> = {},
): PdfReportOptions {
  const positions = [
...
describe("generateBOQPdf", () => {
  ...
  it("uses the boqTitle as the filename base (sanitised)", async () => {
    ...
    expect(mockSave).toHaveBeenCalledWith(
      expect.stringMatching(/^My Project  Phase 1\.pdf$/),
    );
  });
