...
export function CreateBOQModal({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      boqApi.create({ project_id: selectedProjectId, name, description }),
    onSuccess: (boq) => {
      queryClient.invalidateQueries({ queryKey: ["boqs", selectedProjectId] });
      queryClient.invalidateQueries({ queryKey: ["all-boqs"] });
      addToast({
        type: "success",
        title: t("toasts.boq_created", {
          defaultValue: "Bill of Quantities created successfully‌⁠‍",
        }),
      });
      onClose();
      navigate(`/boq/${boq.id}`);
    ...
    onError: (error: Error) => {
      addToast({
        type: "error",
        title: t("toasts.boq_create_failed", {
          defaultValue: "Failed to create Bill of Quantities‌⁠‍",
        }),
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-content-tertiary">
                {t("boq.create_subtitle", {
                  defaultValue: "Create a new bill of quantities‌⁠‍",
                })}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={t("boq.scope_placeholder", {
                defaultValue: "Scope of this BOQ...",
              })}
              rows={3}
          ...
          {mutation.error && (
            <div className="rounded-lg bg-semantic-error-bg px-3 py-2 text-sm text-semantic-error">
              {(mutation.error as Error).message ||
                t("boq.create_failed", "Failed to create BOQ")}
            </div>
          )}
...
export function CreateBOQPage() {
  ...
  useEffect(() => {
    navigate("/boq", {
      state: { openCreateModal: true, projectId },
      replace: true,
    });
  }, [navigate, projectId]);
