...
export function BIMViewCube({
  ...
  useEffect(() => {
    ...
    let mainCamRef: THREE.Camera | null =
      sceneManagerRef.current?.camera ?? null;
    ...
    function syncCubeRotation() {
      ...
      mainCamRef.matrixWorld.decompose(
        new THREE.Vector3(),
        tmpQuat,
        new THREE.Vector3(),
      );
      ...
      <div className="sr-only">
        {FACE_DEFS.map((face) => (
          <button
            ...
            onClick={() => {
              sceneManagerRef.current?.setViewPreset(face.preset).catch(() => {
