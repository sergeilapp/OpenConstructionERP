...
export function ddcGetFingerprint(): string {
  return ["DDC", "CWICR", "OE", new Date().getFullYear().toString()].join("-");
}

/** @internal DDC-CWICR-OE-2026 origin marker */
export const DDC_ORIGIN =
  "DataDrivenConstruction/OpenConstructionERP/CWICR" as const;
...
export function ddcInjectMeta(): void {
  ...
  try {
    if (
      ...
      !localStorage.getItem("_ff_build_hash")
    ) {
